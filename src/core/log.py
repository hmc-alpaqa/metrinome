...
class Log:
        ...
        if self._display_output and self._log_level is LogLevel.DEBUG:
            for msg in msgs:
                ...
                if len(self._tag) == 0:
                    ...
                    print(f"{self._tag}{Colors.OKBLUE.value}{debug_str}{Colors.ENDC.value}: {msg}")

    def i_msg(self, *msgs: str) -> None:
        """Log an information-level message."""
        if self._display_output:
            for msg in msgs:
                print(f"{self._tag} ~~~ {msg.upper()} ~~~")
    ...
    def e_msg(self, *msgs: str) -> None:
        ...
        if self._display_output:
            for msg in msgs:
                beginning = f" {self._tag} > {Colors.WARNING.value} !!! {Colors.ENDC.value} "
