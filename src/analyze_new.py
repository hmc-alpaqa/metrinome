...
if __name__ == "__main__":
    subprocess.run("mkdir /app/code/tests/cFiles/simpleAlgs/autobpgraphs/", shell=True)
    field = "CompletedPaths"
    functions = ['01_greatestof3', '13_check_arrays_equal', '22_selectionsort', '02_fib', '14_lexicographic_array_compare',
    ...
    for f in functions:
        ...
        predicted = func_to_optimize(X, *p)

        left = X[:list(X).index(best_bp)]
        right = X[list(X).index(best_bp):]
        leftY = Y[:list(X).index(best_bp)]
        rightY = Y[list(X).index(best_bp):]
        ...
        plt.savefig(f"/app/code/tests/cFiles/simpleAlgs/autobpgraphs/{field}_{f}".replace("%", "percent"))
        plt.close()
