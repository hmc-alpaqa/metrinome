...
def klee_compare(file_name, preferences, depths, inputs, function):
    ...
    for preference in preferences:
        for depth in depths:
            for input in inputs:
                output_file = f"/app/code/tests/cFiles/simpleAlgs/klee_{preference}_{depth}_{input}_{function}_output".replace(" ","_")
                results = klee_with_preferences(file_name, output_file, preference, depth, input)
                stats = subprocess.run(f"{klee_path}-stats --table-format=simple --print-all {output_file}", shell=True, stdout=PIPE, stderr=PIPE)
