...
__attribute__((always_inline)) inline struct document get_document(char* text) {
  struct document doc;
...
__attribute__((always_inline)) inline struct word
kth_word_in_mth_sentence_of_nth_paragraph(struct document Doc, int k, int m,
                                          int n) {
  return Doc.data[n - 1].data[m - 1].data[k - 1];
}

__attribute__((always_inline)) inline struct sentence
kth_sentence_in_mth_paragraph(struct document Doc, int k, int m) {
  return Doc.data[m - 1].data[k - 1];
}

__attribute__((always_inline)) inline struct paragraph kth_paragraph(
    struct document Doc, int k) {
  return Doc.data[k - 1];
}

__attribute__((always_inline)) inline void print_word(struct word w) {
  printf("%s", w.data);
}

__attribute__((always_inline)) inline void print_sentence(struct sentence sen) {
...
__attribute__((always_inline)) inline void print_paragraph(
...
__attribute__((always_inline)) inline void print_document(struct document doc) {
  for (int i = 0; i < doc.paragraph_count; i++) {
...
__attribute__((always_inline)) inline char* get_input_text() {
  int paragraph_count;
