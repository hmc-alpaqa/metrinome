...
#define AUTHORS \
  proper_name("Torbjorn Granlund"), proper_name("Richard M. Stallman")
...
__attribute__((always_inline)) static inline bool ignorable(int err) { return filter_command && err == EPIPE; }

__attribute__((always_inline)) inline static void set_suffix_length(uintmax_t n_units, enum Split_type split_type) {
#define DEFAULT_SUFFIX_LENGTH 2
  ...
  if (numeric_suffix_start) suffix_auto = false;
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines ||
    ...
    if (numeric_suffix_start) {
      ...
      if (e == LONGINT_OK && n_start <= UINTMAX_MAX - n_units) {
        ...
        if (n_start < n_units) n_units_end += n_start;
      }
    }
...
__attribute__((always_inline)) inline static off_t input_file_size(int fd, struct stat const *st, char *buf,
                             size_t bufsize) {
  off_t cur = lseek(fd, 0, SEEK_CUR);
  if (cur < 0) {
    if (errno == ESPIPE) errno = 0; /* Suppress confusing seek error.  */
    return -1;
  }
  ...
  do {
    ...
    if (n_read == 0) return size;
    if (n_read == SAFE_READ_ERROR) return -1;
    size += n_read;
  } while (size < bufsize);
...
__attribute__((always_inline)) inline static void next_file_name(void) {
  ...
  new_name:
    ...
    if (outfile_length + 1 < outbase_length) xalloc_die();
    ...
    if (numeric_suffix_start) {
      ...
      while (i-- != 0) *--sufindex_end = numeric_suffix_start[i] - '0';
    }
...
__attribute__((always_inline)) inline static int create(const char *name) {
  if (!filter_command) {
    if (verbose) fprintf(stdout, _("creating file %s\n"), quoteaf(name));
    ...
    if (shell_prog == NULL) shell_prog = "/bin/sh";
    ...
    if (child_pid == 0) {
      ...
      if (close(fd_pair[1])) die(EXIT_FAILURE, errno, _("closing output pipe"));
    ...
    if (child_pid == -1) die(EXIT_FAILURE, errno, _("fork system call failed"));
    if (close(fd_pair[0]) != 0)
...
__attribute__((always_inline)) inline static void closeout(FILE *fp, int fd, pid_t pid, char const *name) {
  ...
  if (pid > 0) {
    ...
    if (WIFSIGNALED(wstatus)) {
      ...
      if (sig != SIGPIPE) {
        ...
        if (sig2str(sig, signame) != 0) sprintf(signame, "%d", sig);
      ...
      /* shouldn't happen.  */
      die(EXIT_FAILURE, 0, _("unknown status from command (0x%X)"),
          wstatus + 0u);
    }
  }
}
...
__attribute__((always_inline)) inline static bool cwrite(bool new_file_flag, const char *bp, size_t bytes) {
  if (new_file_flag) {
    if (!bp && bytes == 0 && elide_empty_files) return true;
    closeout(NULL, output_desc, filter_pid, outfile);
    next_file_name();
    output_desc = create(outfile);
    if (output_desc < 0) die(EXIT_FAILURE, errno, "%s", quotef(outfile));
  }

  if (full_write(output_desc, bp, bytes) == bytes)
    return true;
  else {
    if (!ignorable(errno)) die(EXIT_FAILURE, errno, "%s", quotef(outfile));
    return false;
  }
}
...
__attribute__((always_inline)) inline static void bytes_split(uintmax_t n_bytes, char *buf, size_t bufsize,
  ...
  while (opened++ < max_files) cwrite(true, NULL, 0);
}

/* Split into pieces of exactly N_LINES lines.
   Use buffer BUF, whose size is BUFSIZE.  */

__attribute__((always_inline)) inline static void lines_split(uintmax_t n_lines, char *buf, size_t bufsize) {
  size_t n_read;
  char *bp, *bp_out, *eob;
  bool new_file_flag = true;
  uintmax_t n = 0;
...
__attribute__((always_inline)) inline static void line_bytes_split(uintmax_t n_bytes, char *buf, size_t bufsize) {
  size_t n_read;
  ...
  do {
    ...
    while (n_left) {
      ...
      if (n_hold && !(!eol && n_out)) {
        ...
        n_out += n_hold;
        if (n_hold > bufsize) hold = xrealloc(hold, bufsize);
        n_hold = 0;
        hold_size = bufsize;
      }
  ...
  if (n_hold) cwrite(n_out == 0, hold, n_hold);
...
__attribute__((always_inline)) inline static void lines_chunk_split(uintmax_t k, uintmax_t n, char *buf,
  ...
  if (chunk_truncated) chunk_no++;

  /* Ensure NUMBER files are created, which truncates
     any existing files or notifies any consumers on fifos.
     FIXME: Should we do this before EXIT_FAILURE?  */
  while (!k && chunk_no++ <= n) cwrite(true, NULL, 0);
}

/* -n K/N: Extract Kth of N chunks.  */

__attribute__((always_inline)) inline static void bytes_chunk_extract(uintmax_t k, uintmax_t n, char *buf,
                                size_t bufsize, size_t initial_read,
                                off_t file_size) {
  off_t start;
  off_t end;
...
__attribute__((always_inline)) inline static bool ofile_open(of_t *files, size_t i_check, size_t nfiles) {
...
__attribute__((always_inline)) inline static void lines_rr(uintmax_t k, uintmax_t n, char *buf, size_t bufsize) {
  bool wrapped = false;
  bool wrote = false;
  bool file_limit;
  size_t i_file;
  ...
  while (true) {
    ...
    while (bp != eob) {
      ...
      if (k) {
        ...
        if (next) line_no = (line_no == n) ? 1 : line_no + 1;
      } else {
        ...
        if (unbuffered) {
          ...
          if (full_write(files[i_file].ofd, bp, to_write) != to_write &&
              !ignorable(errno)) {
            die(EXIT_FAILURE, errno, "%s", quotef(files[i_file].of_name));
          }
...
__attribute__((always_inline)) inline static void parse_chunk(uintmax_t *k_units, uintmax_t *n_units, char *slash) {
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    c = getopt_long(argc, argv, "0123456789C:a:b:del:n:t:ux", longopts, NULL);
    if (c == -1) break;
    ...
    switch (c) {
      ...
      case 'b':
        if (split_type != type_undef) FAIL_ONLY_ONE_WAY();
      ...
      case 'n':
        if (split_type != type_undef) FAIL_ONLY_ONE_WAY();
        /* skip any whitespace */
        while (isspace(to_uchar(*optarg))) optarg++;
      ...
      case 't': {
        ...
        if (!neweol) die(EXIT_FAILURE, 0, _("empty record separator"));
        if (optarg[1]) {
          if (STREQ(optarg, "\\0"))
            neweol = '\0';
          else {
        ...
        if (0 <= eolchar && neweol != eolchar) {
          die(EXIT_FAILURE, 0, _("multiple separator characters specified"));
        }
      ...
      case '9':
        ...
        if (!DECIMAL_DIGIT_ACCUMULATE(n_units, c - '0', uintmax_t)) {
          ...
          die(EXIT_FAILURE, 0, _("line count option -%s%c... is too large"),
      ...
      case 'x':
        ...
        if (optarg) {
          if (strlen(optarg) != strspn(optarg, suffix_alphabet)) {
            ...
            while (*optarg == '0' && *(optarg + 1) != '\0') optarg++;
  ...
  if (optind < argc) outbase = argv[optind++];
  ...
  if (numeric_suffix_start && strlen(numeric_suffix_start) > suffix_length) {
    error(0, 0,
          _("numerical suffix start value is too large "
  ...
  if (!STREQ(infile, "-") && fd_reopen(STDIN_FILENO, infile, O_RDONLY, 0) < 0)
    die(EXIT_FAILURE, errno, _("cannot open %s for reading"), quoteaf(infile));
  ...
  if (!specified_buf_size) in_blk_size = io_blksize(in_stat_buf);
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines) {
    ...
    if (OFF_T_MAX < n_units) {
      char buffer[INT_BUFSIZE_BOUND(uintmax_t)];
      die(EXIT_FAILURE, EOVERFLOW, "%s: %s", _("invalid number of chunks"),
  ...
  if (filter_command) {
    ...
    if (act.sa_handler != SIG_IGN) sigaddset(&newblocked, SIGPIPE);
  ...
  if (close(STDIN_FILENO) != 0) die(EXIT_FAILURE, errno, "%s", quotef(infile));
  closeout(NULL, output_desc, filter_pid, outfile);
