...
__attribute__((always_inline)) inline void extent_scan_init(int src_fd, struct extent_scan *scan);

__attribute__((always_inline)) inline bool extent_scan_read(struct extent_scan *scan);

__attribute__((always_inline)) static inline void extent_scan_free(struct extent_scan *scan) {
  free(scan->ext_info);
  scan->ext_info = NULL;
  scan->ei_count = 0;
}
