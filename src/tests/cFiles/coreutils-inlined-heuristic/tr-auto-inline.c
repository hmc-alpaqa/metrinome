...
__attribute__((always_inline)) static inline bool es_match(struct E_string const *es, size_t i, char c) {
...
__attribute__((always_inline)) static inline bool is_equiv_class_member(unsigned char equiv_class,
                                         unsigned char c) {
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE is_char_class_member(enum Char_class char_class,
                                                    unsigned char c) {
  int result;
...
__attribute__((always_inline)) inline static void es_free(struct E_string *es) {
  free(es->s);
  free(es->escaped);
}
...
__attribute__((always_inline)) inline static bool unquote(char const *s, struct E_string *es) {
  size_t len = strlen(s);
 ...
 \\%c%c%c is being\n\tinterpreted as the 2-byte sequence \\0%c%c, %c"),
          ...
          case '\0':
            error(0, 0,
                  _("warning: an unescaped backslash "
...
__attribute__((always_inline)) inline static char *make_printable_char(unsigned char c) {
  char *buf = xmalloc(5);
...
__attribute__((always_inline)) inline static char *make_printable_str(char const *s, size_t len) {
...
__attribute__((always_inline)) inline static void append_normal_char(struct Spec_list *list, unsigned char c) {
...
__attribute__((always_inline)) inline static bool append_range(struct Spec_list *list, unsigned char first,
                         unsigned char last) {
  if (last < first) {
...
__attribute__((always_inline)) inline static bool append_char_class(struct Spec_list *list,
                              char const *char_class_str, size_t len) {
  enum Char_class char_class = look_up_char_class(char_class_str, len);
  if (char_class == CC_NO_CLASS) return false;
  struct List_element *new = xmalloc(sizeof *new);
  new->next = NULL;
...
__attribute__((always_inline)) inline static void append_repeated_char(struct Spec_list *list, unsigned char the_char,
                                 count repeat_count) {
  struct List_element *new = xmalloc(sizeof *new);
  new->next = NULL;
...
__attribute__((always_inline)) inline static bool append_equiv_class(struct Spec_list *list,
...
__attribute__((always_inline)) inline static bool find_closing_delim(const struct E_string *es, size_t start_idx,
...
__attribute__((always_inline)) inline static int find_bracketed_repeat(const struct E_string *es, size_t start_idx,
                                 unsigned char *char_to_repeat,
                                 count *repeat_count,
                                 size_t *closing_bracket_idx) {
  assert(start_idx + 1 < es->len);
  if (!es_match(es, start_idx + 1, '*')) return -1;
...
__attribute__((always_inline)) inline static bool build_spec_list(const struct E_string *es,
  ...
  for (i = 0; i + 2 < es->len; /* empty */) {
          ...
          if (p[i + 1] == ':') {
            ...
            if (!append_equiv_class(result, opnd_str, opnd_str_len)) {
              if (star_digits_closebracket(es, i + 2))
                ...
                char *tmp = make_printable_str(opnd_str, opnd_str_len);
                error(0, 0,
                      _("%s: equivalence class operand must be a single "
                        "character"),
                      tmp);
                free(tmp);
                return false;
    ...
    try_bracketed_repeat:
      ...
      if (matched_multi_char_construct) continue;
    ...
    if (es_match(es, i + 1, '-')) {
      if (!append_range(result, p[i], p[i + 2])) return false;
      i += 3;
    } else {
...
__attribute__((always_inline)) inline static void skip_construct(struct Spec_list *s) {
  s->tail = s->tail->next;
  s->state = NEW_ELEMENT;
}
...
__attribute__((always_inline)) inline static int get_next(struct Spec_list *s, enum Upper_Lower_class *class) {
  struct List_element *p;
  int return_val;
  int i;

  if (class) *class = UL_NONE;
  ...
  switch (p->type) {
    ...
    case RE_CHAR_CLASS:
      ...
      if (s->state == NEW_ELEMENT) {
        for (i = 0; i < N_CHARS; i++)
          if (is_char_class_member(p->u.char_class, i)) break;
        assert(i < N_CHARS);
        s->state = i;
      }
...
__attribute__((always_inline)) inline static int card_of_complement(struct Spec_list *s) {
  int c;
...
__attribute__((always_inline)) inline static void validate_case_classes(struct Spec_list *s1, struct Spec_list *s2) {
  size_t n_upper = 0;
  size_t n_lower = 0;
  int c1 = 0;
  int c2 = 0;
  ...
  if (!s2->has_char_class) return;

  for (int i = 0; i < N_CHARS; i++) {
    if (isupper(i)) n_upper++;
    if (islower(i)) n_lower++;
  }
...
__attribute__((always_inline)) inline static void get_spec_stats(struct Spec_list *s) {
  struct List_element *p;
  count length = 0;
  ...
  for (p = s->head->next; p; p = p->next) {
    ...
    switch (p->type) {
      ...
      case RE_CHAR_CLASS:
        ...
        for (int i = 0; i < N_CHARS; i++)
          if (is_char_class_member(p->u.char_class, i)) ++len;
        switch (p->u.char_class) {
          case CC_UPPER:
          case CC_LOWER:
            break;
      ...
      case RE_EQUIV_CLASS:
        for (int i = 0; i < N_CHARS; i++)
          if (is_equiv_class_member(p->u.equiv_code, i)) ++len;
...
__attribute__((always_inline)) inline static void get_s1_spec_stats(struct Spec_list *s1) {
  get_spec_stats(s1);
  if (complement) s1->length = card_of_complement(s1);
}

__attribute__((always_inline)) inline static void get_s2_spec_stats(struct Spec_list *s2, count len_s1) {
  get_spec_stats(s2);
...
__attribute__((always_inline)) inline static void spec_init(struct Spec_list *spec_list) {
  struct List_element *new = xmalloc(sizeof *new);
...
__attribute__((always_inline)) inline static bool parse_str(char const *s, struct Spec_list *spec_list) {
  struct E_string es;
...
__attribute__((always_inline)) inline static void string2_extend(const struct Spec_list *s1, struct Spec_list *s2) {
  struct List_element *p;
  unsigned char char_to_repeat;
...
__attribute__((always_inline)) inline static bool homogeneous_spec_list(struct Spec_list *s) {
  int b, c;

  s->state = BEGIN_STATE;

  if ((b = get_next(s, NULL)) == -1) return false;
...
__attribute__((always_inline)) inline static void validate(struct Spec_list *s1, struct Spec_list *s2) {
  ...
  if (s2) {
    ...
    if (translating) {
      if (s2->has_equiv_class) {
        die(EXIT_FAILURE, 0, _("[=c=] expressions may not appear in string2\
 when translating"));
      }
...
\nstring2 must map all characters in the domain to one"));
    ...
    /* Not translating.  */
    {
      if (s2->n_indefinite_repeats > 0)
        die(EXIT_FAILURE, 0, _("the [c*] construct may appear in string2 only\
 when translating"));
    }
  }
}
...
__attribute__((always_inline)) inline static void squeeze_filter(char *buf, size_t size,
  ...
  while (true) {
    ...
    if (char_to_squeeze == NOT_A_CHAR) {
      ...
      /* There is a special case when i == nr and we've just
         ...
         the squeeze set.  */
      if (i == nr && in_squeeze_set[to_uchar(buf[i - 1])]) --i;
      ...
      if (i >= nr)
        ...
        if (i > 0 && buf[i - 1] == char_to_squeeze) --out_len;
    ...
    if (char_to_squeeze != NOT_A_CHAR) {
      ...
      if (i < nr) char_to_squeeze = NOT_A_CHAR;
...
__attribute__((always_inline)) inline static size_t plain_read(char *buf, size_t size) {
  size_t nr = safe_read(STDIN_FILENO, buf, size);
  if (nr == SAFE_READ_ERROR) die(EXIT_FAILURE, errno, _("read error"));
  return nr;
}
...
__attribute__((always_inline)) inline static size_t read_and_delete(char *buf, size_t size) {
  size_t n_saved;
  ...
  do {
    ...
    size_t i;
    for (i = 0; i < nr && !in_delete_set[to_uchar(buf[i])]; i++) continue;
    n_saved = i;

    for (++i; i < nr; i++)
      if (!in_delete_set[to_uchar(buf[i])]) buf[n_saved++] = buf[i];
  } while (n_saved == 0);
...
__attribute__((always_inline)) inline static size_t read_and_xlate(char *buf, size_t size) {
...
__attribute__((always_inline)) inline static void set_initialize(struct Spec_list *s, bool complement_this_set,
                           bool *in_set) {
  int c;

  s->state = BEGIN_STATE;
  while ((c = get_next(s, NULL)) != -1) in_set[c] = true;
  if (complement_this_set)
    for (size_t i = 0; i < N_CHARS; i++) in_set[i] = (!in_set[i]);
}
...
int main(int argc, char **argv) {
  ...
  spec_init(s1);
  if (!parse_str(argv[optind], s1)) return EXIT_FAILURE;

  if (non_option_args == 2) {
    spec_init(s2);
    if (!parse_str(argv[optind + 1], s2)) return EXIT_FAILURE;
  } else
    s2 = NULL;
  ...
  if (squeeze_repeats && non_option_args == 1) {
    ...
    if (complement) {
      ...
      s2->state = BEGIN_STATE;
      for (int i = 0; i < N_CHARS; i++) xlate[i] = i;
      for (int i = 0; i < N_CHARS; i++) {
        if (!in_s1[i]) {
      ...
      while (true) {
        ...
        if (class_s1 == UL_LOWER && class_s2 == UL_UPPER) {
          for (int i = 0; i < N_CHARS; i++)
            if (islower(i)) xlate[i] = toupper(i);
        } else if (class_s1 == UL_UPPER && class_s2 == UL_LOWER) {
          for (int i = 0; i < N_CHARS; i++)
            if (isupper(i)) xlate[i] = tolower(i);
        } else {
  ...
  if (close(STDIN_FILENO) != 0) die(EXIT_FAILURE, errno, _("standard input"));
