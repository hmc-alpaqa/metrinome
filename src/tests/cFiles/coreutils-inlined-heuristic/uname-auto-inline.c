...
__attribute__((always_inline)) inline static void print_element(char const *element) {
  static bool printed;
  if (printed) putchar(' ');
  printed = true;
...
int main(int argc, char **argv) {
  ...
  if (toprint == 0) toprint = PRINT_KERNEL_NAME;
  ...
  if (toprint & (PRINT_KERNEL_NAME | PRINT_NODENAME | PRINT_KERNEL_RELEASE |
    ...
    if (toprint & PRINT_KERNEL_NAME) print_element(name.sysname);
    if (toprint & PRINT_NODENAME) print_element(name.nodename);
    if (toprint & PRINT_KERNEL_RELEASE) print_element(name.release);
    if (toprint & PRINT_KERNEL_VERSION) print_element(name.version);
    if (toprint & PRINT_MACHINE) print_element(name.machine);
  }
  ...
  if (toprint & PRINT_PROCESSOR) {
    ...
    if (element == unknown) {
      ...
      if (sysctl(mib, 2, processor, &s, 0, 0) >= 0) element = processor;
...
#endif
    if (!(toprint == UINT_MAX && element == unknown)) print_element(element);
  }
  ...
  if (toprint & PRINT_OPERATING_SYSTEM) print_element(HOST_OPERATING_SYSTEM);
