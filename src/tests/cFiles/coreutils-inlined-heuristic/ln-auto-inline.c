...
__attribute__((always_inline)) inline static int errnoize(int status) { return status < 0 ? errno : 0; }

/* Return FROM represented as relative to the dir of TARGET.
   The result is malloced.  */

__attribute__((always_inline)) inline static char *convert_abs_rel(const char *from, const char *target) {
...
__attribute__((always_inline)) inline static int atomic_link(char const *source, int destdir_fd,
...
__attribute__((always_inline)) inline static bool do_link(char const *source, int destdir_fd, char const *dest_base,
  ...
  if (link_errno < 0) link_errno = atomic_link(source, destdir_fd, dest_base);
  ...
  if (link_errno) {
    if (!symbolic_link && !hard_dir_link && S_ISDIR(source_stats.st_mode)) {
      error(0, 0, _("%s: hard link not allowed for directory"), quotef(source));
      return false;
    }

    if (relative) source = rel_source = convert_abs_rel(source, dest);
    ...
    if (force) {
      ...
      if (fstatat(destdir_fd, dest_base, &dest_stats, AT_SYMLINK_NOFOLLOW) !=
        ...
        if (backup_type != no_backups ? !symbolic_link
          ...
          if (source_status != 0) source_status = stat(source, &source_stats);
          if (source_status == 0 && SAME_INODE(source_stats, dest_stats) &&
    ...
    link_errno =
        (symbolic_link
             ...
             : force_linkat(AT_FDCWD, source, destdir_fd, dest_base,
                            logical ? AT_SYMLINK_FOLLOW : 0, force,
                            link_errno));
  ...
  if (link_errno <= 0) {
    ...
    if (!symbolic_link) record_file(dest_set, dest, &source_stats);
    ...
    if (verbose) {
      ...
      printf("%s%s%s %c> %s\n", quoted_backup, backup_sep, quoteaf_n(0, dest),
