...
__attribute__((always_inline)) inline static void next_line_num(void) {
...
__attribute__((always_inline)) inline static bool simple_cat(
...
__attribute__((always_inline)) static inline void write_pending(char *outbuf, char **bpout) {
...
__attribute__((always_inline)) inline static bool cat(
  ...
  while (true) {
    do {
      ...
      if (bpin > eob) {
        ...
        if (n_to_read != 0) input_pending = true;
#endif

        if (!input_pending) write_pending(outbuf, &bpout);
...
int main(int argc, char **argv) {
  ...
  do {
    if (argind < argc) infile = argv[argind];

    if (STREQ(infile, "-")) {
      have_read_stdin = true;
      input_desc = STDIN_FILENO;
      if (file_open_mode & O_BINARY) xset_binary_mode(STDIN_FILENO, O_BINARY);
    } else {
