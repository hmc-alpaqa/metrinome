...
__attribute__((always_inline)) inline static int hextobin(unsigned char c) {
  switch (c) {
    default:
      return c - '0';
    case 'a':
...
int main(int argc, char **argv) {
  ...
  bool allow_options = (!posixly_correct || (!DEFAULT_ECHO_TO_XPG && 1 < argc &&
                                             STREQ(argv[1], "-n")));
  ...
  if (allow_options)
    while (argc > 0 && *argv[0] == '-') {
      ...
      for (i = 0; temp[i]; i++) switch (temp[i]) {
          case 'e':
          case 'E':
          case 'n':
            break;
      ...
      if (i == 0) goto just_echo;

      /* All of the options in TEMP are valid options to ECHO.
         Handle them. */
      while (*temp) switch (*temp++) {
          case 'e':
            do_v9 = true;
            break;
      ...
      while ((c = *s++)) {
        if (c == '\\' && *s) {
          switch (c = *s++) {
            ...
            case 'x': {
              unsigned char ch = *s;
              if (!isxdigit(ch)) goto not_an_escape;
              s++;
              c = hextobin(ch);
              ch = *s;
              if (isxdigit(ch)) {
