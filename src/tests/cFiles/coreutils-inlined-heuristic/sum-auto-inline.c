...
#define AUTHORS proper_name("Kayvan Aghaiepour"), proper_name("David MacKenzie")
...
__attribute__((always_inline)) inline static bool bsd_sum_file(const char *file, int print_name) {
  FILE *fp;
...
__attribute__((always_inline)) inline static bool sysv_sum_file(const char *file, int print_name) {
  int fd;
  ...
  while (1) {
    ...
    for (size_t i = 0; i < bytes_read; i++) s += buf[i];
