...
__attribute__((always_inline))    NO_ASM -- Disable inline asm
...
__attribute__((always_inline))    chips they're merely libgcc calls, where we will instead want an inline
...
#define sub_ddmmss(sh, sl, ah, al, bh, bl)                                    \
  do {                                                                        \
    if (__builtin_constant_p(ah) && __builtin_constant_p(bh) && (ah) == (bh)) \
      __asm__("subs\t%1, %2, %3\n\tsbc\t%0, %0, %0"                           \
              : "=r"(sh), "=r"(sl)                                            \
...
__attribute__((always_inline)) inline extern UWtype __MPN(udiv_qrnnd)(UWtype *, UWtype, UWtype, UWtype);
#endif /* LONGLONG_STANDALONE */
#else  /* ARMv4 or newer */
#define umul_ppmm(xh, xl, a, b) \
...
#define umul_ppmm(xh, xl, m0, m1)                                              \
  do {                                                                         \
    /* When we have 64-bit regs and gcc is aware of that, we cannot simply use \
       ...
       64-bit register, whereas mlr uses the low 32-bits of an even-odd                     \
       register pair.                                                          \
    */                                                                         \
...
#define udiv_qrnnd(q, r, n1, n0, dx) /* d renamed to dx avoiding "=d" */ \
  __asm__("divq %4"                  /* stringification in K&R C */      \
    ...
    HAVE_HOST_CPU_skylake || HAVE_HOST_CPU_k10 || HAVE_HOST_CPU_bd1 || \
    HAVE_HOST_CPU_bd2 || HAVE_HOST_CPU_bd3 || HAVE_HOST_CPU_bd4 ||     \
    HAVE_HOST_CPU_zen || HAVE_HOST_CPU_bobcat || HAVE_HOST_CPU_jaguar
#define count_leading_zeros(count, x)                                   \
  do {                                                                  \
...
#define umul_ppmm(xh, xl, a, b)                                       \
  do {                                                                \
    ...
    __asm__(                                                          \
        ...
        "	add%.l	%3,%1\n"                                             \
        "	addx%.l	%2,%0\n"                                            \
__attribute__((always_inline))         "	| End inlined umul_ppmm"                                    \
...
__attribute__((always_inline))    the system vendor compilers.  (Is that vendor compilers with inline asm,
   or what?)  */
  ...
  do {                                                            \
    if (__builtin_constant_p(ah) && (ah) == 0)                    \
      ...
      __asm__("subf%I4c %1,%5,%4\n\tsubfe %0,%3,%2"               \
              : "=r"(sh), "=&r"(sl)                               \
              : "r"(ah), "r"(bh), "rI"(al), "r"(bl)__CLOBBER_CC); \
  } while (0)
#define count_leading_zeros(count, x) \
...
#define udiv_qrnnd(q, r, n1, n0, d)                           \
  __asm__(                                                    \
      ...
      "	bl,a 1f\n"                                            \
      "	add	%1,%4,%1\n"                                       \
__attribute__((always_inline))       "1:	! End of inline udiv_qrnnd"                         \
      : "=r"(q), "=r"(r)                                      \
      : "r"(n1), "r"(n0), "rI"(d)                             \
      : "%g1" __AND_CLOBBER_CC)
#define count_leading_zeros(count, x) \
...
__attribute__((always_inline)) inline extern __longlong_h_C UWtype mpn_umul_ppmm(UWtype *, UWtype, UWtype);
...
__attribute__((always_inline)) inline extern __longlong_h_C UWtype mpn_umul_ppmm_r(UWtype, UWtype, UWtype *);
...
__attribute__((always_inline)) inline extern __longlong_h_C UWtype mpn_udiv_qrnnd(UWtype *, UWtype, UWtype, UWtype);
...
__attribute__((always_inline)) inline extern __longlong_h_C UWtype mpn_udiv_qrnnd_r(UWtype, UWtype, UWtype, UWtype *);
...
__attribute__((always_inline)) /* If this machine has no inline assembler, use C macros.  */
