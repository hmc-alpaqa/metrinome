...
__attribute__((always_inline)) inline extern bool print_group_list(const char *username, uid_t ruid, gid_t rgid,
  ...
  if (username) {
    pwd = getpwuid(ruid);
    if (pwd == NULL) ok = false;
  }

  if (!print_group(rgid, use_names)) ok = false;

  if (egid != rgid) {
    putchar(delim);
    if (!print_group(egid, use_names)) ok = false;
  }

  {
    ...
    for (int i = 0; i < n_groups; i++)
      if (groups[i] != rgid && groups[i] != egid) {
        putchar(delim);
        if (!print_group(groups[i], use_names)) ok = false;
      }
    free(groups);
  }
  return ok;
...
__attribute__((always_inline)) inline static char *gidtostr_ptr(gid_t const *gid) {
...
__attribute__((always_inline)) inline extern bool print_group(gid_t gid, bool use_name) {
