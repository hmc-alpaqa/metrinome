 ...
 */
__attribute__((always_inline)) inline static bool periodic_pattern(int type) {
 ...
 */
__attribute__((always_inline)) inline static void fillpattern(int type, unsigned char *r, size_t size) {
  size_t i;
  unsigned int bits = type & 0xfff;
  ...
  r[1] = (bits >> 8) & 255;
  r[2] = bits & 255;
  for (i = 3; i <= size / 2; i *= 2) memcpy(r + i, r, i);
  if (i < size) memcpy(r + i, r, size - i);

  /* Invert the first bit of every sector. */
  if (type & 0x1000)
    for (i = 0; i < size; i += SECTOR_SIZE) r[i] ^= 0x80;
}
...
__attribute__((always_inline)) inline static void passname(unsigned char const *data, char name[PASS_NAME_SIZE]) {
  if (data)
...
__attribute__((always_inline)) inline static bool ignorable_sync_errno(int errno_val) {
...
__attribute__((always_inline)) inline static int dosync(int fd, char const *qname) {
  int err;
...
__attribute__((always_inline)) inline static void direct_mode(int fd, bool enable) {
  if (O_DIRECT) {
    int fd_flags = fcntl(fd, F_GETFL);
    if (0 < fd_flags) {
      int new_flags = (enable ? (fd_flags | O_DIRECT) : (fd_flags & ~O_DIRECT));
      if (new_flags != fd_flags) fcntl(fd, F_SETFL, new_flags);
    }
  }
...
__attribute__((always_inline)) inline static bool dorewind(int fd, struct stat const *st) {
  if (S_ISCHR(st->st_mode)) {
    ...
    op.mt_op = MTREW;
    op.mt_count = 1;
    if (ioctl(fd, MTIOCTOP, &op) == 0) return true;
#endif
  }
  off_t offset = lseek(fd, 0, SEEK_SET);
  if (0 < offset) errno = EINVAL;
  return offset == 0;
}
 ...
 */
__attribute__((always_inline)) inline static int dopass(int fd, struct stat const *st, char const *qname,
...
#define PATTERNBUF_SIZE (PAGE_ALIGN_SLOP + FILLPATTERN_SIZE)
  ...
  if (!try_without_directio) direct_mode(fd, true);
    ...
    for (soff = 0; soff < lim; soff += ssize) {
      ...
      if (0 < ssize)
        ...
        if (!known(size) && (ssize == 0 || errno == ENOSPC)) {
          ...
          if (soff <= OFF_T_MAX - offset) *sizep = size = offset + soff;
          break;
        } else {
          int errnum = errno;
    ...
    if (n &&
      ...
      if (done || !STREQ(previous_human_offset, human_offset)) {
        if (!known(size))
          ...
          if (done) human_offset = human_size;
 ...
 * order.
 */
__attribute__((always_inline)) inline static void genpattern(int *dest, size_t num, struct randint_source *s) {
  size_t randpasses;
  int const *p;
  int *d;
  size_t n;
 ...
 */
__attribute__((always_inline)) inline static bool do_wipefd(int fd, char const *qname, struct randint_source *s,
  ...
  if (flags->verbose) n = flags->n_iterations + flags->zero_fill;
  ...
  if (size == -1) {
    if (S_ISREG(st.st_mode)) {
      ...
      if (!flags->exact) {
        ...
        if (size && size < ST_BLKSIZE(st)) i_size = size;
        if (remainder != 0) {
...
__attribute__((always_inline)) inline static bool wipefd(int fd, char const *qname, struct randint_source *s,
                   struct Options const *flags) {
  int fd_flags = fcntl(fd, F_GETFL);
...
__attribute__((always_inline)) inline static bool incname(char *name, size_t len) {
  while (len--) {
 ...
 */
__attribute__((always_inline)) inline static bool wipename(char *oldname, char const *qoldname,
                     struct Options const *flags) {
  ...
  if (flags->verbose) error(0, 0, _("%s: removing"), qoldname);
  ...
  if (flags->remove_file != remove_unlink)
    for (size_t len = base_len(base); len != 0; len--) {
      ...
      while (!(rename_ok = (renameatu(AT_FDCWD, oldname, AT_FDCWD, newname,
        ...
        continue;
      if (rename_ok) {
        if (0 <= dir_fd && dosync(dir_fd, qdir) != 0) ok = false;
        if (flags->verbose) {
  ...
  if (0 <= dir_fd) {
    if (dosync(dir_fd, qdir) != 0) ok = false;
    if (close(dir_fd) != 0) {
 ...
 */
__attribute__((always_inline)) inline static bool wipefile(char *name, char const *qname, struct randint_source *s,
                     struct Options const *flags) {
  bool ok;
  int fd;
  ...
  if (ok && flags->remove_file) ok = wipename(name, qname, flags);
  return ok;
}
...
/* Just on general principles, wipe buffers containing information
   ...
   shredding.  */
__attribute__((always_inline)) inline static void clear_random_data(void) { randint_all_free(randint_source); }
...
int main(int argc, char **argv) {
  ...
  if (!randint_source) die(EXIT_FAILURE, errno, "%s", quotef(random_source));
