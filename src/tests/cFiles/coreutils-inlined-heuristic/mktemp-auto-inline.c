...
__attribute__((always_inline)) inline static size_t count_consecutive_X_s(const char *s, size_t len) {
  size_t n = 0;
  for (; len && s[len - 1] == 'X'; len--) ++n;
  return n;
}

__attribute__((always_inline)) inline static int mkstemp_len(char *tmpl, size_t suff_len, size_t x_len,
                       bool dry_run) {
  return gen_tempname_len(tmpl, suff_len, 0, dry_run ? GT_NOCREATE : GT_FILE,
                          x_len);
}

__attribute__((always_inline)) inline static int mkdtemp_len(char *tmpl, size_t suff_len, size_t x_len,
                       bool dry_run) {
