...
__attribute__((always_inline)) inline static int _GL_ATTRIBUTE_PURE iswnbspace(wint_t wc) {
  return !posixly_correct &&
         (wc == 0x00A0 || wc == 0x2007 || wc == 0x202F || wc == 0x2060);
}

__attribute__((always_inline)) inline static int isnbspace(int c) { return iswnbspace(btowc(c)); }

/* FILE is the name of the file (or NULL for standard input)
   associated with the specified counters.  */
__attribute__((always_inline)) inline static void write_counts(uintmax_t lines, uintmax_t words, uintmax_t chars,
...
__attribute__((always_inline)) inline static bool wc(int fd, char const *file_x, struct fstatus *fstatus,
               off_t current_pos) {
  bool ok = true;
  ...
  if (count_bytes && !count_chars && !print_lines && !count_complicated) {
    ...
    if (0 < fstatus->failed) fstatus->failed = fstat(fd, &fstatus->st);
    ...
    if (!fstatus->failed && usable_st_size(&fstatus->st) &&
      ...
      if (current_pos < 0) current_pos = lseek(fd, 0, SEEK_CUR);
...
#define SUPPORT_OLD_MBRTOWC 1
    ...
    while ((bytes_read = safe_read(fd, buf + prev, BUFFER_SIZE - prev)) > 0) {
      ...
      do {
        ...
        if (!in_shift && is_basic(*p)) {
          ...
          if (mbsinit(&state)) in_shift = false;
          if (n == 0) {
            wide_char = 0;
            n = 1;
          }
        ...
        switch (wide_char) {
          ...
          default:
            if (wide && iswprint(wide_char)) {
              ...
              linepos++;
              if (isspace(to_uchar(*p))) goto mb_word_separator;
              in_word = true;
            }
            break;
        }
        ...
        switch (*p++) {
          ...
          default:
            if (isprint(to_uchar(p[-1]))) {
              linepos++;
              if (isspace(to_uchar(p[-1])) || isnbspace(to_uchar(p[-1])))
                goto word_separator;
              in_word = true;
            }
            break;
  ...
  if (count_chars < print_chars) chars = bytes;
  ...
  total_chars += chars;
  total_bytes += bytes;
  if (linelength > max_line_length) max_line_length = linelength;

  return ok;
}

__attribute__((always_inline)) inline static bool wc_file(char const *file, struct fstatus *fstatus) {
  if (!file || STREQ(file, "-")) {
...
__attribute__((always_inline)) inline static struct fstatus *get_input_fstatus(size_t nfiles, char *const *file) {
...
argv_iter_done:
  ...
  if (read_tokens) readtokens0_free(&tok);
