...
__attribute__((always_inline)) inline static int cache_fstatat(int fd, char const *file, struct stat *st, int flag) {
...
__attribute__((always_inline)) static inline struct stat *cache_stat_init(struct stat *st) {
  st->st_size = -1;
  return st;
}
...
__attribute__((always_inline)) inline static int write_protected_non_symlink(int fd_cwd, char const *file,
                                       struct stat *buf) {
  if (can_write_any_file()) return 0;
  if (cache_fstatat(fd_cwd, file, buf, AT_SYMLINK_NOFOLLOW) != 0) return -1;
...
__attribute__((always_inline)) inline static enum RM_status prompt(FTS const *fts, FTSENT const *ent, bool is_dir,
  ...
  if (is_empty_p) *is_empty_p = T_UNKNOWN;
  ...
  if (ent->fts_number) return RM_USER_DECLINED;

  if (x->interactive == RMI_NEVER) return RM_OK;
  ...
  if (write_protected || x->interactive == RMI_ALWAYS) {
    ...
    if (0 <= write_protected) switch (dirent_type) {
        case DT_LNK:
          /* Using permissions doesn't make sense for symlinks.  */
          if (x->interactive != RMI_ALWAYS) return RM_OK;
          break;
    ...
    if (!yesno()) return RM_USER_DECLINED;
  }
  return RM_OK;
}
...
__attribute__((always_inline)) static inline bool nonexistent_file_errno(int errnum) {
...
__attribute__((always_inline)) static inline bool ignorable_missing(struct rm_options const *x, int errnum) {
  return x->ignore_missing_files && nonexistent_file_errno(errnum);
}

/* Tell fts not to traverse into the hierarchy at ENT.  */
__attribute__((always_inline)) inline static void fts_skip_tree(FTS *fts, FTSENT *ent) {
...
/* Upon unlink failure, or when the user declines to remove ENT, mark
   ...
   its removal.  */
__attribute__((always_inline)) inline static void mark_ancestor_dirs(FTSENT *ent) {
  FTSENT *p;
  for (p = ent->fts_parent; FTS_ROOTLEVEL <= p->fts_level; p = p->fts_parent) {
    if (p->fts_number) break;
    p->fts_number = 1;
  }
}
...
__attribute__((always_inline)) inline static enum RM_status excise(FTS *fts, FTSENT *ent, struct rm_options const *x,
                             bool is_dir) {
  ...
  if (ignorable_missing(x, errno)) return RM_OK;
...
__attribute__((always_inline)) inline static enum RM_status rm_fts(FTS *fts, FTSENT *ent,
                             struct rm_options const *x) {
  switch (ent->fts_info) {
      ...
      if (ent->fts_level == FTS_ROOTLEVEL) {
        ...
        if (x->preserve_all_root) {
          ...
          if (!parent || lstat(parent, &statbuf)) {
            error(0, 0, _("failed to stat %s: skipping %s"),
                  quoteaf_n(0, parent), quoteaf_n(1, ent->fts_accpath));
            failed = true;
          }
    ...
    default:
      error(0, 0,
            _("unexpected failure: fts_info=%d: %s\n"
              "please report to %s"),
...
__attribute__((always_inline)) inline enum RM_status rm(char *const *file, struct rm_options const *x) {
  ...
  if (*file) {
    ...
    if (x->one_file_system) bit_flags |= FTS_XDEV;
