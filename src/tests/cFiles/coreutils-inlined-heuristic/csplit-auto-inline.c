...
__attribute__((always_inline)) inline extern void xalloc_die(void) {
...
__attribute__((always_inline)) inline static void save_to_hold_area(char *start, size_t num) {
  free(hold_area);
  hold_area = start;
  hold_count = num;
}
...
__attribute__((always_inline)) inline static size_t read_input(char *dest, size_t max_n_bytes) {
  size_t bytes_read;

  if (max_n_bytes == 0) return 0;

  bytes_read = safe_read(STDIN_FILENO, dest, max_n_bytes);

  if (bytes_read == 0) have_read_eof = true;
...
__attribute__((always_inline)) inline static void clear_line_control(struct line *p) {
  p->used = 0;
  p->insert_index = 0;
  p->retrieve_index = 0;
}
...
__attribute__((always_inline)) inline static struct line *new_line_control(void) {
...
__attribute__((always_inline)) inline static void keep_new_line(struct buffer_record *b, char *line_start,
                          size_t line_len) {
  struct line *l;

  /* If there is no existing area to keep line info, get some. */
  if (b->line_start == NULL) b->line_start = b->curr_line = new_line_control();
...
__attribute__((always_inline)) inline static size_t record_line_starts(struct buffer_record *b) {
...
__attribute__((always_inline)) inline static struct buffer_record *create_new_buffer(size_t size) {
...
__attribute__((always_inline)) inline static struct buffer_record *get_new_buffer(size_t min_size) {
...
__attribute__((always_inline)) inline static void free_buffer(struct buffer_record *buf) {
  struct line *l;
...
__attribute__((always_inline)) inline static void save_buffer(struct buffer_record *buf) {
  ...
  if (head == NULL)
    ...
    for (p = head; p->next; p = p->next) /* Do nothing. */;
    p->next = buf;
  }
}
...
__attribute__((always_inline)) inline static bool load_buffer(void) {
  ...
  while (1) {
    ...
    if (lines_found || have_read_eof) break;

    if (xalloc_oversized(2, b->bytes_alloc)) xalloc_die();
...
__attribute__((always_inline)) inline static uintmax_t get_first_line_in_buffer(void) {
...
__attribute__((always_inline)) inline static struct cstring *remove_line(void) {
  ...
  if (head == NULL && !load_buffer()) return NULL;
...
__attribute__((always_inline)) inline static struct cstring *find_line(uintmax_t linenum) {
  struct buffer_record *b;

  if (head == NULL && !load_buffer()) return NULL;

  if (linenum < head->start_line) return NULL;
  ...
  for (b = head;;) {
    ...
    if (b->next == NULL && !load_buffer()) return NULL;
...
__attribute__((always_inline)) inline static void set_input_file(const char *name) {
  if (!STREQ(name, "-") && fd_reopen(STDIN_FILENO, name, O_RDONLY, 0) < 0)
    die(EXIT_FAILURE, errno, _("cannot open %s for reading"), quoteaf(name));
}
...
__attribute__((always_inline)) inline static void write_to_file(uintmax_t last_line, bool ignore, int argnum) {
  struct cstring *line;
  ...
  for (i = 0; i < lines; i++) {
    ...
    if (!ignore) save_line_to_file(line);
  }
}

/* Output any lines left after all regexps have been processed. */

__attribute__((always_inline)) inline static void dump_rest_of_file(void) {
  struct cstring *line;

  while ((line = remove_line()) != NULL) save_line_to_file(line);
}

/* Handle an attempt to read beyond EOF under the control of record P,
   on iteration REPETITION if nonzero. */

__attribute__((always_inline)) inline static void handle_line_error(const struct control *,
                              uintmax_t) ATTRIBUTE_NORETURN;
__attribute__((always_inline)) inline static void handle_line_error(const struct control *p, uintmax_t repetition) {
...
__attribute__((always_inline)) inline static void process_line_count(const struct control *p, uintmax_t repetition) {
  uintmax_t linenum;
  ...
  while (linenum++ < last_line_to_save) {
    ...
    if (line == NULL) handle_line_error(p, repetition);
    save_line_to_file(line);
  }

  close_output_file();

  if (suppress_matched) remove_line();
...
__attribute__((always_inline)) inline static void process_regexp(struct control *p, uintmax_t repetition) {
  ...
  if (!ignore) create_output_file();

  if (suppress_matched && current_line > 0) remove_line();
  ...
  if (!ignore) close_output_file();

  if (p->offset > 0) current_line = break_line;
}

/* Split the input file according to the control records we have built. */

__attribute__((always_inline)) inline static void split_file(void) {
...
__attribute__((always_inline)) inline static char *make_filename(unsigned int num) {
...
__attribute__((always_inline)) inline static void delete_all_files(bool in_signal_handler) {
  if (!remove_files) return;
...
__attribute__((always_inline)) inline static void close_output_file(void) {
  if (output_stream) {
    ...
    if (bytes_written == 0 && elide_empty_files) {
      ...
      if (!unlink_ok) error(0, unlink_errno, "%s", quotef(output_filename));
    } else {
      if (!suppress_count) {
        char buf[INT_BUFSIZE_BOUND(uintmax_t)];
...
__attribute__((always_inline)) inline static struct control *new_control_record(void) {
...
__attribute__((always_inline)) inline static void check_for_offset(struct control *p, const char *str,
                             const char *num) {
  if (xstrtoimax(num, NULL, 10, &p->offset, "") != LONGINT_OK)
    die(EXIT_FAILURE, 0, _("%s: integer expected after delimiter"), quote(str));
}
...
__attribute__((always_inline)) inline static void parse_repeat_count(int argnum, struct control *p, char *str) {
  uintmax_t val;
  char *end;

  end = str + strlen(str) - 1;
  if (*end != '}')
    die(EXIT_FAILURE, 0, _("%s: '}' is required in repeat count"), quote(str));
  *end = '\0';
  ...
  if (str + 1 == end - 1 && *(str + 1) == '*')
    ...
    if (xstrtoumax(str + 1, NULL, 10, &val, "") != LONGINT_OK) {
      die(EXIT_FAILURE, 0, _("%s}: integer required between '{' and '}'"),
          quote(global_argv[argnum]));
    }
    p->repeat = val;
  }
...
__attribute__((always_inline)) inline static struct control *extract_regexp(int argnum, bool ignore,
                                      char const *str) {
  ...
  if (closing_delim == NULL)
    die(EXIT_FAILURE, 0, _("%s: closing delimiter '%c' missing"), str, delim);
  ...
  if (closing_delim[1]) check_for_offset(p, str, closing_delim + 1);
...
__attribute__((always_inline)) inline static void parse_patterns(int argc, int start, char **argv) {
  ...
  for (int i = start; i < argc; i++) {
    if (*argv[i] == '/' || *argv[i] == '%') {
      ...
      if (val == 0)
        die(EXIT_FAILURE, 0, _("%s: line number must be greater than zero"),
            argv[i]);
      if (val < last_val) {
        char buf[INT_BUFSIZE_BOUND(uintmax_t)];
        die(EXIT_FAILURE, 0,
...
__attribute__((always_inline)) inline static size_t get_format_flags(char const *format, int *flags_ptr) {
  int flags = 0;
...
__attribute__((always_inline)) inline static void check_format_conv_type(char *format, int flags) {
  ...
  switch (ch) {
    ...
    default:
      if (isprint(ch))
        die(EXIT_FAILURE, 0, _("invalid conversion specifier in suffix: %c"),
            ch);
      else
        die(EXIT_FAILURE, 0,
  ...
  if (flags & ~compatible_flags)
    die(EXIT_FAILURE, 0, _("invalid flags in conversion specification: %%%c%c"),
...
__attribute__((always_inline)) inline static size_t max_out(char *format) {
  bool percent = false;
  ...
  for (char *f = format; *f; f++)
    if (*f == '%' && *++f != '%') {
      ...
      while (ISDIGIT(*f)) f++;
      if (*f == '.')
        while (ISDIGIT(*++f)) continue;
      check_format_conv_type(f, flags);
    }

  if (!percent)
    die(EXIT_FAILURE, 0, _("missing %% conversion specification in suffix"));

  int maxlen = snprintf(NULL, 0, format, UINT_MAX);
  if (!(0 <= maxlen && maxlen <= SIZE_MAX)) xalloc_die();
  return maxlen;
}
...
int main(int argc, char **argv) {
  ...
  if (SIZE_MAX - 1 - prefix_len < max_digit_string_len) xalloc_die();
