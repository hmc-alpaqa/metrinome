...
__attribute__((always_inline)) inline static bool do_ftruncate(int fd, char const *fname, off_t ssize, off_t rsize,
                         rel_mode_t rel_mode) {
  struct stat sb;
  off_t nsize;
  ...
  if (block_mode) {
    ...
    if (INT_MULTIPLY_WRAPV(ssize, blksize, &ssize)) {
      error(
          0, 0,
          _("overflow in %" PRIdMAX " * %" PRIdPTR " byte blocks for file %s"),
