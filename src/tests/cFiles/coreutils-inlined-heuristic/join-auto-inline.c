...
__attribute__((always_inline)) inline static void extract_field(struct line *line, char *field, size_t len) {
...
__attribute__((always_inline)) inline static void xfields(struct line *line) {
  ...
  if (0 <= tab && tab != '\n') {
    ...
    do {
      char *sep;
      for (sep = ptr + 1; sep != lim && !field_sep(*sep); sep++) continue;
      extract_field(line, ptr, sep - ptr);
      if (sep == lim) return;
      for (ptr = sep + 1; ptr != lim && field_sep(*ptr); ptr++) continue;
    } while (ptr != lim);
  }
...
__attribute__((always_inline)) inline static void freeline(struct line *line) {
  if (line == NULL) return;
  free(line->fields);
  line->fields = NULL;
...
__attribute__((always_inline)) inline static int keycmp(struct line const *line1, struct line const *line2,
  ...
  if (ignore_case) {
    ...
    if (hard_LC_COLLATE) return xmemcoll(beg1, len1, beg2, len2);
...
__attribute__((always_inline)) inline static void check_order(const struct line *prev, const struct line *current,
                        int whatfile) {
...
__attribute__((always_inline)) static inline void reset_line(struct line *line) { line->nfields = 0; }

__attribute__((always_inline)) inline static struct line *init_linep(struct line **linep) {
...
__attribute__((always_inline)) inline static bool get_line(FILE *fp, struct line **linep, int which) {
  ...
  if (!readlinebuffer_delim(&line->buf, fp, eolchar)) {
    if (ferror(fp)) die(EXIT_FAILURE, errno, _("read error"));
    freeline(line);
    return false;
  }
  ++line_no[which - 1];
  ...
  if (prevline[which - 1]) check_order(prevline[which - 1], line, which);

  prevline[which - 1] = line;
  return true;
}

__attribute__((always_inline)) inline static void free_spareline(void) {
...
__attribute__((always_inline)) inline static void initseq(struct seq *seq) {
  seq->count = 0;
  seq->alloc = 0;
  seq->lines = NULL;
}
...
__attribute__((always_inline)) inline static bool getseq(FILE *fp, struct seq *seq, int whichfile) {
  if (seq->count == seq->alloc) {
    seq->lines = X2NREALLOC(seq->lines, &seq->alloc);
    for (size_t i = seq->count; i < seq->alloc; i++) seq->lines[i] = NULL;
  }
...
__attribute__((always_inline)) inline static bool advance_seq(FILE *fp, struct seq *seq, bool first, int whichfile) {
  if (first) seq->count = 0;

  return getseq(fp, seq, whichfile);
}

__attribute__((always_inline)) inline static void delseq(struct seq *seq) {
...
__attribute__((always_inline)) inline static void prfield(size_t n, struct line const *line) {
  size_t len;
...
__attribute__((always_inline)) inline static void prfields(struct line const *line, size_t join_field,
                     size_t autocount) {
  size_t i;
...
__attribute__((always_inline)) inline static void prjoin(struct line const *line1, struct line const *line2) {
...
__attribute__((always_inline)) inline static void join(FILE *fp1, FILE *fp2) {
  ...
  if (join_header_lines && (seq1.count || seq2.count)) {
    ...
    prevline[0] = NULL;
    prevline[1] = NULL;
    if (seq1.count) advance_seq(fp1, &seq1, true, 1);
    if (seq2.count) advance_seq(fp2, &seq2, true, 2);
  }

  while (seq1.count && seq2.count) {
    diff = keycmp(seq1.lines[0], seq2.lines[0], join_field_1, join_field_2);
    if (diff < 0) {
      if (print_unpairables_1) prjoin(seq1.lines[0], &uni_blank);
  ...
  if ((print_unpairables_1 || checktail) && seq1.count) {
    if (print_unpairables_1) prjoin(seq1.lines[0], &uni_blank);
    if (seq2.count) seen_unpairable = true;
    while (get_line(fp1, &line, 1)) {
      if (print_unpairables_1) prjoin(line, &uni_blank);
  ...
  if ((print_unpairables_2 || checktail) && seq2.count) {
    if (print_unpairables_2) prjoin(&uni_blank, seq2.lines[0]);
    if (seq1.count) seen_unpairable = true;
    while (get_line(fp2, &line, 2)) {
      if (print_unpairables_2) prjoin(&uni_blank, line);
      if (issued_disorder_warning[1] && !print_unpairables_2) break;
    }
  }
...
__attribute__((always_inline)) inline static void add_field(int file, size_t field) {
  struct outlist *o;
...
__attribute__((always_inline)) inline static size_t string_to_join_field(char const *str) {
  size_t result;
  uintmax_t val;
...
__attribute__((always_inline)) inline static void decode_field_spec(const char *s, int *file_index,
                              size_t *field_index) {
  ...
  switch (s[0]) {
    ...
    default:
      die(EXIT_FAILURE, 0, _("invalid file number in field spec: %s"),
          quote(s));
...
__attribute__((always_inline)) inline static void add_field_list(char *str) {
  char *p = str;
...
__attribute__((always_inline)) inline static void set_join_field(size_t *var, size_t val) {
  if (*var != SIZE_MAX && *var != val) {
    unsigned long int var1 = *var + 1;
    unsigned long int val1 = val + 1;
    die(EXIT_FAILURE, 0, _("incompatible join fields %lu, %lu"), var1, val1);
  }
  *var = val;
}
...
__attribute__((always_inline)) inline static void add_file_name(char *name, char *names[2], int operand_status[2],
  ...
  names[n] = name;
  *nfiles = n + 1;
  if (*prev_optc_status == MIGHT_BE_O_ARG) *optc_status = MIGHT_BE_O_ARG;
}
...
int main(int argc, char **argv) {
    ...
    switch (optc) {
      ...
      case 'a': {
        ...
        if (xstrtoul(optarg, NULL, 10, &val, "") != LONGINT_OK ||
            (val != 1 && val != 2))
          die(EXIT_FAILURE, 0, _("invalid field number: %s"), quote(optarg));
        if (val == 1)
      ...
      case 't': {
        ...
        if (!newtab)
          ...
          if (STREQ(optarg, "\\0"))
            ...
            die(EXIT_FAILURE, 0, _("multi-character tab %s"), quote(optarg));
        }
        if (0 <= tab && tab != newtab)
  ...
  if (join_field_1 == SIZE_MAX) join_field_1 = 0;
  if (join_field_2 == SIZE_MAX) join_field_2 = 0;

  fp1 = STREQ(g_names[0], "-") ? stdin : fopen(g_names[0], "r");
  if (!fp1) die(EXIT_FAILURE, errno, "%s", quotef(g_names[0]));
  fp2 = STREQ(g_names[1], "-") ? stdin : fopen(g_names[1], "r");
  if (!fp2) die(EXIT_FAILURE, errno, "%s", quotef(g_names[1]));
  if (fp1 == fp2)
    die(EXIT_FAILURE, errno, _("both files cannot be standard input"));
  join(fp1, fp2);

  if (fclose(fp1) != 0) die(EXIT_FAILURE, errno, "%s", quotef(g_names[0]));
  if (fclose(fp2) != 0) die(EXIT_FAILURE, errno, "%s", quotef(g_names[1]));
