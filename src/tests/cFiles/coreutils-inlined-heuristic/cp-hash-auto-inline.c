...
__attribute__((always_inline)) inline static size_t src_to_dest_hash(void const *x, size_t table_size) {
...
__attribute__((always_inline)) inline static bool src_to_dest_compare(void const *x, void const *y) {
  struct Src_to_dest const *a = x;
  struct Src_to_dest const *b = y;
  return SAME_INODE(*a, *b) ? true : false;
}

__attribute__((always_inline)) inline static void src_to_dest_free(void *x) {
...
__attribute__((always_inline)) inline extern void forget_created(ino_t ino, dev_t dev) {
...
__attribute__((always_inline)) inline extern char *src_to_dest_lookup(ino_t ino, dev_t dev) {
  struct Src_to_dest ent;
...
__attribute__((always_inline)) inline extern char *remember_copied(const char *name, ino_t ino, dev_t dev) {
  struct Src_to_dest *ent;
...
__attribute__((always_inline)) inline extern void hash_init(void) {
...
__attribute__((always_inline)) inline extern void forget_all(void) { hash_free(src_to_dest); }
