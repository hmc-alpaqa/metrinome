...
__attribute__((always_inline)) inline static bool re_protect(char const *const_dst_name, size_t src_offset,
...
__attribute__((always_inline)) inline static bool make_dir_parents_private(char const *const_dir, size_t src_offset,
    ...
    while ((slash = strchr(slash, '/'))) {
      ...
      *slash = '\0';
      ...
      if (!*new_dst &&
        ...
        if (!set_file_security_ctx(dir, x->preserve_security_context, false,
                                   x) &&
...
__attribute__((always_inline)) inline static bool target_directory_operand(char const *file, struct stat *st,
...
__attribute__((always_inline)) inline static bool do_copy(int n_files, char **file, const char *target_directory,
                    bool no_target_directory, struct cp_options *x) {
  struct stat sb;
  bool new_dst = false;
  bool ok = true;
  bool forcing =
      x->unlink_dest_before_opening || x->unlink_dest_after_failed_open;
  ...
  if (no_target_directory) {
    ...
    ignore_value(
        target_directory_operand(file[n_files - 1], &sb, &new_dst, forcing));
  ...
  if (target_directory) {
    ...
    for (int i = 0; i < n_files; i++) {
      ...
      if (remove_trailing_slashes) strip_trailing_slashes(arg);
...
__attribute__((always_inline)) inline static void decode_preserve_arg(char const *arg, struct cp_options *x,
                                bool on_off) {
  enum File_attribute {
    PRESERVE_MODE,
    PRESERVE_TIMESTAMPS,
  ...
  do {
    ...
    switch (val) {
      ...
      case PRESERVE_ALL:
        ...
        x->preserve_links = on_off;
        x->explicit_no_preserve_mode = !on_off;
        if (selinux_enabled) x->preserve_security_context = on_off;
        x->preserve_xattr = on_off;
        break;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "abdfHilLnprst:uvxPRS:TZ", long_opts,
    ...
    switch (c) {
      ...
      case 'a':
        ...
        if (selinux_enabled) x.preserve_security_context = true;
  ...
  if (x.dereference == DEREF_UNDEFINED) {
    if (x.recursive && !x.hard_link) /* This is compatible with FreeBSD.  */
      x.dereference = DEREF_NEVER;
    else
      x.dereference = DEREF_ALWAYS;
  }

  if (x.recursive) x.copy_as_regular = copy_contents;
  ...
  if (x.require_preserve_xattr)
    die(EXIT_FAILURE, 0,
        _("cannot preserve extended attributes, cp is "
