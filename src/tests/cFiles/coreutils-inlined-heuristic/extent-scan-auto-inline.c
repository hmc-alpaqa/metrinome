...
__attribute__((always_inline)) inline static bool extent_need_sync(void) {
...
__attribute__((always_inline)) inline extern void extent_scan_init(int src_fd, struct extent_scan *scan) {
  scan->fd = src_fd;
  scan->ei_count = 0;
  scan->ext_info = NULL;
  scan->scan_start = 0;
...
__attribute__((always_inline)) inline extern bool extent_scan_read(struct extent_scan *scan) {
  unsigned int si = 0;
  ...
  while (true) {
    ...
    if (ioctl(scan->fd, FS_IOC_FIEMAP, fiemap) < 0) {
      if (scan->scan_start == 0) scan->initial_scan_failed = true;
      return false;
    }
    ...
    if (last_ei->ext_flags & FIEMAP_EXTENT_LAST) scan->hit_final_extent = true;
...
#else
__attribute__((always_inline)) inline extern bool extent_scan_read(struct extent_scan *scan _GL_UNUSED) {
