...
__attribute__((always_inline)) inline static int _GL_ATTRIBUTE_PURE path_common_prefix(const char *path1,
                                                 const char *path2) {
  int i = 0;
  int ret = 0;
...
__attribute__((always_inline)) inline static bool buffer_or_output(const char *str, char **pbuf, size_t *plen) {
  if (*pbuf) {
...
__attribute__((always_inline)) inline bool relpath(const char *can_fname, const char *can_reldir, char *buf,
  ...
  if (*relto_suffix) {
    ...
    for (; *relto_suffix; ++relto_suffix) {
      if (*relto_suffix == '/') buf_err |= buffer_or_output("/..", &buf, &len);
    }
  ...
  if (buf_err) error(0, ENAMETOOLONG, "%s", _("generating relative path"));
