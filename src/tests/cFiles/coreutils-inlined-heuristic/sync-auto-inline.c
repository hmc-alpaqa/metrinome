...
__attribute__((always_inline)) inline static bool sync_arg(enum sync_mode mode, char const *file) {
  bool ret = true;
  ...
  if (mode == MODE_FILE) open_flags = O_WRONLY | O_NONBLOCK;
#endif
...
int main(int argc, char **argv) {
  ...
  if (mode == MODE_SYNC)
    ...
    for (; optind < argc; optind++) ok &= sync_arg(mode, argv[optind]);
  }
