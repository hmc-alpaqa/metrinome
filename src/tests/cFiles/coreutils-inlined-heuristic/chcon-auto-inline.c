...
/* Given a security context, CONTEXT, derive a context_t (*RET),
   ...
   specified_role, etc.  */
__attribute__((always_inline)) inline static int compute_context_from_mask(char const *context, context_t *ret) {
  bool ok = true;
...
__attribute__((always_inline)) inline static int change_file_context(int fd, char const *file) {
  ...
  if (specified_context == NULL) {
    ...
    if (compute_context_from_mask(file_context, &context)) return 1;
...
__attribute__((always_inline)) inline static bool process_file(FTS *fts, FTSENT *ent) {
  ...
  if (ok) {
    ...
    if (change_file_context(fts->fts_cwd_fd, file) != 0) ok = false;
  }

  if (!recurse) fts_set(fts, ent, FTS_SKIP);
...
__attribute__((always_inline)) inline static bool process_files(char **files, int bit_flags) {
  bool ok = true;
...
int main(int argc, char **argv) {
  ...
  if (recurse) {
    if (bit_flags == FTS_PHYSICAL) {
      ...
      if (dereference == 0) die(EXIT_FAILURE, 0, _("-R -h requires -P"));
