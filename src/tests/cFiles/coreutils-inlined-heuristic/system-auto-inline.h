...
__attribute__((always_inline)) static inline void initialize_exit_failure(int status) {
  if (status != EXIT_FAILURE) exit_failure = status;
}
...
__attribute__((always_inline)) static inline unsigned char to_uchar(char ch) { return ch; }

/* '\n' is considered a field separator with  --zero-terminated.  */
__attribute__((always_inline)) static inline bool field_sep(unsigned char ch) {
...
__attribute__((always_inline)) static inline unsigned long int select_plural(uintmax_t n) {
...
__attribute__((always_inline)) inline char *getlogin(void);
#endif

#if !HAVE_DECL_TTYNAME
__attribute__((always_inline)) inline char *ttyname(int);
#endif
...
#define X2NREALLOC(P, PN) \
  verify_expr(sizeof *(P) != 1, x2nrealloc(P, PN, sizeof *(P)))
...
__attribute__((always_inline)) static inline bool dot_or_dotdot(char const *file_name) {
  if (file_name[0] == '.') {
...
__attribute__((always_inline)) static inline struct dirent const *readdir_ignoring_dot_and_dotdot(DIR *dirp) {
  while (1) {
    struct dirent const *dp = readdir(dirp);
    if (dp == NULL || !dot_or_dotdot(dp->d_name)) return dp;
  }
}
...
__attribute__((always_inline)) static inline bool is_empty_dir(int fd_cwd, char const *dir) {
  DIR *dirp;
...
__attribute__((always_inline)) static inline size_t _GL_ATTRIBUTE_CONST gcd(size_t u, size_t v) {
  do {
    size_t t = u % v;
    u = v;
    v = t;
...
__attribute__((always_inline)) static inline size_t _GL_ATTRIBUTE_CONST lcm(size_t u, size_t v) {
  return u * (v / gcd(u, v));
}
...
__attribute__((always_inline)) static inline void *ptr_align(void const *ptr, size_t alignment) {
  char const *p0 = ptr;
...
__attribute__((always_inline)) static inline bool _GL_ATTRIBUTE_PURE is_nul(void const *buf, size_t length) {
...
__attribute__((always_inline)) static inline void emit_stdin_note(void) {
  fputs(_("\n\
With no FILE, or when FILE is -, read standard input.\n\
"),
        stdout);
}
__attribute__((always_inline)) static inline void emit_mandatory_arg_note(void) {
  fputs(_("\n\
...
__attribute__((always_inline)) static inline void emit_size_note(void) {
  fputs(_("\n\
...
__attribute__((always_inline)) static inline void emit_blocksize_note(char const *program) {
  printf(_("\n\
...
__attribute__((always_inline)) static inline void emit_backup_suffix_note(void) {
  fputs(_("\
\n\
...
__attribute__((always_inline)) static inline void emit_ancillary_info(char const *program) {
  struct infomap {
    char const *program;
    char const *node;
...
__attribute__((always_inline)) /* Use a macro rather than an inline function, as this references
...
__attribute__((always_inline)) static inline char *timetostr(time_t t, char *buf) {
  return (TYPE_SIGNED(time_t) ? imaxtostr(t, buf) : umaxtostr(t, buf));
}

__attribute__((always_inline)) static inline char *bad_cast(char const *s) { return (char *)s; }

/* Return a boolean indicating whether SB->st_size is defined.  */
__attribute__((always_inline)) static inline bool usable_st_size(struct stat const *sb) {
  return (S_ISREG(sb->st_mode) || S_ISLNK(sb->st_mode) || S_TYPEISSHM(sb) ||
          S_TYPEISTMO(sb));
}

__attribute__((always_inline)) inline void usage(int status) ATTRIBUTE_NORETURN;

/* Like error(0, 0, ...), but without an implicit newline.
   Also a noop unless the global DEV_DEBUG is set.  */
#define devmsg(...)                              \
  do {                                           \
    if (dev_debug) fprintf(stderr, __VA_ARGS__); \
  } while (0)
...
__attribute__((always_inline)) static inline char *stzncpy(char *restrict dest, char const *restrict src,
                            size_t len) {
  char const *src_end = src + len;
  while (src < src_end && *src) *dest++ = *src++;
  *dest = 0;
  return dest;
}
...
__attribute__((always_inline)) static inline char *se_const(char const *sctx) { return (char *)sctx; }
...
__attribute__((always_inline)) static inline bool is_ENOTSUP(int err) {
