...
__attribute__((always_inline)) inline static bool have_same_content(int a_fd, int b_fd) {
  ...
  while (0 < (size = full_read(a_fd, a_buff, sizeof a_buff))) {
    if (size != full_read(b_fd, b_buff, sizeof b_buff)) return false;

    if (memcmp(a_buff, b_buff, size) != 0) return false;
  }
...
__attribute__((always_inline)) inline static bool extra_mode(mode_t input) {
  mode_t mask = S_IRWXUGO | S_IFMT;
  return !!(input & ~mask);
}

/* Return true if copy of file SRC_NAME to file DEST_NAME is necessary. */
__attribute__((always_inline)) inline static bool need_copy(const char *src_name, const char *dest_name,
                      const struct cp_options *x) {
  struct stat src_sb, dest_sb;
  int src_fd, dest_fd;
  bool content_match;

  if (extra_mode(mode)) return true;

  /* compare files using stat */
  if (lstat(src_name, &src_sb) != 0) return true;

  if (lstat(dest_name, &dest_sb) != 0) return true;
  ...
  if (owner_id == (uid_t)-1) {
    errno = 0;
    uid_t ruid = getuid();
    if ((ruid == (uid_t)-1 && errno) || dest_sb.st_uid != ruid) return true;
  ...
  if (group_id == (uid_t)-1) {
    errno = 0;
    gid_t rgid = getgid();
    if ((rgid == (uid_t)-1 && errno) || dest_sb.st_gid != rgid) return true;
  ...
  if (selinux_enabled && x->preserve_security_context) {
    ...
    if (getfilecon(src_name, &file_scontext) == -1) return true;
    ...
    freecon(file_scontext);
    freecon(to_scontext);
    if (!scontext_match) return true;
  }

  /* compare files content */
  src_fd = open(src_name, O_RDONLY | O_BINARY);
  if (src_fd < 0) return true;
...
__attribute__((always_inline)) inline static void cp_option_init(struct cp_options *x) {
...
__attribute__((always_inline)) inline static void setdefaultfilecon(char const *file) {
  struct stat st;
  char *scontext = NULL;
  ...
  if ((matchpathcon(file, st.st_mode, &scontext) != 0) ||
      STREQ(scontext, "<<none>>")) {
    if (scontext != NULL) freecon(scontext);
    return;
  }
...
#else
__attribute__((always_inline)) inline static void setdefaultfilecon(char const *file) { (void)file; }
#endif
...
__attribute__((always_inline)) inline static bool target_directory_operand(char const *file) {
  ...
  if (is_a_dir < looks_like_a_dir)
    die(EXIT_FAILURE, err, _("target %s is not a directory"), quoteaf(file));
  return is_a_dir;
}

/* Report that directory DIR was made, if OPTIONS requests this.  */
__attribute__((always_inline)) inline static void announce_mkdir(char const *dir, void *options) {
...
/* Make ancestor directory DIR, whose last file name component is
   ...
   COMPONENT's parent.  */
__attribute__((always_inline)) inline static int make_ancestor(char const *dir, char const *component,
                         void *options) {
  struct cp_options const *x = options;
  ...
  if (r == 0) announce_mkdir(dir, options);
  return r;
}

/* Process a command-line file name, for the -d option.  */
__attribute__((always_inline)) inline static int process_dir(char *dir, struct savewd *wd, void *options) {
  struct cp_options const *x = options;
...
__attribute__((always_inline)) inline static bool copy_file(const char *from, const char *to,
                      const struct cp_options *x) {
  bool copy_into_self;

  if (copy_only_if_needed && !need_copy(from, to, x)) return true;
...
__attribute__((always_inline)) inline static bool change_attributes(char const *name) {
  bool ok = false;
  ...
  if (use_default_selinux_context) setdefaultfilecon(name);
...
__attribute__((always_inline)) inline static bool change_timestamps(struct stat const *src_sb, char const *dest) {
...
__attribute__((always_inline)) inline static bool strip(char const *name) {
  int status;
  bool ok = false;
  pid_t pid = fork();
...
__attribute__((always_inline)) inline static void get_ids(void) {
  struct passwd *pw;
  struct group *gr;
  ...
  if (owner_name) {
    ...
    if (pw == NULL) {
      ...
      if (xstrtoumax(owner_name, NULL, 0, &tmp, "") != LONGINT_OK ||
          UID_T_MAX < tmp)
        die(EXIT_FAILURE, 0, _("invalid user %s"), quote(owner_name));
      owner_id = tmp;
    } else
      owner_id = pw->pw_uid;
    endpwent();
  ...
  if (group_name) {
    ...
    if (gr == NULL) {
      ...
      if (xstrtoumax(group_name, NULL, 0, &tmp, "") != LONGINT_OK ||
          GID_T_MAX < tmp)
        die(EXIT_FAILURE, 0, _("invalid group %s"), quote(group_name));
      group_id = tmp;
    } else
      group_id = gr->gr_gid;
    endgrent();
...
__attribute__((always_inline)) inline static bool install_file_in_file(const char *from, const char *to,
                                 const struct cp_options *x) {
  struct stat from_sb;
  ...
  if (!copy_file(from, to, x)) return false;
  if (strip_files)
    if (!strip(to)) {
...
__attribute__((always_inline)) inline static bool mkancesdirs_safe_wd(char const *from, char *to,
                                struct cp_options *x, bool save_always) {
  bool save_working_directory = save_always || !(IS_ABSOLUTE_FILE_NAME(from) &&
                                                 IS_ABSOLUTE_FILE_NAME(to));
  int status = EXIT_SUCCESS;

  struct savewd wd;
  savewd_init(&wd);
  if (!save_working_directory) savewd_finish(&wd);
  ...
  if (save_working_directory) {
    ...
    savewd_finish(&wd);
    if (EXIT_SUCCESS < restore_result) return false;
...
__attribute__((always_inline)) inline static bool install_file_in_file_parents(char const *from, char *to,
                                         const struct cp_options *x) {
...
__attribute__((always_inline)) inline static bool install_file_in_dir(const char *from, const char *to_dir,
                                const struct cp_options *x,
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "bcCsDdg:m:o:pt:TvS:Z", long_options,
    ...
    switch (optc) {
      ...
      case PRESERVE_CONTEXT_OPTION:
        if (!selinux_enabled) {
          error(0, 0,
                _("WARNING: ignoring --preserve-context; "
  ...
  if (strip_program_specified && !strip_files)
    error(0, 0,
          _("WARNING: ignoring --strip-program option as -s option was "
            "not specified"));

  if (copy_only_if_needed && x.preserve_timestamps) {
    error(0, 0,
          _("options --compare (-C) and --preserve-timestamps are "
            "mutually exclusive"));
    usage(EXIT_FAILURE);
  }
  ...
  if (copy_only_if_needed && strip_files) {
    error(0, 0,
          _("options --compare (-C) and --strip are mutually "
            "exclusive"));
    usage(EXIT_FAILURE);
  }
  ...
  if (copy_only_if_needed && extra_mode(mode))
    error(0, 0,
          _("the --compare (-C) option is ignored when you"
