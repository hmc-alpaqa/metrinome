...
int main(int argc, char **argv) {
  ...
  if (specified_mode) {
    ...
    if (!change) die(EXIT_FAILURE, 0, _("invalid mode"));
    umask_value = umask(0);
    umask(umask_value);
    newmode = mode_adjust(newmode, false, umask_value, change, NULL);
    free(change);
    if (newmode & ~S_IRWXUGO)
      die(EXIT_FAILURE, 0, _("mode must specify only file permission bits"));
  }
  ...
  switch (argv[optind + 1][0]) {
    ...
    block_or_character : {
      ...
      if (xstrtoumax(s_major, NULL, 0, &i_major, "") != LONGINT_OK ||
        ...
        die(EXIT_FAILURE, 0, _("invalid major device number %s"),
            quote(s_major));

      if (xstrtoumax(s_minor, NULL, 0, &i_minor, "") != LONGINT_OK ||
          i_minor != (minor_t)i_minor)
        die(EXIT_FAILURE, 0, _("invalid minor device number %s"),
            quote(s_minor));

      device = makedev(i_major, i_minor);
#ifdef NODEV
      if (device == NODEV)
        die(EXIT_FAILURE, 0, _("invalid device %s %s"), s_major, s_minor);
#endif

      if (set_security_context) defaultcon(argv[optind], node_type);
    ...
    case 'p': /* 'pipe' */
      if (set_security_context) defaultcon(argv[optind], S_IFIFO);
