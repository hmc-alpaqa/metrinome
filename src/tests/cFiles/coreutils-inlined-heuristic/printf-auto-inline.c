...
__attribute__((always_inline)) inline static void verify_numeric(const char *s, const char *end) {
  if (errno) {
...
#define STRTOX(TYPE, FUNC_NAME, LIB_FUNC_EXPR)                         \
  static TYPE FUNC_NAME(char const *s) {                               \
    ...
    if ((*s == '\"' || *s == '\'') && *(s + 1)) {                      \
      ...
      if (*++s != 0 && !posixly_correct) error(0, 0, _(cfcc_msg), s);  \
    } else {                                                           \
      errno = 0;                                                       \
      val = (LIB_FUNC_EXPR);                                           \
...
__attribute__((always_inline)) inline static int print_esc(const char *escstart, bool octal_0) {
...
__attribute__((always_inline)) inline static void print_esc_string(const char *str) {
  for (; *str; str++)
    if (*str == '\\')
...
__attribute__((always_inline)) inline static void print_direc(const char *start, size_t length, char conversion,
...
__attribute__((always_inline)) inline static int print_formatted(const char *format, int argc, char **argv) {
    ...
    switch (*f) {
      case '%':
        ...
        ok['a'] = ok['A'] = ok['c'] = ok['d'] = ok['e'] = ok['E'] = ok['f'] =
            ok['F'] = ok['g'] = ok['G'] = ok['i'] = ok['o'] = ok['s'] =
                ok['u'] = ok['x'] = ok['X'] = 1;

        for (;; f++, direc_length++) switch (*f) {
#if (__GLIBC__ == 2 && 2 <= __GLIBC_MINOR__) || 3 <= __GLIBC__
            case 'I':
#endif
            case '\'':
              ok['a'] = ok['A'] = ok['c'] = ok['e'] = ok['E'] = ok['o'] =
                  ok['s'] = ok['x'] = ok['X'] = 0;
              break;
            case '-':
            case '+':
      ...
      no_more_flag_characters:
        ...
        if (*f == '*') {
          ...
          if (argc > 0) {
            ...
            if (INT_MIN <= width && width <= INT_MAX)
              ...
              die(EXIT_FAILURE, 0, _("invalid field width: %s"), quote(*argv));
            ++argv;
            --argc;
          } else
            field_width = 0;
          ...
          if (*f == '*') {
            ...
            if (argc > 0) {
              ...
              if (prec < 0) {
                ...
                die(EXIT_FAILURE, 0, _("invalid precision: %s"), quote(*argv));
              else
                precision = prec;
              ++argv;
              --argc;
...

        {
          ...
          if (!ok[conversion])
            die(EXIT_FAILURE, 0, _("%.*s: invalid conversion specification"),
                (int)(f + 1 - direc_start), direc_start);
        }

        print_direc(direc_start, direc_length, *f, have_field_width,
                    field_width, have_precision, precision,
                    (argc <= 0 ? "" : (argc--, *argv++)));
        break;
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    if (STREQ(argv[1], "--help")) usage(EXIT_SUCCESS);
