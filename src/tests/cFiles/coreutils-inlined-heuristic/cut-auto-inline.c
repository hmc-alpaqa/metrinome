...
__attribute__((always_inline)) static inline void next_item(uintmax_t *item_idx) {
  (*item_idx)++;
  if ((*item_idx) > current_rp->hi) current_rp++;
}

/* Return nonzero if the K'th field or byte is printable. */

__attribute__((always_inline)) static inline bool print_kth(uintmax_t k) { return current_rp->lo <= k; }

/* Return nonzero if K'th byte is the beginning of a range. */

__attribute__((always_inline)) static inline bool is_range_start_index(uintmax_t k) {
  return k == current_rp->lo;
}

/* Read from stream STREAM, printing to standard output any selected bytes.  */

__attribute__((always_inline)) inline static void cut_bytes(FILE *stream) {
  ...
  while (true) {
    ...
    if (c == line_delim) {
      ...
      if (byte_idx > 0) putchar(line_delim);
      break;
    } else {
      next_item(&byte_idx);
...
__attribute__((always_inline)) inline static void cut_fields(FILE *stream) {
  int c;
    ...
    if (field_idx == 1 && buffer_first_field) {
      ...
      if (to_uchar(field_1_buffer[n_bytes - 1]) != delim) {
        if (suppress_non_delimited) {
          ...
          if (field_1_buffer[n_bytes - 1] != line_delim) putchar(line_delim);
          c = line_delim;
        }
        continue;
      }
...
__attribute__((always_inline)) inline static void cut_stream(FILE *stream) {
...
__attribute__((always_inline)) inline static bool cut_file(char const *file) {
  FILE *stream;
...
int main(int argc, char **argv) {
  ...
  if (delim_specified && operating_mode != field_mode)
    FATAL_ERROR(
        _("an input delimiter may be specified only\
 when operating on fields"));

  if (suppress_non_delimited && operating_mode != field_mode)
    FATAL_ERROR(
        _("suppressing non-delimited lines makes sense\n\
\tonly when operating on fields"));
  ...
  if (optind == argc)
    ...
    for (ok = true; optind < argc; optind++) ok &= cut_file(argv[optind]);
