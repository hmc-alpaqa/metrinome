...
/* Return a string representing the time between WHEN and now.
   ...
   FIXME: locale? */
__attribute__((always_inline)) inline static const char *idle_string(time_t when, time_t boottime) {
  static time_t now = TYPE_MINIMUM(time_t);

  if (now == TYPE_MINIMUM(time_t)) time(&now);
...
__attribute__((always_inline)) inline static const char *time_string(const STRUCT_UTMP *utmp_ent) {
...
/* Print formatted output line. Uses mostly arbitrary field sizes, probably
   ...
   pids, etc. */
__attribute__((always_inline)) inline static void print_line(int userlen, const char *user, const char state,
...
__attribute__((always_inline)) inline static bool is_tty_writable(struct stat const *pstat) {
#ifdef TTY_GROUP_NAME
  ...
  if (!ttygr || (pstat->st_gid != ttygr->gr_gid)) return false;
#endif
...
__attribute__((always_inline)) inline static void print_user(const STRUCT_UTMP *utmp_ent, time_t boottime) {
  struct stat stats;
  time_t last_change;
  char mesg;
...
__attribute__((always_inline)) inline static void print_boottime(const STRUCT_UTMP *utmp_ent) {
  print_line(-1, "", ' ', -1, _("system boot"), time_string(utmp_ent), "", "",
             "", "");
}

__attribute__((always_inline)) inline static char *make_id_equals_comment(STRUCT_UTMP const *utmp_ent) {
...
__attribute__((always_inline)) inline static void print_deadprocs(const STRUCT_UTMP *utmp_ent) {
  static char *exitstr;
...
__attribute__((always_inline)) inline static void print_login(const STRUCT_UTMP *utmp_ent) {
  char *comment = make_id_equals_comment(utmp_ent);
  PIDSTR_DECL_AND_INIT(pidstr, utmp_ent);
...
__attribute__((always_inline)) inline static void print_initspawn(const STRUCT_UTMP *utmp_ent) {
  char *comment = make_id_equals_comment(utmp_ent);
  PIDSTR_DECL_AND_INIT(pidstr, utmp_ent);
...
__attribute__((always_inline)) inline static void print_clockchange(const STRUCT_UTMP *utmp_ent) {
  /* FIXME: handle NEW_TIME & OLD_TIME both */
  print_line(-1, "", ' ', -1, _("clock change"), time_string(utmp_ent), "", "",
             "", "");
}

__attribute__((always_inline)) inline static void print_runlevel(const STRUCT_UTMP *utmp_ent) {
  static char *runlevline, *comment;
  unsigned char last = UT_PID(utmp_ent) / 256;
  unsigned char curr = UT_PID(utmp_ent) % 256;

  if (!runlevline) runlevline = xmalloc(strlen(_("run-level")) + 3);
...
__attribute__((always_inline)) inline static void list_entries_who(size_t n, const STRUCT_UTMP *utmp_buf) {
...
__attribute__((always_inline)) inline static void print_heading(void) {
  print_line(-1, _("NAME"), ' ', -1, _("LINE"), _("TIME"), _("IDLE"), _("PID"),
             _("COMMENT"), _("EXIT"));
}

/* Display UTMP_BUF, which should have N entries. */
__attribute__((always_inline)) inline static void scan_entries(size_t n, const STRUCT_UTMP *utmp_buf) {
  char *ttyname_b IF_LINT(= NULL);
  time_t boottime = TYPE_MINIMUM(time_t);

  if (include_heading) print_heading();
  ...
  while (n--) {
    ...
    if (UT_TYPE_BOOT_TIME(utmp_buf)) boottime = UT_TIME_MEMBER(utmp_buf);
...
__attribute__((always_inline)) inline static void who(const char *filename, int options) {
  size_t n_users;
