...
__attribute__((always_inline)) inline extern void add_tab_stop(uintmax_t tabval) {
...
__attribute__((always_inline)) inline static bool set_extend_size(uintmax_t tabval) {
  bool ok = true;
...
__attribute__((always_inline)) inline static bool set_increment_size(uintmax_t tabval) {
  bool ok = true;
...
__attribute__((always_inline)) inline extern void parse_tab_stops(char const *stops) {
...
__attribute__((always_inline)) inline static void validate_tab_stops(uintmax_t const *tabs, size_t entries) {
  uintmax_t prev_tab = 0;

  for (size_t i = 0; i < entries; i++) {
    if (tabs[i] == 0) die(EXIT_FAILURE, 0, _("tab size cannot be 0"));
    if (tabs[i] <= prev_tab)
...
*/
__attribute__((always_inline)) inline extern void finalize_tab_stops(void) {
...
__attribute__((always_inline)) inline extern uintmax_t get_next_tab_column(const uintmax_t column, size_t *tab_index,
                                     bool *last_tab) {
  *last_tab = false;

  /* single tab-size - return multiples of it */
  if (tab_size) return column + (tab_size - column % tab_size);
  ...
  if (extend_size) return column + (extend_size - column % extend_size);
...
/* Sets new file-list */
__attribute__((always_inline)) inline extern void set_file_list(char **list) {
...
__attribute__((always_inline)) inline extern FILE *next_file(FILE *fp) {
...
/* */
__attribute__((always_inline)) inline extern void cleanup_file_list_stdin(void) {
  if (have_read_stdin && fclose(stdin) != 0) die(EXIT_FAILURE, errno, "-");
}

__attribute__((always_inline)) inline extern void emit_tab_list_info(void) {
