...
__attribute__((always_inline)) inline static bool mode_changed(int dir_fd, char const *file,
...
__attribute__((always_inline)) inline static void describe_change(const char *file, mode_t old_mode, mode_t mode,
...
__attribute__((always_inline)) inline static bool process_file(FTS *fts, FTSENT *ent) {
  ...
  switch (ent->fts_info) {
    ...
    case FTS_ERR:
      if (!force_silent) error(0, ent->fts_errno, "%s", quotef(file_full_name));
      ok = false;
      break;
  ...
  while (1) {
    ...
    if (ent == NULL) {
      if (errno != 0) {
        ...
        if (!force_silent) error(0, errno, _("fts_read failed"));
        ok = false;
      }
      break;
    }
