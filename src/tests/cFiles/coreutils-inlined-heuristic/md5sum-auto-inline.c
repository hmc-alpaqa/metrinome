...
#if HASH_ALGO_BLAKE2
#define AUTHORS proper_name("Padraig Brady"), proper_name("Samuel Neves")
#else
#define AUTHORS                                               \
...
#if HASH_ALGO_BLAKE2
static char const *const algorithm_in_string[] = {"blake2b", NULL};
static char const *const algorithm_out_string[] = {"BLAKE2b", NULL};
enum Algorithm { BLAKE2b };
...
static blake2fn blake2fns[] = {blake2b_stream};
static uintmax_t blake2_max_len[] = {BLAKE2B_OUTBYTES};
...
__attribute__((always_inline)) inline static char *filename_unescape(char *s, size_t s_len) {
  char *dst = s;
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE hex_digits(unsigned char const *s) {
...
__attribute__((always_inline)) inline static bool bsd_split_3(char *s, size_t s_len, unsigned char **hex_digest,
  ...
  *file_name = s;
  ...
  if (escaped_filename && filename_unescape(s, i) == NULL) return false;
...
__attribute__((always_inline)) inline static bool split_3(char *s, size_t s_len, unsigned char **hex_digest,
  ...
  if (STREQ_LEN(s + i, DIGEST_TYPE_STRING, algo_name_len)) {
    ...
    while (s[i] && !ISWHITE(s[i]) && s[i] != '-' && s[i] != '(') ++i;
    ...
    if (algo < 0) return false;
    b2_algorithm = algo;
    if (openssl_format) s[--i] = '(';
    ...
    if (length_specified) {
      ...
      if (!(xstrtoumax(s + i, &siend, 0, &length, NULL) == LONGINT_OK &&
            0 < length && length <= b2_length && length % 8 == 0))
        return false;
  ...
  /* Ignore this line if it is too short.
     ...
     information.  */
  if (s_len - i < min_digest_line_length + (s[i] == '\\')) return false;
  ...
  *hex_digest = (unsigned char *)&s[i];
  ...
  if (!hex_digits(*hex_digest)) return false;
  ...
  if ((s_len - i == 1) || (s[i] != ' ' && s[i] != '*')) {
    /* Don't allow mixing bsd and standard formats,
       ...
       a leading ' ' or '*'.  */
    if (bsd_reversed == 0) return false;
    bsd_reversed = 1;
  ...
  *file_name = &s[i];
  ...
  if (escaped_filename) return filename_unescape(&s[i], s_len - i) != NULL;
...
__attribute__((always_inline)) inline static void print_filename(char const *file, bool escape) {
  if (!escape) {
    fputs(file, stdout);
    return;
  }
...
__attribute__((always_inline)) inline static bool digest_file(const char *filename, int *binary,
                        unsigned char *bin_result, bool *missing) {
  FILE *fp;
  int err;
  ...
  *missing = false;
  ...
  if (is_stdin) {
    ...
    fp = stdin;
    if (O_BINARY && *binary) {
      if (*binary < 0) *binary = !isatty(STDIN_FILENO);
      if (*binary) xset_binary_mode(STDIN_FILENO, O_BINARY);
    }
  } else {
...
__attribute__((always_inline)) inline static bool digest_check(const char *checkfile_name) {
...
int main(int argc, char **argv) {
  ...
  if (b2_length > blake2_max_len[b2_algorithm] * 8) {
    error(0, 0, _("invalid length: %s"), quote(b2_length_str));
    die(EXIT_FAILURE, 0, _("maximum digest length for %s is %" PRIuMAX " bits"),
        quote(algorithm_in_string[b2_algorithm]),
        blake2_max_len[b2_algorithm] * 8);
  }
  if (b2_length == 0 && !do_check) b2_length = blake2_max_len[b2_algorithm] * 8;
  digest_hex_bytes = b2_length / 4;
#else
  ...
  if (delim != '\n' && do_check) {
    error(0, 0,
          _("the --zero option is not supported when "
            "verifying checksums"));
    usage(EXIT_FAILURE);
  }
  ...
  if (prefix_tag && do_check) {
    error(0, 0,
          _("the --tag option is meaningless when "
            "verifying checksums"));
    usage(EXIT_FAILURE);
  }
  ...
  if (0 <= binary && do_check) {
    error(0, 0,
          _("the --binary and --text options are meaningless when "
            "verifying checksums"));
    usage(EXIT_FAILURE);
  }
  ...
  for (char **operandp = argv + optind; operandp < operand_lim; operandp++) {
    ...
    if (do_check)
      ...
      if (!digest_file(file, &file_is_binary, bin_buffer, &missing))
        ...
        if (!prefix_tag && needs_escape) putchar('\\');
