...
__attribute__((always_inline)) static inline int default_scale_base(enum scale_type scale) {
  switch (scale) {
    case scale_IEC:
    case scale_IEC_I:
      return 1024;
...
__attribute__((always_inline)) static inline int valid_suffix(const char suf) {
  static const char *valid_suffixes = "KMGTPEZY";
  return (strchr(valid_suffixes, suf) != NULL);
}

__attribute__((always_inline)) static inline int suffix_power(const char suf) {
  switch (suf) {
    case 'K': /* kilo or kibi.  */
      return 1;
...
__attribute__((always_inline)) static inline const char *suffix_power_char(unsigned int power) {
  switch (power) {
    case 0:
      return "";
...
__attribute__((always_inline)) inline static long double powerld(long double base, unsigned int x) {
...
__attribute__((always_inline)) static inline long double absld(long double val) {
...
__attribute__((always_inline)) inline static long double expld(long double val, unsigned int base,
...
__attribute__((always_inline)) static inline intmax_t simple_round_ceiling(long double val) {
  intmax_t intval = val;
  if (intval < val) intval++;
  return intval;
}

/* EXTREMELY limited 'floor' - without 'libm'.
   Assumes values that fit in intmax_t.  */
__attribute__((always_inline)) static inline intmax_t simple_round_floor(long double val) {
  return -simple_round_ceiling(-val);
}

/* EXTREMELY limited 'round away from zero'.
   Assumes values that fit in intmax_t.  */
__attribute__((always_inline)) static inline intmax_t simple_round_from_zero(long double val) {
  return val < 0 ? simple_round_floor(val) : simple_round_ceiling(val);
}

/* EXTREMELY limited 'round away to zero'.
   Assumes values that fit in intmax_t.  */
__attribute__((always_inline)) static inline intmax_t simple_round_to_zero(long double val) { return val; }

/* EXTREMELY limited 'round' - without 'libm'.
   Assumes values that fit in intmax_t.  */
__attribute__((always_inline)) static inline intmax_t simple_round_nearest(long double val) {
  return val < 0 ? val - 0.5 : val + 0.5;
}

__attribute__((always_inline)) static inline long double _GL_ATTRIBUTE_CONST simple_round(long double val,
                                                           enum round_type t) {
  intmax_t rval;
...
__attribute__((always_inline)) inline static enum simple_strtod_error simple_strtod_int(const char *input_str,
                                                  char **endptr,
                                                  long double *value,
                                                  bool *negative) {
  ...
  *endptr = (char *)input_str;
  while (*endptr && c_isdigit(**endptr)) {
    ...
    if (digits > MAX_UNSCALED_DIGITS) e = SSE_OK_PRECISION_LOSS;

    if (digits > MAX_ACCEPTABLE_DIGITS) return SSE_OVERFLOW;
...
/* Read a floating-point INPUT_STR represented as "NNNN[.NNNNN]",
   ...
   Returns:
      ...
      SSE_OVERFLOW          - if more than 27 digits (999Y) were used.
      SSE_INVALID_NUMBER    - if no digits were found.  */
__attribute__((always_inline)) inline static enum simple_strtod_error simple_strtod_float(const char *input_str,
                                                    char **endptr,
                                                    long double *value,
                                                    size_t *precision) {
  bool negative;
  ...
  if (e != SSE_OK && e != SSE_OK_PRECISION_LOSS) return e;
  ...
  if (STREQ_LEN(*endptr, decimal_point, decimal_point_length)) {
    ...
    if (e2 != SSE_OK && e2 != SSE_OK_PRECISION_LOSS) return e2;
    if (e2 == SSE_OK_PRECISION_LOSS) e = e2; /* propagate warning.  */
    if (neg_frac) return SSE_INVALID_NUMBER;
    ...
    if (precision) *precision = exponent;
...
/* Read a 'human' INPUT_STR represented as "NNNN[.NNNNN] + suffix",
   ...
   Returns:
      ...
      SSE_INVALID_SUFFIX
      SSE_MISSING_I_SUFFIX  */
__attribute__((always_inline)) inline static enum simple_strtod_error simple_strtod_human(
    const char *input_str, char **endptr, long double *value, size_t *precision,
  ...
  if (**endptr != '\0') {
    ...
    while (isblank(to_uchar(**endptr))) (*endptr)++;

    if (!valid_suffix(**endptr)) return SSE_INVALID_SUFFIX;

    if (allowed_scaling == scale_none) return SSE_VALID_BUT_FORBIDDEN_SUFFIX;
...
__attribute__((always_inline)) inline static void simple_strtod_fatal(enum simple_strtod_error err,
...
__attribute__((always_inline)) inline static void double_to_human(long double val, int precision, char *buf,
  ...
  *pfmt++ = '%';
  ...
  if (scale == scale_none) {
    ...
    if (num_size < 0 || num_size >= (int)buf_size)
      die(EXIT_FAILURE, 0, _("failed to prepare value '%Lf' for printing"),
          val);
    return;
  }
  ...
  if (num_size < 0 || num_size >= (int)buf_size - 1)
    die(EXIT_FAILURE, 0, _("failed to prepare value '%Lf' for printing"), val);
...
__attribute__((always_inline)) inline static uintmax_t unit_to_umax(const char *n_string) {
  strtol_error s_err;
...
__attribute__((always_inline)) inline static void setup_padding_buffer(size_t min_size) {
  if (padding_buffer_size > min_size) return;
...
__attribute__((always_inline)) inline static void parse_format_string(char const *fmt) {
  size_t i;
  size_t prefix_len = 0;
  size_t suffix_pos;
  long int pad = 0;
  ...
  if (errno == ERANGE || pad < -LONG_MAX)
    die(EXIT_FAILURE, 0, _("invalid format %s (width overflow)"), quote(fmt));
  ...
  if (fmt[i] == '.') {
    ...
    if (errno == ERANGE || user_precision < 0 || SIZE_MAX < user_precision ||
      ...
      die(EXIT_FAILURE, 0, _("invalid precision in format %s"), quote(fmt));
    }
    i = endptr - fmt;
  }
  ...
  if (prefix_len) format_str_prefix = xstrndup(fmt, prefix_len);
  if (fmt[suffix_pos] != '\0') format_str_suffix = xstrdup(fmt + suffix_pos);
...
__attribute__((always_inline)) inline static enum simple_strtod_error parse_human_number(
    const char *str, long double /*output */ *value, size_t *precision) {
  char *ptr = NULL;
...
__attribute__((always_inline)) inline static int prepare_padded_number(const long double val, size_t precision) {
  /* Generate Output. */
  char buf[128];
  ...
  double_to_human(val, precision_used, buf, sizeof(buf), scale_to, grouping,
                  round_style);
  if (suffix) strncat(buf, suffix, sizeof(buf) - strlen(buf) - 1);
...
__attribute__((always_inline)) inline static void print_padded_number(void) {
  if (format_str_prefix) fputs(format_str_prefix, stdout);

  fputs(padding_buffer, stdout);

  if (format_str_suffix) fputs(format_str_suffix, stdout);
}

/* Converts the TEXT number string to the requested representation,
   and handles automatic suffix addition.  */
__attribute__((always_inline)) inline static int process_suffixed_number(char *text, long double *result,
...
__attribute__((always_inline)) inline static char *next_field(char **line) {
  char *field_start = *line;
  char *field_end = field_start;

  if (delimiter != DELIMITER_DEFAULT) {
    if (*field_start != delimiter) {
      while (*field_end && *field_end != delimiter) ++field_end;
    }
    /* else empty field */
  } else {
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE include_field(uintmax_t field) {
  struct field_range_pair *p = frp;
  if (!p) return field == 1;

  while (p->lo != UINTMAX_MAX) {
    if (p->lo <= field && p->hi >= field) return true;
    ++p;
  }
  return false;
}
...
__attribute__((always_inline)) inline static bool process_field(char *text, uintmax_t field) {
  long double val = 0;
  size_t precision = 0;
  ...
  if (include_field(field)) {
    ...
    if (valid_number) valid_number = prepare_padded_number(val, precision);
...
__attribute__((always_inline)) inline static int process_line(char *line, bool newline) {
  char *next;
  uintmax_t field = 0;
  bool valid_number = true;
  ...
  while (true) {
    ...
    if (*line != '\0') {
      ...
      *line = '\0';
      ...
      if (!process_field(next, field)) valid_number = false;
...
int main(int argc, char **argv) {
  ...
  if (decimal_point == NULL || strlen(decimal_point) == 0) decimal_point = ".";
    ...
    switch (c) {
      ...
      case PADDING_OPTION:
        if (xstrtol(optarg, NULL, 10, &padding_width, "") != LONGINT_OK ||
            padding_width == 0)
          die(EXIT_FAILURE, 0, _("invalid padding value %s"), quote(optarg));
        if (padding_width < 0) {
          padding_alignment = MBS_ALIGN_LEFT;
      ...
      case FIELD_OPTION:
        if (n_frp) die(EXIT_FAILURE, 0, _("multiple field specifications"));
      ...
      case 'd':
        ...
        if (optarg[0] != '\0' && optarg[1] != '\0')
          die(EXIT_FAILURE, 0, _("the delimiter must be a single character"));
        delimiter = optarg[0];
        break;
      ...
      case HEADER_OPTION:
        if (optarg) {
          if (xstrtoumax(optarg, NULL, 10, &header, "") != LONGINT_OK ||
              header == 0)
            die(EXIT_FAILURE, 0, _("invalid header value %s"), quote(optarg));
        } else {
          header = 1;
        }
        break;
  ...
  if (debug && !locale_ok) error(0, 0, _("failed to set locale"));
  ...
  if (format_str) parse_format_string(format_str);
  ...
  if (inval_style != inval_abort) conv_exit_code = 0;
  ...
  if (argc > optind) {
    ...
    while ((len = getdelim(&line, &line_allocated, line_delim, stdin)) > 0) {
      ...
      if (newline) line[len - 1] = '\0';
      valid_numbers &= process_line(line, newline);
    }

    IF_LINT(free(line));

    if (ferror(stdin)) error(0, errno, _("error reading input"));
  }
