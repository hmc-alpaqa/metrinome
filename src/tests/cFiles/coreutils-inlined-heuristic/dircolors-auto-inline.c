...
__attribute__((always_inline)) inline static enum Shell_syntax guess_shell_syntax(void) {
  char *shell;

  shell = getenv("SHELL");
  if (shell == NULL || *shell == '\0') return SHELL_SYNTAX_UNKNOWN;

  shell = last_component(shell);

  if (STREQ(shell, "csh") || STREQ(shell, "tcsh")) return SHELL_SYNTAX_C;

  return SHELL_SYNTAX_BOURNE;
}

__attribute__((always_inline)) inline static void parse_line(char const *line, char **keyword, char **arg) {
  char const *p;
  ...
  *arg = NULL;
  ...
  for (p = line; isspace(to_uchar(*p)); ++p) continue;
  ...
  *keyword = xstrndup(keyword_start, p - keyword_start);
  ...
  for (--p; isspace(to_uchar(*p)); --p) continue;
  ++p;
...
__attribute__((always_inline)) inline static void append_quoted(const char *str) {
...
__attribute__((always_inline)) inline static bool dc_parse_stream(FILE *fp, const char *filename) {
  ...
  while (1) {
    ...
    if (fp) {
      ...
      if (next_G_line == G_line + sizeof G_line) break;
      line = next_G_line;
      ...
      if (state != ST_TERMNO) {
        if (keywd[0] == '.') {
          ...
          for (i = 0; slack_codes[i] != NULL; ++i)
            if (c_strcasecmp(keywd, slack_codes[i]) == 0) break;
...
__attribute__((always_inline)) inline static bool dc_parse_file(const char *filename) {
  bool ok;
