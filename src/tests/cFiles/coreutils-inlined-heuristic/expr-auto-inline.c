...
__attribute__((always_inline)) inline static int mpz_init_set_str(mpz_t z, char *s, int base) {
  return xstrtoimax(s, NULL, base, z, "") == LONGINT_OK ? 0 : -1;
}
__attribute__((always_inline)) inline static void mpz_add(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a + b;
  if ((val < a) != (b < 0)) integer_overflow('+');
  r[0] = val;
}
__attribute__((always_inline)) inline static void mpz_sub(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a - b;
  if ((a < val) != (b < 0)) integer_overflow('-');
  r[0] = val;
}
__attribute__((always_inline)) inline static void mpz_mul(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a * b;
...
__attribute__((always_inline)) inline static void mpz_tdiv_q(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];

  /* Some x86-style hosts raise an exception for INT_MIN / -1.  */
  if (a < -INTMAX_MAX && b == -1) integer_overflow('/');
  r[0] = a / b;
}
__attribute__((always_inline)) inline static void mpz_tdiv_r(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
...
__attribute__((always_inline)) inline static char *_GL_ATTRIBUTE_MALLOC mpz_get_str(char const *str, int base,
                                              mpz_t z) {
  (void)str;
  (void)base;
...
__attribute__((always_inline)) inline static size_t mbs_logical_cspn(const char *s, const char *accept) {
  size_t idx = 0;
  ...
  if (MB_CUR_MAX > 1) {
    ...
    for (mbui_init(iter, s); mbui_avail(iter); mbui_advance(iter)) {
      ...
      if (mb_len(mbui_cur(iter)) == 1) {
        if (mbschr(accept, *mbui_cur_ptr(iter))) return idx;
      } else {
        mbui_iterator_t aiter;

        for (mbui_init(aiter, accept); mbui_avail(aiter); mbui_advance(aiter))
          if (mb_equal(mbui_cur(aiter), mbui_cur(iter))) return idx;
      }
    }
...
__attribute__((always_inline)) inline static char *mbs_logical_substr(const char *s, size_t pos, size_t len) {
  char *v, *vlim;

  size_t blen = strlen(s);                           /* byte length */
  size_t llen = (MB_CUR_MAX > 1) ? mbslen(s) : blen; /* logical length */

  if (llen < pos || pos == 0 || len == 0 || len == SIZE_MAX) return xstrdup("");
  ...
  if (MB_CUR_MAX == 1) {
    ...
    mbui_iterator_t iter;
    size_t idx = 1;
    for (mbui_init(iter, s); mbui_avail(iter) && vlen > 0;
...
__attribute__((always_inline)) inline static size_t mbs_offset_to_chars(const char *s, size_t ofs) {
  mbui_iterator_t iter;
  size_t c = 0;
  for (mbui_init(iter, s); mbui_avail(iter); mbui_advance(iter)) {
...
__attribute__((always_inline)) inline static void integer_overflow(char op) { die(EXPR_FAILURE, ERANGE, "%c", op); }
#endif
...
__attribute__((always_inline)) inline static VALUE *int_value(unsigned long int i) {
...
__attribute__((always_inline)) inline static VALUE *str_value(char const *s) {
  VALUE *v = xmalloc(sizeof *v);
  v->type = string;
  v->u.s = xstrdup(s);
  return v;
...
__attribute__((always_inline)) inline static void freev(VALUE *v) {
  if (v->type == string)
    free(v->u.s);
  else
    mpz_clear(v->u.i);
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE null(VALUE *v) {
  switch (v->type) {
    case integer:
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE looks_like_integer(char const *cp) {
  cp += (*cp == '-');
...
__attribute__((always_inline)) inline static void tostring(VALUE *v) {
  switch (v->type) {
    case integer: {
...
__attribute__((always_inline)) inline static bool toarith(VALUE *v) {
  switch (v->type) {
    case integer:
      return true;
    case string: {
      ...
      if (!looks_like_integer(s)) return false;
...
__attribute__((always_inline)) inline static size_t getsize(mpz_t i) {
  if (mpz_sgn(i) < 0) return SIZE_MAX;
  if (mpz_fits_ulong_p(i)) {
    unsigned long int ul = mpz_get_ui(i);
    if (ul < SIZE_MAX) return ul;
  }
  return SIZE_MAX - 1;
}
...
__attribute__((always_inline)) inline static bool nextarg(char const *str) {
  if (*args == NULL)
    return false;
  else {
...
/* Report missing operand.
   ...
   and (args-1) is valid. */
__attribute__((always_inline)) inline static void require_more_args(void) {
  if (nomoreargs())
...
__attribute__((always_inline)) inline static VALUE *docolon(VALUE *sv, VALUE *pv) {
  ...
  if (errmsg) die(EXPR_INVALID, 0, "%s", (errmsg));
  ...
  if (0 <= matchlen) {
    ...
    if (re_buffer.re_nsub > 0) {
      ...
      size_t i =
          (MB_CUR_MAX == 1 ? matchlen : mbs_offset_to_chars(sv->u.s, matchlen));
      v = int_value(i);
    }
  } else if (matchlen == -1) {
    ...
    die(EXPR_FAILURE, (matchlen == -2 ? errno : EOVERFLOW),
...
__attribute__((always_inline)) inline static VALUE *eval7(bool evaluate) {
  VALUE *v;
  ...
  if (nextarg(")")) die(EXPR_INVALID, 0, _("syntax error: unexpected ')'"));
...
__attribute__((always_inline)) inline static VALUE *eval6(bool evaluate) {
  VALUE *l;
  VALUE *r;
  VALUE *v;
  VALUE *i1;
...
__attribute__((always_inline)) inline static VALUE *eval5(bool evaluate) {
  VALUE *l;
  VALUE *r;
  VALUE *v;
...
__attribute__((always_inline)) inline static VALUE *eval4(bool evaluate) {
  VALUE *l;
  VALUE *r;
...
__attribute__((always_inline)) inline static VALUE *eval3(bool evaluate) {
  VALUE *l;
  VALUE *r;
  enum { plus, minus } fxn;
...
__attribute__((always_inline)) inline static VALUE *eval2(bool evaluate) {
  VALUE *l;
    ...
    if (evaluate) {
      ...
      if (looks_like_integer(l->u.s) && looks_like_integer(r->u.s))
        ...
        if (errno) {
          ...
          die(EXPR_INVALID, 0, _("the strings compared were %s and %s"),
...
__attribute__((always_inline)) inline static VALUE *eval1(bool evaluate) {
  VALUE *l;
  VALUE *r;
...
__attribute__((always_inline)) inline static VALUE *eval(bool evaluate) {
  VALUE *l;
  VALUE *r;
