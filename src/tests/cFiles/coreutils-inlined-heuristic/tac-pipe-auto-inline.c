...
__attribute__((always_inline)) inline static bool buf_init_from_stdin(Buf *x, char eol_byte) {
...
__attribute__((always_inline)) inline static void buf_free(Buf *x) {
  for (size_t i = 0; i < x->n_bufs; i++) free(x->p[i].start);
...
__attribute__((always_inline)) inline static bool find_bol(const Buf *x, const Line_ptr *last_bol, Line_ptr *new_bol,
                     char eol_byte) {
  size_t i;
  Line_ptr tmp;
  char *last_bol_ptr;

  if (last_bol->ptr == x->p[0].start) return false;
...
__attribute__((always_inline)) inline static void print_line(FILE *out_stream, const Buf *x, const Line_ptr *bol,
...
__attribute__((always_inline)) inline static bool tac_mem() {
  Buf x;
  Line_ptr bol;
  char eol_byte = '\n';
  ...
  while (1) {
    Line_ptr new_bol;
    if (!find_bol(&x, &bol, &new_bol, eol_byte)) break;
