...
#define AUTHORS                                                     \
  proper_name("Torbjorn Granlund"), proper_name("David MacKenzie"), \
      proper_name("Paul Eggert"), proper_name("Jim Meyering")
...
__attribute__((always_inline)) static inline void duinfo_init(struct duinfo *a) {
  a->size = 0;
  a->inodes = 0;
...
/* Set directory data.  */
__attribute__((always_inline)) static inline void duinfo_set(struct duinfo *a, uintmax_t size,
                              struct timespec tmax) {
  a->size = size;
  a->inodes = 1;
  a->tmax = tmax;
...
__attribute__((always_inline)) static inline void duinfo_add(struct duinfo *a, struct duinfo const *b) {
  uintmax_t sum = a->size + b->size;
  a->size = a->size <= sum ? sum : UINTMAX_MAX;
  a->inodes = a->inodes + b->inodes;
  if (timespec_cmp(a->tmax, b->tmax) < 0) a->tmax = b->tmax;
}
...
__attribute__((always_inline)) inline static bool hash_ins(struct di_set *di_set, ino_t ino, dev_t dev) {
  int inserted = di_set_insert(di_set, dev, ino);
  if (inserted < 0) xalloc_die();
  return inserted;
}
...
__attribute__((always_inline)) inline static void show_date(const char *format, struct timespec when, timezone_t tz) {
  struct tm tm;
...
__attribute__((always_inline)) inline static void print_only_size(uintmax_t n_bytes) {
  char buf[LONGEST_HUMAN_READABLE + 1];
  fputs(
      (n_bytes == UINTMAX_MAX ? _("Infinity")
...
__attribute__((always_inline)) inline static void print_size(const struct duinfo *pdui, const char *string) {
...
__attribute__((always_inline)) inline static void fill_mount_table(void) {
...
__attribute__((always_inline)) inline static bool mount_point_in_fts_cycle(FTSENT const *ent) {
  FTSENT const *cycle_ent = ent->fts_cycle;

  if (!di_mnt) {
    /* Initialize the set of dev,inode pairs.  */
    di_mnt = di_set_alloc();
    if (!di_mnt) xalloc_die();
...
__attribute__((always_inline)) inline static bool process_file(FTS *fts, FTSENT *ent) {
  bool ok = true;
  struct duinfo dui;
  ...
  duinfo_set(
      &dui,
      (apparent_size ? MAX(0, sb->st_size)
...
__attribute__((always_inline)) inline static bool du_files(char **files, int bit_flags) {
  bool ok = true;
...
int main(int argc, char **argv) {
  ...
  if (opt_inodes) {
    if (apparent_size) {
      error(0, 0,
            _("warning: options --apparent-size and -b are "
