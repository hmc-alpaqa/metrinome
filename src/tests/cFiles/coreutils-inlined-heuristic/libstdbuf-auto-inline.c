...
__attribute__((always_inline)) inline static const char *fileno_to_name(const int fd) {
  const char *ret = NULL;
...
__attribute__((always_inline)) inline static void apply_mode(FILE *stream, const char *mode) {
  char *buf = NULL;
  int setvbuf_mode;
  uintmax_t size = 0;
...
__attribute__((always_inline)) inline static void __attribute((constructor)) stdbuf(void) {
