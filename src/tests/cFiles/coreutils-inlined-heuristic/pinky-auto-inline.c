...
__attribute__((always_inline)) inline static size_t _GL_ATTRIBUTE_PURE count_ampersands(const char *str) {
  size_t count = 0;
  do {
    if (*str == '&') count++;
  } while (*str++);
...
__attribute__((always_inline)) inline static char *create_fullname(const char *gecos_name, const char *user_name) {
  ...
  if (ampersands != 0) {
    ...
    if (xalloc_oversized(ulen, ampersands) || rsize < product) xalloc_die();
  }
  ...
  while (*gecos_name) {
    if (*gecos_name == '&') {
      ...
      if (islower(to_uchar(*uname))) *r++ = toupper(to_uchar(*uname++));
      while (*uname) *r++ = *uname++;
    } else {
      *r++ = *gecos_name;
    }
...
__attribute__((always_inline)) inline static const char *idle_string(time_t when) {
  static time_t now = 0;
...
__attribute__((always_inline)) inline static void print_entry(const STRUCT_UTMP *utmp_ent) {
  struct stat stats;
  time_t last_change;
  char mesg;
...
#define DEV_DIR_LEN (sizeof(DEV_DIR_WITH_TRAILING_SLASH) - 1)
  ...
  if (include_where && utmp_ent->ut_host[0]) {
    ...
    if (*ut_host) /* See if we can canonicalize it.  */
...
__attribute__((always_inline)) inline static void print_long_entry(const char name[]) {
  struct passwd *pw;
...
__attribute__((always_inline)) inline static void print_heading(void) {
  printf("%-8s", _("Login"));
  if (include_fullname) printf(" %-19s", _("Name"));
  printf(" %-9s", _(" TTY"));
  if (include_idle) printf(" %-6s", _("Idle"));
  printf(" %-*s", time_format_width, _("When"));
#ifdef HAVE_UT_HOST
  if (include_where) printf(" %s", _("Where"));
#endif
  putchar('\n');
}
...
__attribute__((always_inline)) inline static void short_pinky(const char *filename, const int argc_names,
...
__attribute__((always_inline)) inline static void long_pinky(const int argc_names, char *const argv_names[]) {
  for (int i = 0; i < argc_names; i++) print_long_entry(argv_names[i]);
}
