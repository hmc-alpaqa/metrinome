...
__attribute__((always_inline)) inline static bool errno_rmdir_non_empty(int error_number) {
...
__attribute__((always_inline)) inline static bool errno_may_be_non_empty(int error_number) {
  switch (error_number) {
    case EACCES:
    case EPERM:
    case EROFS:
...
__attribute__((always_inline)) inline static bool ignorable_failure(int error_number, char const *dir) {
  return (
      ignore_fail_on_non_empty &&
      (errno_rmdir_non_empty(error_number) ||
       (errno_may_be_non_empty(error_number) && !is_empty_dir(AT_FDCWD, dir) &&
...
__attribute__((always_inline)) inline static bool remove_parents(char *dir) {
  char *slash;
  bool ok = true;
  ...
  while (1) {
    ...
    if (!ok) {
      ...
      if (ignorable_failure(rmdir_errno, dir)) {
        ...
        error(0, rmdir_errno, _("failed to remove directory %s"), quoteaf(dir));
      }
      break;
    }
  }
...
int main(int argc, char **argv) {
  ...
  for (; optind < argc; ++optind) {
    ...
    if (rmdir(dir) != 0) {
      int rmdir_errno = errno;
      if (ignorable_failure(rmdir_errno, dir)) continue;
