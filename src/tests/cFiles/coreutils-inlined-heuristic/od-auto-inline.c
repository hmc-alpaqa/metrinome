...
__attribute__((always_inline)) inline static void dump_hexl_mode_trailer(size_t n_bytes, const char *block) {
  fputs("  >", stdout);
...
__attribute__((always_inline)) inline static void print_named_ascii(size_t fields, size_t blank, void const *block,
...
__attribute__((always_inline)) inline static void print_ascii(size_t fields, size_t blank, void const *block,
...
__attribute__((always_inline)) inline static bool simple_strtoul(const char *s, const char **p,
  ...
  while (ISDIGIT(*s)) {
    int c = *s++ - '0';
    if (sum > (ULONG_MAX - c) / 10) return false;
    sum = sum * 10 + c;
  }
  *p = s;
  *val = sum;
...
__attribute__((always_inline)) inline static bool decode_one_format(const char *s_orig, const char *s,
...
__attribute__((always_inline)) inline static bool open_next_file(void) {
  bool ok = true;

  do {
    input_filename = *file_list;
    if (input_filename == NULL) return ok;
    ++file_list;
...
__attribute__((always_inline)) inline static bool check_and_close(int in_errno) {
  bool ok = true;

  if (in_stream != NULL) {
    if (ferror(in_stream)) {
      error(0, in_errno, _("%s: read error"), quotef(input_filename));
      if (!STREQ(file_list[-1], "-")) fclose(in_stream);
      ok = false;
...
__attribute__((always_inline)) inline static bool decode_format_string(const char *s) {
  const char *s_orig = s;
  assert(s != NULL);
  ...
  while (*s != '\0') {
    ...
    if (!decode_one_format(s_orig, s, &next, &spec[n_specs])) return false;
...
__attribute__((always_inline)) inline static bool skip(uintmax_t n_skip) {
  bool ok = true;
  int in_errno = 0;

  if (n_skip == 0) return true;
...
__attribute__((always_inline)) inline static void format_address_none(uintmax_t address _GL_UNUSED,
                                char c _GL_UNUSED) {}

__attribute__((always_inline)) inline static void format_address_std(uintmax_t address, char c) {
...
__attribute__((always_inline)) inline static void format_address_paren(uintmax_t address, char c) {
  putchar('(');
  format_address_std(address, ')');
  if (c) putchar(c);
}

__attribute__((always_inline)) inline static void format_address_label(uintmax_t address, char c) {
  format_address_std(address, ' ');
...
__attribute__((always_inline)) inline static void write_block(uintmax_t current_offset, size_t n_bytes,
...
__attribute__((always_inline)) inline static bool read_char(int *c) {
  bool ok = true;
...
__attribute__((always_inline)) inline static bool read_block(size_t n, char *block, size_t *n_bytes_in_buffer) {
  bool ok = true;
...
__attribute__((always_inline)) inline static int _GL_ATTRIBUTE_PURE get_lcm(void) {
  int l_c_m = 1;
...
__attribute__((always_inline)) inline static bool parse_old_offset(const char *s, uintmax_t *offset) {
  int radix;
...
__attribute__((always_inline)) inline static bool dump(void) {
  char *block[2];
  ...
  if (limit_bytes_to_format) {
    while (1) {
      ...
      if (n_bytes_read < bytes_per_block) break;
...
__attribute__((always_inline)) inline static bool dump_strings(void) {
  ...
  tryline:
    ...
    while (!limit_bytes_to_format || address < end_offset) {
      ...
      if (!isprint(c)) goto tryline; /* It isn't; give up on this string.  */
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i <= MAX_INTEGRAL_TYPE_SIZE; i++) integral_type_size[i] = NO_SIZE;
  ...
  for (i = 0; i <= MAX_FP_TYPE_SIZE; i++) fp_type_size[i] = NO_SIZE;
      ...
      case 'A':
        ...
        switch (optarg[0]) {
          ...
          case 'n':
            ...
            break;
          default:
            die(EXIT_FAILURE, 0, _("invalid output address radix '%c';\
...
#undef CASE_OLD_ARG
  ...
  if (flag_dump_strings && n_specs > 0)
    die(EXIT_FAILURE, 0, _("no type may be specified when dumping strings"));
  ...
  */
  ...
  if (n_specs == 0) decode_format_string("oS");
  ...
  ok = open_next_file();
  if (in_stream == NULL) goto cleanup;
  ...
  for (i = 0; i < n_specs; i++) {
    ...
    if (width_per_block < block_width) width_per_block = block_width;
  }
  for (i = 0; i < n_specs; i++) {
    int fields_per_block = bytes_per_block / width_bytes[spec[i].size];
