...
#define AUTHORS                                                    \
  proper_name("Richard Mlynarik"), proper_name("David MacKenzie"), \
...
__attribute__((always_inline)) inline static void append_unset_var(const char *var) {
  if (usvars_used == usvars_alloc)
    usvars = x2nrealloc(usvars, &usvars_alloc, sizeof *usvars);
  usvars[usvars_used++] = var;
}

__attribute__((always_inline)) inline static void unset_envvars(void) {
  for (size_t i = 0; i < usvars_used; ++i) {
    devmsg("unset:    %s\n", usvars[i]);

    if (unsetenv(usvars[i]))
      die(EXIT_CANCELED, errno, _("cannot unset %s"), quote(usvars[i]));
  }
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE valid_escape_sequence(const char c) {
  return (c == 'c' || c == 'f' || c == 'n' || c == 'r' || c == 't' ||
          c == 'v' || c == '#' || c == '$' || c == '_' || c == '"' ||
          c == '\'' || c == '\\');
}

__attribute__((always_inline)) inline static char _GL_ATTRIBUTE_PURE escape_char(const char c) {
  switch (c) {
...
__attribute__((always_inline)) inline static const char *_GL_ATTRIBUTE_PURE scan_varname(const char *str) {
  assert(str && *str == '$'); /* LCOV_EXCL_LINE */
  if (*(str + 1) == '{' && (c_isalpha(*(str + 2)) || *(str + 2) == '_')) {
    const char *end = str + 3;
    while (c_isalnum(*end) || *end == '_') ++end;
    if (*end == '}') return end;
  }
...
__attribute__((always_inline)) inline static char *extract_varname(const char *str) {
  ptrdiff_t i;
  const char *p;
...
__attribute__((always_inline)) inline static void validate_split_str(const char *str, size_t * /*out*/ bufsize,
                               int * /*out*/ maxargc) {
  bool dq, sq, sp;
  const char *pch;
  size_t buflen;
  ...
  while (*str) {
    ...
    switch (*str) {
      ...
      case '\\':
        ...
        if (next == '\0')
          die(EXIT_CANCELED, 0, _("invalid backslash at end of string in -S"));
      ...
      case '$':
        ...
        if (!(pch = extract_varname(str)))
          die(EXIT_CANCELED, 0,
              _("only ${VARNAME} expansion is supported,"
                " error at: %s"),
              str);

        if ((pch = getenv(pch))) buflen += strlen(pch);
        break;
    }
    ++str;
  }

  if (dq || sq) die(EXIT_CANCELED, 0, _("no terminating quote in -S string"));
...
/* Return a newly-allocated *arg[]-like array,
   ...
   To free allocated memory:
     free (argv[0]);
     free (argv); */
__attribute__((always_inline)) inline static char **build_argv(const char *str, int extra_argc) {
...
__attribute__((always_inline)) inline static void parse_split_string(const char *str, int /*out*/ *orig_optind,
                               int /*out*/ *orig_argc,
                               char *** /*out*/ orig_argv) {
  int i, newargc;
  char **newargv, **nextargv;
  ...
  *newargv = (*orig_argv)[0];
  ...
  if (dev_debug && *nextargv) {
    devmsg("split -S:  %s\n", quote(str));
    devmsg(" into:    %s\n", quote(*nextargv++));
    while (*nextargv) devmsg("     &    %s\n", quote(*nextargv++));
  } else {
    /* Ensure nextargv points to the last argument */
    while (*nextargv) ++nextargv;
  }

  /* Add remaining arguments from original command line */
  for (i = *orig_optind; i < *orig_argc; ++i) *nextargv++ = (*orig_argv)[i];
  *nextargv = NULL;

  /* Count how many new arguments we have */
  newargc = 0;
  for (nextargv = newargv; *nextargv; ++nextargv) ++newargc;
...
__attribute__((always_inline)) inline static void parse_signal_action_params(const char *optarg, bool set_default) {
  ...
  while (opt_sig) {
    ...
    if (signum == 0) error(0, 0, _("%s: invalid signal"), quote(opt_sig));
    if (signum <= 0) usage(exit_failure);
...
__attribute__((always_inline)) inline static void reset_signal_handlers(void) {
  for (int i = 1; i <= SIGNUM_BOUND; i++) {
    struct sigaction act;

    if (signals[i] == UNCHANGED) continue;
    ...
    if (sig_err && !ignore_errors)
      die(EXIT_CANCELED, errno, _("failed to get signal action for signal %d"),
          i);
...
__attribute__((always_inline)) inline static void parse_block_signal_params(const char *optarg, bool block) {
  char signame[SIG2STR_MAX];
  char *opt_sig;
  char *optarg_writable;
...
__attribute__((always_inline)) inline static void set_signal_proc_mask(void) {
...
__attribute__((always_inline)) inline static void list_signal_handling(void) {
  sigset_t set;
  char signame[SIG2STR_MAX];
  ...
  for (int i = 1; i <= SIGNUM_BOUND; i++) {
    struct sigaction act;
    if (sigaction(i, NULL, &act)) continue;
...
__attribute__((always_inline)) inline static void initialize_signals(void) {
  signals = xmalloc((sizeof *signals) * (SIGNUM_BOUND + 1));

  for (int i = 0; i <= SIGNUM_BOUND; i++) signals[i] = UNCHANGED;
...
int main(int argc, char **argv) {
  ...
  while (optind < argc && (eq = strchr(argv[optind], '='))) {
    ...
    if (putenv(argv[optind])) {
      *eq = '\0';
      die(EXIT_CANCELED, errno, _("cannot set %s"), quote(argv[optind]));
    }
    optind++;
  }
  ...
  if (!program_specified) {
    ...
    while (*e) printf("%s%c", *e++, opt_nul_terminate_output ? '\0' : '\n');
    return EXIT_SUCCESS;
  }

  reset_signal_handlers();
  if (sig_mask_changed) set_signal_proc_mask();

  if (report_signal_handling) list_signal_handling();
