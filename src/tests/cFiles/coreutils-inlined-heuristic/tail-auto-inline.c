...
__attribute__((always_inline)) inline static void ATTRIBUTE_NORETURN die_pipe(void) {
  raise(SIGPIPE);
  exit(EXIT_FAILURE);
}
...
__attribute__((always_inline)) inline static void check_output_alive(void) {
  if (!monitor_output) return;
  ...
  if (poll(&pfd, 1, 0) >= 0 && (pfd.revents & POLLERR)) die_pipe();
#else
  ...
  if (select(STDOUT_FILENO + 1, &rfd, NULL, NULL, &delay) == 1) die_pipe();
#endif
}

__attribute__((always_inline)) inline static bool valid_file_spec(struct File_spec const *f) {
  /* Exactly one of the following subexpressions must be true. */
  return ((f->fd == -1) ^ (f->errnum == 0));
}

__attribute__((always_inline)) inline static char const *pretty_name(struct File_spec const *f) {
...
__attribute__((always_inline)) inline static void record_open_fd(struct File_spec *f, int fd, off_t size,
...
__attribute__((always_inline)) inline static void close_fd(int fd, const char *filename) {
  if (fd != -1 && fd != STDIN_FILENO && close(fd)) {
    error(0, errno, _("closing %s (fd=%d)"), quoteaf(filename), fd);
  }
}

__attribute__((always_inline)) inline static void write_header(const char *pretty_filename) {
...
__attribute__((always_inline)) inline static void xwrite_stdout(char const *buffer, size_t n_bytes) {
  if (n_bytes > 0 && fwrite(buffer, 1, n_bytes, stdout) < n_bytes) {
    clearerr(stdout); /* To avoid redundant close_stdout diagnostic.  */
    die(EXIT_FAILURE, errno, _("error writing %s"), quoteaf("standard output"));
  }
}
...
__attribute__((always_inline)) inline static uintmax_t dump_remainder(bool want_header, const char *pretty_filename,
  ...
  while (1) {
    ...
    if (n_bytes != COPY_TO_EOF) {
      ...
      if (n_remaining == 0 || n_bytes == COPY_A_BUFFER) break;
    }
  }
...
__attribute__((always_inline)) inline static off_t xlseek(int fd, off_t offset, int whence, char const *filename) {
  off_t new_offset = lseek(fd, offset, whence);
  char buf[INT_BUFSIZE_BOUND(offset)];
  char *s;

  if (0 <= new_offset) return new_offset;
...
__attribute__((always_inline)) inline static bool file_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       off_t start_pos, off_t end_pos, uintmax_t *read_pos) {
  char buffer[BUFSIZ];
  size_t bytes_read;
  off_t pos = end_pos;

  if (n_lines == 0) return true;
  ...
  *read_pos = pos + bytes_read;
  ...
  if (bytes_read && buffer[bytes_read - 1] != line_end) --n_lines;
  ...
  do {
    ...
    while (n) {
      ...
      if (n_lines-- == 0) {
        ...
        if (n != bytes_read - 1) xwrite_stdout(nl + 1, bytes_read - (n + 1));
    ...
    if (pos == start_pos) {
      ...
      *read_pos =
          start_pos + dump_remainder(false, pretty_filename, fd, end_pos);
      return true;
    }
    pos -= BUFSIZ;
    xlseek(fd, pos, SEEK_SET, pretty_filename);
...
__attribute__((always_inline)) inline static bool pipe_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  struct linebuffer {
    char buffer[BUFSIZ];
    size_t nbytes;
  ...
  while (1) {
    ...
    if (n_read == 0 || n_read == SAFE_READ_ERROR) break;
    tmp->nbytes = n_read;
    *read_pos += n_read;
    tmp->nlines = 0;
    tmp->next = NULL;
...
__attribute__((always_inline)) inline static bool pipe_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  struct charbuffer {
    char buffer[BUFSIZ];
    size_t nbytes;
...
__attribute__((always_inline)) inline static int start_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  char buffer[BUFSIZ];
  ...
  while (0 < n_bytes) {
    ...
    *read_pos += bytes_read;
    if (bytes_read <= n_bytes)
      ...
      if (n_remaining) xwrite_stdout(&buffer[n_bytes], n_remaining);
      break;
    }
  }
...
__attribute__((always_inline)) inline static int start_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  ...
  while (1) {
    ...
    *read_pos += bytes_read;
    ...
    while ((p = memchr(p, line_end, buffer_end - p))) {
      ++p;
      if (--n_lines == 0) {
        if (p < buffer_end) xwrite_stdout(p, buffer_end - p);
        return 0;
      }
    }
  }
...
__attribute__((always_inline)) inline static bool fremote(int fd, const char *name) {
...
__attribute__((always_inline)) inline static void recheck(struct File_spec *f, bool blocking) {
...
__attribute__((always_inline)) inline static bool any_live_files(const struct File_spec *f, size_t n_files) {
  ...
  if (reopen_inaccessible_files && follow_mode == Follow_name) return true;

  for (size_t i = 0; i < n_files; i++) {
    if (0 <= f[i].fd)
      return true;
    else {
      if (!f[i].ignore && reopen_inaccessible_files) return true;
    }
  }
...
__attribute__((always_inline)) inline static void tail_forever(struct File_spec *f, size_t n_files,
  ...
  while (1) {
    ...
    for (i = 0; i < n_files; i++) {
      ...
      if (f[i].blocking != blocking) {
        ...
        if (old_flags < 0 ||
          ...
          if (S_ISREG(f[i].mode) && errno == EPERM) {
            ...
            die(EXIT_FAILURE, errno, _("%s: cannot change nonblocking mode"),
                quotef(name));
        } else
      ...
      if (!f[i].blocking) {
        ...
        if (i != last) {
          if (print_headers) write_header(name);
          last = i;
        }
      }
...
__attribute__((always_inline)) inline static bool any_remote_file(const struct File_spec *f, size_t n_files) {
  for (size_t i = 0; i < n_files; i++)
    if (0 <= f[i].fd && f[i].remote) return true;
  return false;
}
...
__attribute__((always_inline)) inline static bool any_non_remote_file(const struct File_spec *f, size_t n_files) {
  for (size_t i = 0; i < n_files; i++)
    if (0 <= f[i].fd && !f[i].remote) return true;
  return false;
}
...
__attribute__((always_inline)) inline static bool any_symlinks(const struct File_spec *f, size_t n_files) {
  struct stat st;
  for (size_t i = 0; i < n_files; i++)
    if (lstat(f[i].name, &st) == 0 && S_ISLNK(st.st_mode)) return true;
  return false;
}
...
__attribute__((always_inline)) inline static bool any_non_regular_fifo(const struct File_spec *f, size_t n_files) {
  for (size_t i = 0; i < n_files; i++)
...
__attribute__((always_inline)) inline static bool tailable_stdin(const struct File_spec *f, size_t n_files) {
  for (size_t i = 0; i < n_files; i++)
    if (!f[i].ignore && STREQ(f[i].name, "-")) return true;
  return false;
}

__attribute__((always_inline)) inline static size_t wd_hasher(const void *entry, size_t tabsize) {
  const struct File_spec *spec = entry;
  return spec->wd % tabsize;
}

__attribute__((always_inline)) inline static bool wd_comparator(const void *e1, const void *e2) {
...
__attribute__((always_inline)) inline static void check_fspec(struct File_spec *fspec,
                        struct File_spec **prev_fspec) {
  struct stat stats;
  char const *name;

  if (fspec->fd == -1) return;
  ...
  uintmax_t bytes_read =
      dump_remainder(want_header, name, fspec->fd, COPY_TO_EOF);
  fspec->size += bytes_read;

  if (bytes_read) {
    *prev_fspec = fspec;
    if (fflush(stdout) != 0) die(EXIT_FAILURE, errno, _("write error"));
  }
}
...
__attribute__((always_inline)) inline static bool tail_forever_inotify(int wd, struct File_spec *f, size_t n_files,
                                 double sleep_interval) {
#if TAIL_TEST_SLEEP
  ...
  for (i = 0; i < n_files; i++) {
    if (!f[i].ignore) {
      ...
      if (hash_insert(wd_to_name, &(f[i])) == NULL) xalloc_die();
  ...
  if (follow_mode == Follow_descriptor && !found_watchable_file) return false;
  ...
  while (1) {
    ...
    while (len <= evbuf_off) {
      ...
      if (pid) {
        if (writer_is_dead) exit(EXIT_SUCCESS);
      ...
      FD_ZERO(&rfd);
      FD_SET(wd, &rfd);
      if (monitor_output) FD_SET(STDOUT_FILENO, &rfd);

      int file_change = select(MAX(wd, STDOUT_FILENO) + 1, &rfd, NULL, NULL,
                               pid ? &delay : NULL);
    ...
    if ((ev->mask & IN_DELETE_SELF) && !ev->len) {
      for (i = 0; i < n_files; i++) {
        if (ev->wd == f[i].parent_wd) {
          hash_free(wd_to_name);
          error(0, 0, _("directory containing watched file was removed"));
    ...
    if (ev->len) /* event on ev->name in watched directory.  */
    {
      ...
      if (new_watch) {
        ...
        if (prev && prev != fspec) {
          if (follow_mode == Follow_name) recheck(prev, false);
          prev->wd = -1;
          close_fd(prev->fd, pretty_name(prev));
        }

        if (hash_insert(wd_to_name, fspec) == NULL) xalloc_die();
      }

      if (follow_mode == Follow_name) recheck(fspec, false);
    } else {
...
__attribute__((always_inline)) inline static bool tail_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  struct stat stats;
...
__attribute__((always_inline)) inline static bool tail_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  struct stat stats;
    ...
    *read_pos += dump_remainder(false, pretty_filename, fd, COPY_TO_EOF);
    ...
    if (!presume_input_pipe && S_ISREG(stats.st_mode) &&
      ...
      *read_pos = end_pos;
      ...
      if (start_pos != -1) xlseek(fd, start_pos, SEEK_SET, pretty_filename);
...
__attribute__((always_inline)) inline static bool tail(const char *filename, int fd, uintmax_t n_units,
                 uintmax_t *read_pos) {
  *read_pos = 0;
  if (count_lines)
...
__attribute__((always_inline)) inline static bool tail_file(struct File_spec *f, uintmax_t n_units) {
  int fd;
  bool ok;
  ...
  if (fd == -1) {
    ...
    if (print_headers) write_header(pretty_name(f));
...
__attribute__((always_inline)) inline static bool parse_obsolete_option(int argc, char *const *argv,
                                  uintmax_t *n_units) {
  const char *p;
  const char *n_string;
  ...
  switch (*p++) {
    ...
    case '+':
      ...
      if (!traditional_usage) return false;
    ...
    case '-':
      ...
      if (!obsolete_usage && !p[p[0] == 'c']) return false;
  ...
  if (n_string == n_string_end)
    *n_units = default_count;
    ...
    die(EXIT_FAILURE, errno, "%s: %s", _("invalid number"), quote(argv[1]));
  }
...
__attribute__((always_inline)) inline static void parse_options(int argc, char **argv, uintmax_t *n_units,
    ...
    switch (c) {
      ...
      case LONG_FOLLOW_OPTION:
        ...
        if (optarg == NULL)
          ...
          follow_mode = XARGMATCH("--follow", optarg, follow_mode_string,
                                  follow_mode_map);
        break;
      ...
      case 's': {
        ...
        if (!(xstrtod(optarg, NULL, &s, cl_strtod) && 0 <= s))
          die(EXIT_FAILURE, 0, _("invalid number of seconds: %s"),
              quote(optarg));
        *sleep_interval = s;
      } break;
  ...
  if (reopen_inaccessible_files) {
    if (!forever) {
      ...
      error(0, 0,
            _("warning: --retry ignored; --retry is useful"
              " only when following"));
...
__attribute__((always_inline)) inline static size_t ignore_fifo_and_pipe(struct File_spec *f, size_t n_files) {
...
int main(int argc, char **argv) {
    ...
    if (forever && found_hyphen) {
      ...
      blocking_stdin =
          (pid == 0 && follow_mode == Follow_descriptor && n_files == 1 &&
           !fstat(STDIN_FILENO, &in_stat) && !S_ISREG(in_stat.st_mode));

      if (!blocking_stdin && isatty(STDIN_FILENO))
        error(0, 0,
              _("warning: following standard input"
  ...
  if (forever && ignore_fifo_and_pipe(F, n_files)) {
    ...
    if (!disable_inotify) {
      ...
      for (i = 0; i < n_files; i++) {
        ...
        if (F[i].wd != -1) inotify_rm_watch(wd, F[i].wd);
        if (F[i].parent_wd != -1) inotify_rm_watch(wd, F[i].parent_wd);
      }
    }
#endif
    disable_inotify = true;
  ...
  if (have_read_stdin && close(STDIN_FILENO) < 0) die(EXIT_FAILURE, errno, "-");
