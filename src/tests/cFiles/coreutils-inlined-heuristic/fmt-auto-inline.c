...
int main(int argc, char **argv) {
  ...
  while ((optchar = getopt_long(
              argc, argv, "0123456789cstuw:p:g:", long_options, NULL)) != -1)
    switch (optchar) {
      default:
        if (ISDIGIT(optchar))
 ...
 only when it is the first\noption; use -w N instead"),
  ...
  if (goal_width_option) {
    ...
    if (max_width_option == NULL) max_width = goal_width + 10;
  } else {
...
__attribute__((always_inline)) inline static void set_other_indent(bool same_paragraph) {
  if (split)
...
__attribute__((always_inline)) inline static bool get_paragraph(FILE *f) {
  ...
  if (split) {
    ...
    while (same_para(c) && in_column == other_indent) c = get_line(f, c);
  }
...
__attribute__((always_inline)) inline static int copy_rest(FILE *f, int c) {
  ...
  if (in_column > next_prefix_indent || (c != '\n' && c != EOF)) {
    ...
    for (s = prefix; out_column != in_column && *s; out_column++) putchar(*s++);
...
__attribute__((always_inline)) inline static void flush_paragraph(void) {
  ...
  for (w = split_point; w <= word_limit; w++) w->text -= shift;
...
__attribute__((always_inline)) inline static void put_word(WORD *w) {
  ...
  s = w->text;
  for (n = w->length; n != 0; n--) putchar(*s++);
  out_column += w->length;
}
