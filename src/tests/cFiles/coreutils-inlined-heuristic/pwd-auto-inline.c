...
__attribute__((always_inline)) inline static void file_name_free(struct file_name *p) {
  free(p->buf);
  free(p);
}

__attribute__((always_inline)) inline static struct file_name *file_name_init(void) {
...
__attribute__((always_inline)) inline static void file_name_prepend(struct file_name *p, char const *s,
                              size_t s_len) {
...
__attribute__((always_inline)) inline static char *nth_parent(size_t n) {
...
__attribute__((always_inline)) inline static void find_dir_entry(struct stat *dot_sb, struct file_name *file_name,
...
__attribute__((always_inline)) inline static void robust_getcwd(struct file_name *file_name) {
  size_t height = 1;
  ...
  while (1) {
    ...
    if (SAME_INODE(dot_sb, *root_dev_ino)) break;
...
__attribute__((always_inline)) inline static char *logical_getcwd(void) {
  struct stat st1;
  struct stat st2;
  char *wd = getenv("PWD");
  char *p;
