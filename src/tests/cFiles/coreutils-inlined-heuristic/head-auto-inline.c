...
__attribute__((always_inline)) inline static void diagnose_copy_fd_failure(enum Copy_fd_status err,
                                     char const *filename) {
  switch (err) {
...
__attribute__((always_inline)) inline static void write_header(const char *filename) {
...
__attribute__((always_inline)) inline static enum Copy_fd_status copy_fd(int src_fd, uintmax_t n_bytes) {
  char buf[BUFSIZ];
  ...
  while (0 < n_bytes) {
    ...
    if (n_read == SAFE_READ_ERROR) return COPY_FD_READ_ERROR;

    n_bytes -= n_read;

    if (n_read == 0 && n_bytes != 0) return COPY_FD_UNEXPECTED_EOF;
...
__attribute__((always_inline)) inline static off_t elseek(int fd, off_t offset, int whence, char const *filename) {
...
__attribute__((always_inline)) inline static bool elide_tail_bytes_pipe(const char *filename, int fd,
...
#define HEAD_TAIL_PIPE_BYTECOUNT_THRESHOLD 1024 * 1024
  ...
  if (n_elide <= HEAD_TAIL_PIPE_BYTECOUNT_THRESHOLD) {
    ...
    for (i = 0, i_next = 1; !eof; i = i_next, i_next = (i_next + 1) % n_bufs) {
      ...
      if (i + 1 == n_bufs) buffered_enough = true;
...
__attribute__((always_inline)) inline static bool elide_tail_bytes_file(const char *filename, int fd,
  ...
  if (presume_input_pipe || current_pos < 0 || size <= ST_BLKSIZE(*st))
    ...
    if (bytes_remaining <= n_elide) return true;

    enum Copy_fd_status err = copy_fd(fd, bytes_remaining - n_elide);
    if (err == COPY_FD_OK) return true;
...
__attribute__((always_inline)) inline static bool elide_tail_lines_pipe(const char *filename, int fd,
...
__attribute__((always_inline)) inline static bool elide_tail_lines_seekable(const char *pretty_filename, int fd,
  ...
  pos -= bytes_read;
  if (elseek(fd, pos, SEEK_SET, pretty_filename) < 0) return false;
  ...
  if (n_lines && bytes_read && buffer[bytes_read - 1] != line_end) --n_lines;
...
__attribute__((always_inline)) inline static bool elide_tail_lines_file(const char *filename, int fd,
                                  uintmax_t n_elide, struct stat const *st,
                                  off_t current_pos) {
  off_t size = st->st_size;
  if (presume_input_pipe || current_pos < 0 || size <= ST_BLKSIZE(*st))
...
__attribute__((always_inline)) inline static bool head_bytes(const char *filename, int fd, uintmax_t bytes_to_write) {
  char buffer[BUFSIZ];
  size_t bytes_to_read = BUFSIZ;

  while (bytes_to_write) {
    size_t bytes_read;
    if (bytes_to_write < bytes_to_read) bytes_to_read = bytes_to_write;
...
__attribute__((always_inline)) inline static bool head_lines(const char *filename, int fd, uintmax_t lines_to_write) {
  char buffer[BUFSIZ];
...
__attribute__((always_inline)) inline static bool head(const char *filename, int fd, uintmax_t n_units,
                 bool count_lines, bool elide_from_end) {
  if (print_headers) write_header(filename);
...
__attribute__((always_inline)) inline static bool head_file(const char *filename, uintmax_t n_units, bool count_lines,
                      bool elide_from_end) {
  int fd;
  bool ok;
...
__attribute__((always_inline)) inline static uintmax_t string_to_integer(bool count_lines, const char *n_string) {
  return xdectoumax(
      n_string, 0, UINTMAX_MAX, "bkKmMGTPEZY0",
      count_lines ? _("invalid number of lines") : _("invalid number of bytes"),
      0);
}
...
int main(int argc, char **argv) {
  ...
  if (1 < argc && argv[1][0] == '-' && ISDIGIT(argv[1][1])) {
    ...
    *end_n_string = multiplier_char;
    if (multiplier_char) *(++end_n_string) = 0;
  ...
  while ((c = getopt_long(argc, argv, "c:n:qvz0123456789", long_options,
    ...
    switch (c) {
      ...
      default:
        if (ISDIGIT(c)) error(0, 0, _("invalid trailing option -- %c"), c);
        usage(EXIT_FAILURE);
    }
  }
