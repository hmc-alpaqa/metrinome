...
__attribute__((always_inline)) inline static void unexpand(void) {
  /* Input stream.  */
  ...
  while (true) {
    ...
    do {
      while ((c = getc(fp)) < 0 && (fp = next_file(fp))) continue;
      ...
      if (convert) {
        ...
        if (blank) {
          ...
          column++;
          if (!column) die(EXIT_FAILURE, 0, _("input line is too long"));
        }

        if (pending) {
          if (pending > 1 && one_blank_before_tab_stop) pending_blank[0] = '\t';
      ...
      if (putchar(c) < 0) die(EXIT_FAILURE, errno, _("write error"));
    } while (c != '\n');
  }
}
...
int main(int argc, char **argv) {
  ...
  if (convert_first_only) convert_entire_line = false;
