...
__attribute__((always_inline)) inline static void output(const char *start, const char *past_end) {
...
__attribute__((always_inline)) inline static bool tac_seekable(int input_fd, const char *file, off_t file_pos) {
  ...
  while (saved_record_size == read_size) {
    ...
    if (saved_record_size == SAFE_READ_ERROR) break;
    file_pos += nread;
  }
  ...
  if (sentinel_length) match_start -= match_length1;
  ...
  while (true) {
    ...
    if (sentinel_length == 0) {
      ...
      if (1 < range) die(EXIT_FAILURE, 0, _("record too large"));

      if (range == 1 || ((ret = re_search(&compiled_separator, G_buffer, i,
                                          i - 1, range, &regs)) == -1))
        match_start = G_buffer - 1;
      else if (ret == -2) {
        die(EXIT_FAILURE, 0, _("error in regular expression search"));
      } else {
    ...
    if (match_start < G_buffer) {
      ...
      if (saved_record_size > read_size) {
        ...
        if (G_buffer_size < old_G_buffer_size) xalloc_die();
      ...
      if (separator_ends_record) {
        ...
        if (!first_time || match_end != past_end) output(match_end, past_end);
        past_end = match_end;
        first_time = false;
      } else {
      ...
      if (sentinel_length > 0) match_start -= match_length - 1;
    }
  }
}
...
__attribute__((always_inline)) inline static void unlink_tempfile(void) {
  fclose(fp_to_close);
  unlink(file_to_remove);
}

__attribute__((always_inline)) inline static void record_or_unlink_tempfile(char const *fn, FILE *fp) {
  if (!file_to_remove) {
    file_to_remove = fn;
    fp_to_close = fp;
...
__attribute__((always_inline)) inline static void record_or_unlink_tempfile(char const *fn, FILE *fp _GL_UNUSED) {
  unlink(fn);
}
...
__attribute__((always_inline)) inline static bool temp_stream(FILE **fp, char **file_name) {
...
__attribute__((always_inline)) inline static off_t copy_to_temp(FILE **g_tmp, char **g_tempfile, int input_fd,
                          char const *file) {
  FILE *fp;
  char *file_name;
  uintmax_t bytes_copied = 0;
  if (!temp_stream(&fp, &file_name)) return -1;
...
__attribute__((always_inline)) inline static bool tac_nonseekable(int input_fd, const char *file) {
  FILE *tmp_stream;
  char *tmp_file;
  off_t bytes_copied = copy_to_temp(&tmp_stream, &tmp_file, input_fd, file);
  if (bytes_copied < 0) return false;
...
__attribute__((always_inline)) inline static bool tac_file(const char *filename) {
  bool ok;
  off_t file_size;
  int fd;
...
int main(int argc, char **argv) {
  ...
  if (sentinel_length == 0) {
    if (*separator == 0) die(EXIT_FAILURE, 0, _("separator cannot be empty"));
  ...
  {
    ok = true;
    for (size_t i = 0; file[i]; ++i) ok &= tac_file(file[i]);
  }
