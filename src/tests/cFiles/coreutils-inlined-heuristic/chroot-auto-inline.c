...
__attribute__((always_inline)) static inline bool uid_unset(uid_t uid) { return uid == (uid_t)-1; }
__attribute__((always_inline)) static inline bool gid_unset(gid_t gid) { return gid == (gid_t)-1; }
...
__attribute__((always_inline)) inline static int setgroups(size_t size, gid_t const *list _GL_UNUSED) {
  if (size == 0) {
...
__attribute__((always_inline)) inline static int parse_additional_groups(char const *groups, GETGROUPS_T **pgids,
  ...
  for (tmp = strtok(buffer, ","); tmp; tmp = strtok(NULL, ",")) {
    ...
    if (xstrtoumax(tmp, NULL, 10, &value, "") == LONGINT_OK &&
        value <= MAXGID) {
      while (isspace(to_uchar(*tmp))) tmp++;
      if (*tmp != '+') {
        /* Handle the case where the name is numeric.  */
        g = getgrnam(tmp);
        if (g != NULL) value = g->gr_gid;
      }
    ...
    if (n_gids == n_gids_allocated) gids = X2NREALLOC(gids, &n_gids_allocated);
    gids[n_gids++] = value;
  }

  if (ret == 0 && n_gids == 0) {
    if (show_errors) error(0, 0, _("invalid group list %s"), quote(groups));
    ret = -1;
  }

  *pgids = gids;

  if (ret == 0) *pn_gids = n_gids;
...
__attribute__((always_inline)) inline static bool is_root(const char *dir) {
...
int main(int argc, char **argv) {
  ...
  if (!is_oldroot) {
    ...
    if (uid_set(uid) && (!groups || gid_unset(gid))) {
      ...
      if ((pwd = getpwuid(uid))) {
        if (gid_unset(gid)) gid = pwd->pw_gid;
  ...
  if (argc == optind + 1) {
    ...
    if (shell == NULL) shell = bad_cast("/bin/sh");
    argv[0] = shell;
    argv[1] = bad_cast("-i");
    argv[2] = NULL;
  } else {
  ...
  if (uid_set(uid) && (!groups || gid_unset(gid))) {
    ...
    if ((pwd = getpwuid(uid))) {
      ...
      die(EXIT_CANCELED, errno, _("no group specified for unknown uid: %d"),
          (int)uid);
    }
  }
  ...
  if (groups && *groups) {
    if (parse_additional_groups(groups, &in_gids, &n_gids, !n_gids) != 0) {
      if (!n_gids) return EXIT_CANCELED;
    ...
    if (ngroups <= 0) {
      if (!n_gids)
        die(EXIT_CANCELED, errno, _("failed to get supplemental groups"));
      /* else look-up outside the chroot worked, then go with those.  */
    } else {
      n_gids = ngroups;
      gids = in_gids;
