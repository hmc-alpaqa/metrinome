...
__attribute__((always_inline)) inline static void settimeout(double duration, bool warn) {
  ...
  if (timer_create(CLOCK_REALTIME, NULL, &timerid) == 0) {
    if (timer_settime(timerid, 0, &its, NULL) == 0)
      ...
      if (warn) error(0, errno, _("warning: timer_settime"));
...
__attribute__((always_inline)) inline static int send_sig(pid_t where, int sig) {
  ...
  if (where == 0) signal(sig, SIG_IGN);
  return kill(where, sig);
}

/* Signal handler which is required for sigsuspend() to be interrupted
   whenever SIGCHLD is received.  */
__attribute__((always_inline)) inline static void chld(int sig) {}

__attribute__((always_inline)) inline static void cleanup(int sig) {
  if (sig == SIGALRM) {
    timed_out = 1;
    sig = term_signal;
  }
  if (monitored_pid) {
    ...
    if (verbose) {
      ...
      error(0, 0, _("sending signal %s to command %s"), signame,
            quote(command));
    }
...
__attribute__((always_inline)) inline static bool apply_time_suffix(double *x, char suffix_char) {
  int multiplier;
...
__attribute__((always_inline)) inline static double parse_duration(const char *str) {
  double duration;
  const char *ep;
...
__attribute__((always_inline)) inline static void unblock_signal(int sig) {
...
__attribute__((always_inline)) inline static void install_sigchld(void) {
  struct sigaction sa;
...
__attribute__((always_inline)) inline static void install_cleanup(int sigterm) {
  struct sigaction sa;
  sigemptyset(&sa.sa_mask); /* Allow concurrent calls to handler */
...
__attribute__((always_inline)) inline static void block_cleanup_and_chld(int sigterm, sigset_t *old_set) {
  sigset_t block_set;
...
__attribute__((always_inline)) inline static bool disable_core_dumps(void) {
#if HAVE_PRCTL && defined PR_SET_DUMPABLE
  if (prctl(PR_SET_DUMPABLE, 0) == 0) return true;
  ...
  if (setrlimit(RLIMIT_CORE, &(struct rlimit){0, 0}) == 0) return true;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "+k:s:v", long_options, NULL)) != -1) {
    switch (c) {
      ...
      case 's':
        ...
        if (term_signal == -1) usage(EXIT_CANCELED);
        break;
  ...
  if (argc - optind < 2) usage(EXIT_CANCELED);
  ...
  if (monitored_pid == -1) {
    ...
    if (timed_out && !preserve_status) status = EXIT_TIMEDOUT;
    return status;
  }
}
