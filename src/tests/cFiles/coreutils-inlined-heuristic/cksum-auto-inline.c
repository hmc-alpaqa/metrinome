...
__attribute__((always_inline)) inline static void fill_r(void) {
  r[0] = GEN;
  for (int i = 1; i < 8; i++)
...
__attribute__((always_inline)) inline static bool cksum(const char *file, bool print_name) {
  ...
  if (ferror(stdout)) die(EXIT_FAILURE, errno, "-: %s", _("write error"));
...
int main(int argc, char **argv) {
  ...
  if (optind == argc)
    ...
    ok = true;
    for (i = optind; i < argc; i++) ok &= cksum(argv[i], true);
  }
