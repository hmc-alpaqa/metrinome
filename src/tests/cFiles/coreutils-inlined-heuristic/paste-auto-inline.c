...
__attribute__((always_inline)) inline static int collapse_escapes(char const *strptr) {
...
__attribute__((always_inline)) static inline void xputchar(char c) {
  if (putchar(c) < 0) write_error();
}
...
__attribute__((always_inline)) inline static bool paste_parallel(size_t nfiles, char **fnamptr) {
  bool ok = true;
    ...
    for (size_t i = 0; i < nfiles && files_open; i++) {
      ...
      if (!sometodo) {
        ...
        if (i + 1 == nfiles) {
          ...
          if (*delimptr != EMPTY_DELIM) delbuf[delims_saved++] = *delimptr;
          if (++delimptr == delim_end) delimptr = delims;
        }
      } else {
        /* Some data read. */
        somedone = true;
        ...
        if (i + 1 != nfiles) {
          if (chr != line_delim && chr != EOF) xputchar(chr);
          if (*delimptr != EMPTY_DELIM) xputchar(*delimptr);
          if (++delimptr == delim_end) delimptr = delims;
        } else {
...
__attribute__((always_inline)) inline static bool paste_serial(size_t nfiles, char **fnamptr) {
  ...
  for (; nfiles; nfiles--, fnamptr++) {
    ...
    if (charold != line_delim) xputchar(line_delim);
