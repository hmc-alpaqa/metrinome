...
__attribute__((always_inline)) inline static void apply_settings(bool checking, const char *device_name,
    ...
    if (!match_found || not_set_attr) {
      ...
      /* This is the BSD interface to "extproc".
        ...
        if (mode->c_line != value)
          error(0, 0, _("invalid line discipline %s"), quote(settings[k]));
        *require_set_attr = true;
      }
#endif
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc - argi, argv + argi, "-agF:", longopts,
    ...
    while (opti < optind) argv[argi + opti++] = NULL;
  }
  ...
  if (!noargs && !verbose_output && !recoverable_output) {
    ...
    apply_settings(/* checking= */ true, device_name, argv, argc, &check_mode,
                   &speed_was_set, &require_set_attr);
  }
  ...
  apply_settings(/* checking= */ false, device_name, argv, argc, &mode,
                 &speed_was_set, &require_set_attr);
...
__attribute__((always_inline)) inline static void set_speed(enum speed_setting type, const char *arg,
  ...
  if (type == input_speed || type == both_speeds) cfsetispeed(mode, baud);
  if (type == output_speed || type == both_speeds) cfsetospeed(mode, baud);
}

#ifdef TIOCGWINSZ

__attribute__((always_inline)) inline static int get_win_size(int fd, struct winsize *win) {
...
__attribute__((always_inline)) inline static void set_window_size(int rows, int cols, char const *device_name) {
  ...
  if (get_win_size(STDIN_FILENO, &win)) {
    if (errno != EINVAL) die(EXIT_FAILURE, errno, "%s", quotef(device_name));
    memset(&win, 0, sizeof(win));
  }

  if (rows >= 0) win.ws_row = rows;
  if (cols >= 0) win.ws_col = cols;
...
__attribute__((always_inline)) inline static void display_window_size(bool fancy, char const *device_name) {
  ...
  if (get_win_size(STDIN_FILENO, &win)) {
    if (errno != EINVAL) die(EXIT_FAILURE, errno, "%s", quotef(device_name));
    if (!fancy)
      die(EXIT_FAILURE, 0, _("%s: no size information for this device"),
          quotef(device_name));
  } else {
    wrapf(fancy ? "rows %d; columns %d;" : "%d %d\n", win.ws_row, win.ws_col);
    if (!fancy) current_col = 0;
  }
}
#endif
...
__attribute__((always_inline)) inline static tcflag_t *_GL_ATTRIBUTE_PURE mode_type_flag(enum mode_type type,
                                                   struct termios *mode) {
  switch (type) {
    case control:
      return &mode->c_cflag;
...
__attribute__((always_inline)) inline static void display_changed(struct termios *mode) {
  ...
  for (i = 0; !STREQ(control_info[i].name, "min"); ++i) {
    if (mode->c_cc[control_info[i].offset] == control_info[i].saneval) continue;

#ifdef VFLUSHO
    /* 'flush' is the deprecated equivalent of 'discard'.  */
    if (STREQ(control_info[i].name, "flush")) continue;
#endif
      /* If swtch is the same as susp, don't print both.  */
#if VSWTCH == VSUSP
    if (STREQ(control_info[i].name, "swtch")) continue;
#endif
...
__attribute__((always_inline)) inline static int strtoul_tcflag_t(char const *s, int base, char **p, tcflag_t *result,
                            char delim) {
  unsigned long ul;
  errno = 0;
  ul = strtoul(s, p, base);
  if (errno || **p != delim || *p == s || (tcflag_t)ul != ul) return -1;
  *result = ul;
  return 0;
}

/* NOTE: identical to above, modulo use of cc_t */
__attribute__((always_inline)) inline static int strtoul_cc_t(char const *s, int base, char **p, cc_t *result,
                        char delim) {
  unsigned long ul;
  errno = 0;
  ul = strtoul(s, p, base);
  if (errno || **p != delim || *p == s || (cc_t)ul != ul) return -1;
  *result = ul;
  return 0;
}
...
__attribute__((always_inline)) inline static bool recover_mode(char const *arg, struct termios *mode) {
  ...
  for (i = 0; i < 4; i++) {
    char *p;
    if (strtoul_tcflag_t(s, 16, &p, flag + i, ':') != 0) return false;
    s = p + 1;
  }
  mode->c_iflag = flag[0];
  mode->c_oflag = flag[1];
  ...
  for (i = 0; i < NCCS; ++i) {
    ...
    if (strtoul_cc_t(s, 16, &p, mode->c_cc + i, delim) != 0) return false;
    s = p + 1;
  }
...
__attribute__((always_inline)) inline static speed_t _GL_ATTRIBUTE_PURE string_to_baud(const char *arg) {
  for (int i = 0; speeds[i].string != NULL; ++i)
    if (STREQ(arg, speeds[i].string)) return speeds[i].speed;
  return (speed_t)-1;
}

__attribute__((always_inline)) inline static unsigned long int _GL_ATTRIBUTE_PURE baud_to_value(speed_t speed) {
  for (int i = 0; speeds[i].string != NULL; ++i)
    if (speed == speeds[i].speed) return speeds[i].value;
  return 0;
}
...
#if VMIN == VEOF
    if (STREQ(control_info[i].name, "min")) break;
#endif
    mode->c_cc[control_info[i].offset] = control_info[i].saneval;
  }

  for (i = 0; mode_info[i].name != NULL; ++i) {
    if (mode_info[i].flags & NO_SETATTR) continue;
...
__attribute__((always_inline)) inline static const char *visible(cc_t ch) {
  static char buf[10];
  char *bpout = buf;

  if (ch == _POSIX_VDISABLE) return "<undef>";
...
__attribute__((always_inline)) inline static unsigned long int integer_arg(const char *s, unsigned long int maxval) {
