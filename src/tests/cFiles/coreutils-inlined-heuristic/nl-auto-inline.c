...
__attribute__((always_inline)) inline static bool build_type_arg(char const **typep, struct re_pattern_buffer *regexp,
                           char *fastmap) {
  char const *errmsg;
  bool rval = true;
  ...
  switch (*optarg) {
    ...
    case 'p':
      *typep = optarg++;
      ...
      if (errmsg) die(EXIT_FAILURE, 0, "%s", (errmsg));
      break;
    default:
      rval = false;
      break;
...
__attribute__((always_inline)) inline static void print_lineno(void) {
...
__attribute__((always_inline)) inline static void proc_header(void) {
  current_type = header_type;
  current_regex = &header_regex;
  if (reset_numbers) line_no = starting_line_number;
  putchar('\n');
}

/* Switch to a body section. */

__attribute__((always_inline)) inline static void proc_body(void) {
...
__attribute__((always_inline)) inline static void proc_footer(void) {
...
__attribute__((always_inline)) inline static void proc_text(void) {
...
__attribute__((always_inline)) inline static enum section check_section(void) {
  size_t len = line_buf.length - 1;

  if (len < 2 || memcmp(line_buf.buffer, section_del, 2)) return Text;
...
__attribute__((always_inline)) inline static void process_file(FILE *fp) {
...
__attribute__((always_inline)) inline static bool nl_file(char const *file) {
  FILE *stream;
...
int main(int argc, char **argv) {
  ...
  if (optind == argc)
    ...
    for (; optind < argc; optind++) ok &= nl_file(argv[optind]);

  if (have_read_stdin && fclose(stdin) == EOF) die(EXIT_FAILURE, errno, "-");
