...
__attribute__((always_inline)) inline static operand scan_arg(const char *arg) {
  operand ret;
  ...
  while (isspace(to_uchar(*arg)) || *arg == '+') arg++;
  ...
  if (!arg[strcspn(arg, "xX")] && isfinite(ret.value)) {
    ...
    if (decimal_point) {
      ...
      if (fraction_len <= INT_MAX) ret.precision = fraction_len;
...
__attribute__((always_inline)) inline static char const *long_double_format(char const *fmt, struct layout *layout) {
  size_t i;
  size_t prefix_len = 0;
  size_t suffix_len = 0;
  ...
  if (!strchr("efgaEFGA", fmt[i]))
    die(EXIT_FAILURE, 0, _("format %s has unknown %%%c directive"), quote(fmt),
        fmt[i]);
...
__attribute__((always_inline)) inline static void ATTRIBUTE_NORETURN io_error(void) {
...
__attribute__((always_inline)) inline static void print_numbers(char const *fmt, struct layout layout,
  ...
  if (!out_of_range) {
    ...
    for (i = 1;; i++) {
      ...
      if (out_of_range) {
        ...
        char *x_str;
        int x_strlen;
        if (locale_ok) setlocale(LC_NUMERIC, "C");
        x_strlen = asprintf(&x_str, fmt, x);
        if (locale_ok) setlocale(LC_NUMERIC, "");
        if (x_strlen < 0) xalloc_die();
...
__attribute__((always_inline)) inline static char const *get_default_format(operand first, operand step,
                                      operand last) {
  ...
  if (prec != INT_MAX && last.precision != INT_MAX) {
    if (equal_width) {
      ...
      if (last.precision == 0 && prec) last_width++; /* include space for '.' */
...
__attribute__((always_inline)) inline static void incr(char **s0, size_t *s_len) {
  char *s = *s0;
  char *endp = s + *s_len - 1;
...
__attribute__((always_inline)) inline static int cmp(char const *a, size_t a_len, char const *b, size_t b_len) {
...
__attribute__((always_inline)) inline static char const *_GL_ATTRIBUTE_PURE trim_leading_zeros(char const *s) {
  char const *p = s;
  while (*s == '0') ++s;
...
__attribute__((always_inline)) inline static bool seq_fast(char const *a, char const *b, uintmax_t step) {
...
#define INITIAL_ALLOC_DIGITS 31
  ...
  if (ok) {
    ...
    while (true) {
      for (uintmax_t n_incr = step; n_incr; n_incr--) incr(&p, &p_len);

      if (!inf && 0 < cmp(p, p_len, q, q_len)) break;
      ...
      *bufp++ = *separator;
      ...
      if (buf_end - (p_len + 1) < bufp) {
        if (fwrite(buf, bufp - buf, 1, stdout) != 1) io_error();
        bufp = buf;
      }
    }
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE all_digits_p(char const *s) {
  size_t n = strlen(s);
...
int main(int argc, char **argv) {
  ...
  if (format_str != NULL && equal_width) {
    error(0, 0,
          _("format string may not be specified"
  ...
  if (all_digits_p(argv[optind]) &&
    ...
    if (seq_fast(s1, s2, step.value)) return EXIT_SUCCESS;
  ...
  if (first.precision == 0 && step.precision == 0 && last.precision == 0 &&
      ...
      0 < step.value && step.value <= SEQ_FAST_STEP_LIMIT && !equal_width &&
      !format_str && strlen(separator) == 1) {
    char *s1;
    char *s2;
    if (asprintf(&s1, "%0.Lf", first.value) < 0) xalloc_die();
