...
__attribute__((always_inline)) inline static void print_uptime(size_t n, const STRUCT_UTMP *this) {
  size_t entries = 0;
  time_t boot_time = 0;
  time_t time_now;
  time_t uptime = 0;
...
#if HAVE_SYSCTL && !defined __GLIBC__ && defined CTL_KERN && \
  ...
  while (n--) {
    ...
    if (UT_TYPE_BOOT_TIME(this)) boot_time = UT_TIME_MEMBER(this);
    ++this;
  }
#else
  (void)n;
...
#endif
  {
    if (boot_time == 0) die(EXIT_FAILURE, errno, _("couldn't get boot time"));
  ...
  if (tmn) /* TRANSLATORS: This prints the current clock time. */
  ...
  if (loads == -1)
    ...
    if (loads > 0) printf(_(",  load average: %.2f"), avg[0]);
    if (loads > 1) printf(", %.2f", avg[1]);
    if (loads > 2) printf(", %.2f", avg[2]);
    if (loads > 0) putchar('\n');
  }
}
...
__attribute__((always_inline)) inline static void uptime(const char *filename, int options) {
  size_t n_users;
