...
int main(int argc, char **argv) {
  ...
  if (default_format && opt_zero)
    die(EXIT_FAILURE, 0, _("option --zero not permitted in default format"));
...
/* Convert a uid_t to string.  Do not use this function directly.
   ...
   Beware that it returns a pointer to static storage.  */
__attribute__((always_inline)) inline static char *uidtostr_ptr(uid_t const *uid) {
  static char buf[INT_BUFSIZE_BOUND(uintmax_t)];
...
__attribute__((always_inline)) inline static void print_full_info(const char *username) {
...

  {
    ...
    if (n_groups > 0) fputs(_(" groups="), stdout);
  ...
  if (context) printf(_(" context=%s"), context);
}

/* Print information about the user based on the arguments passed. */

__attribute__((always_inline)) inline static void print_stuff(const char *pw_name) {
  if (just_user) print_user(use_real ? ruid : euid);
