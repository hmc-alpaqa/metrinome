...
#define AUTHORS                                                     \
  proper_name("Torbjorn Granlund"), proper_name("David MacKenzie"), \
...
struct field_data_t {
  ...
  const char
      *caption;      /* NULL means to use the default header of this field.  */
...
__attribute__((always_inline)) inline static void replace_control_chars(char *cell) {
  char *p = cell;
  while (*p) {
    if (c_iscntrl(to_uchar(*p))) *p = '?';
    p++;
  }
}
...
__attribute__((always_inline)) inline static void replace_invalid_chars(char *cell) {
...
__attribute__((always_inline)) inline static void replace_problematic_chars(char *cell) {
  static int tty_out = -1;
  if (tty_out < 0) tty_out = isatty(STDOUT_FILENO);
...
__attribute__((always_inline)) inline static void alloc_table_row(void) {
  nrows++;
...
__attribute__((always_inline)) inline static void print_table(void) {
  size_t row;
...
__attribute__((always_inline)) inline static void alloc_field(int f, const char *c) {
  ncolumns++;
  columns = xnrealloc(columns, ncolumns, sizeof(struct field_data_t *));
  columns[ncolumns - 1] = &field_data[f];
  if (c != NULL) columns[ncolumns - 1]->caption = c;

  if (field_data[f].used) assert(!"field used");
...
__attribute__((always_inline)) inline static void decode_output_arg(char const *arg) {
...
__attribute__((always_inline)) inline static void get_field_list(void) {
  switch (header_mode) {
    case DEFAULT_MODE:
      alloc_field(SOURCE_FIELD, NULL);
      if (print_type) alloc_field(FSTYPE_FIELD, NULL);
...
__attribute__((always_inline)) inline static void get_header(void) {
  size_t col;
  ...
  for (col = 0; col < ncolumns; col++) {
    ...
    if (columns[col]->field == SIZE_FIELD &&
      ...
      if (divisible_by_1000 < divisible_by_1024) opts |= human_base_1024;
      if (divisible_by_1024 < divisible_by_1000) opts &= ~human_base_1024;
      if (!(opts & human_base_1024)) opts |= human_B;
      ...
      if (asprintf(&cell, _("%s-%s"), num, header) == -1) cell = NULL;
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE selected_fstype(const char *fstype) {
  const struct fs_type_list *fsp;

  if (fs_select_list == NULL || fstype == NULL) return true;
  for (fsp = fs_select_list; fsp; fsp = fsp->fs_next)
    if (STREQ(fstype, fsp->fs_name)) return true;
  return false;
}

/* Is FSTYPE a type of file system that should be omitted?  */

__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE excluded_fstype(const char *fstype) {
  const struct fs_type_list *fsp;

  if (fs_exclude_list == NULL || fstype == NULL) return false;
  for (fsp = fs_exclude_list; fsp; fsp = fsp->fs_next)
    if (STREQ(fstype, fsp->fs_name)) return true;
  return false;
}

__attribute__((always_inline)) inline static size_t devlist_hash(void const *x, size_t table_size) {
  struct devlist const *p = x;
  return (uintmax_t)p->dev_num % table_size;
}

__attribute__((always_inline)) inline static bool devlist_compare(void const *x, void const *y) {
  struct devlist const *a = x;
  struct devlist const *b = y;
  return a->dev_num == b->dev_num;
}

__attribute__((always_inline)) inline static struct devlist *devlist_for_dev(dev_t dev) {
  if (devlist_table == NULL) return NULL;
  struct devlist dev_entry;
  dev_entry.dev_num = dev;
  return hash_lookup(devlist_table, &dev_entry);
}

__attribute__((always_inline)) inline static void devlist_free(void *p) { free(p); }
...
__attribute__((always_inline)) inline static void filter_mount_list(bool devices_only) {
  struct mount_entry *me;
  ...
  for (me = mount_list; me;) {
    ...
    if (discard_me) {
      me = me->me_next;
      if (!devices_only) free_mount_entry(discard_me);
    } else {
      ...
      device_list = devlist;
      if (hash_insert(devlist_table, devlist) == NULL) xalloc_die();
...
__attribute__((always_inline)) inline static struct mount_entry const *_GL_ATTRIBUTE_PURE me_for_dev(dev_t dev) {
...
__attribute__((always_inline)) inline static bool known_value(uintmax_t n) { return n < UINTMAX_MAX - 1; }
...
__attribute__((always_inline)) inline static char const *df_readable(bool negative, uintmax_t n, char *buf,
...
__attribute__((always_inline)) inline static void add_uint_with_neg_flag(uintmax_t *dest, bool *dest_neg,
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE has_uuid_suffix(char const *s) {
  size_t len = strlen(s);
...
__attribute__((always_inline)) inline static void get_field_values(struct field_values_t *bv,
...
__attribute__((always_inline)) inline static void add_to_grand_total(struct field_values_t *bv,
                               struct field_values_t *iv) {
  if (known_value(iv->total)) grand_fsu.fsu_files += iv->total;
  if (known_value(iv->available)) grand_fsu.fsu_ffree += iv->available;
...
__attribute__((always_inline)) inline static void get_dev(char const *disk, char const *mount_point, char const *file,
                    char const *stat_file, char const *fstype, bool me_dummy,
                    bool me_remote, const struct fs_usage *force_fsu,
                    bool process_all) {
  if (me_remote && show_local_fs) return;

  if (me_dummy && !show_all_fs && !show_listed_fs) return;

  if (!selected_fstype(fstype) || excluded_fstype(fstype)) return;

  /* Ignore relative MOUNT_POINTs, which are present for example
     in /proc/mounts on Linux with network namespaces.  */
  if (!force_fsu && mount_point && !IS_ABSOLUTE_FILE_NAME(mount_point)) return;

  /* If MOUNT_POINT is NULL, then the file system is not mounted, and this
     program reports on the file system that the special file is on.
     It would be better to report on the unmounted file system,
     but statfs doesn't do that on most systems.  */
  if (!stat_file) stat_file = mount_point ? mount_point : disk;
  ...
  if (fsu.fsu_blocks == 0 && !show_all_fs && !show_listed_fs) return;

  if (!force_fsu) file_systems_processed = true;

  alloc_table_row();

  if (!disk) disk = "-"; /* unknown */

  if (!file) file = "-"; /* unspecified */
    ...
    switch (columns[col]->field) {
      ...
      case IPCENT_FIELD: {
        ...
        if (!known_value(v->used) || !known_value(v->available))
          ...
          double a =
              v->negate_available ? -(double)-v->available : v->available;
          ...
          if (nonroot_total) {
            ...
            if (ipct - 1 < pct && pct <= ipct + 1) pct = ipct + (ipct < pct);
          }
        }

        if (0 <= pct) {
          if (asprintf(&cell, "%.0f%%", pct) == -1) cell = NULL;
        } else
          cell = strdup("-");
...
__attribute__((always_inline)) inline static char *last_device_for_mount(char const *mount) {
  struct mount_entry const *me;
  struct mount_entry const *le = NULL;

  for (me = mount_list; me; me = me->me_next) {
    if (STREQ(me->me_mountdir, mount)) le = me;
  }

  if (le) {
    char *devname = le->me_devname;
    char *canon_dev = canonicalize_file_name(devname);
    if (canon_dev && IS_ABSOLUTE_FILE_NAME(canon_dev)) return canon_dev;
    free(canon_dev);
...
__attribute__((always_inline)) inline static bool get_disk(char const *disk) {
  struct mount_entry const *me;
  ...
  if (resolved && IS_ABSOLUTE_FILE_NAME(resolved)) disk = resolved;
  ...
  for (me = mount_list; me; me = me->me_next) {
    ...
    if (canon_dev && IS_ABSOLUTE_FILE_NAME(canon_dev)) devname = canon_dev;
...
__attribute__((always_inline)) inline static void get_point(const char *point, const struct stat *statp) {
  struct stat disk_stats;
  struct mount_entry *me;
  struct mount_entry const *best_match = NULL;
...
__attribute__((always_inline)) inline static void get_entry(char const *name, struct stat const *statp) {
...
__attribute__((always_inline)) inline static void get_all_entries(void) {
  struct mount_entry *me;
...
__attribute__((always_inline)) inline static void add_fs_type(const char *fstype) {
...
__attribute__((always_inline)) inline static void add_excluded_fs_type(const char *fstype) {
  struct fs_type_list *fsp;
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    switch (c) {
      ...
      case 'B': {
        ...
        if (e != LONGINT_OK) xstrtol_fatal(e, oi, c, long_options, optarg);
      } break;
      case 'i':
      ...
      case OUTPUT_OPTION:
        ...
        header_mode = OUTPUT_MODE;
        if (optarg) decode_output_arg(optarg);
        break;
  ...
  if (optind < argc) {
    ...
    for (int i = optind; i < argc; ++i)
      if (argv[i]) get_entry(argv[i], &stats[i - optind]);
