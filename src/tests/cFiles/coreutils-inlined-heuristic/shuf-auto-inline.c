...
__attribute__((always_inline)) inline static void input_from_argv(char **operand, int n_operands, char eolbyte) {
  char *p;
  size_t size = n_operands;
  int i;

  for (i = 0; i < n_operands; i++) size += strlen(operand[i]);
  p = xmalloc(size);
...
__attribute__((always_inline)) inline static char *next_line(char *line, char eolbyte, size_t n) {
...
__attribute__((always_inline)) inline static off_t input_size(void) {
  off_t file_size;

  struct stat stat_buf;
  if (fstat(STDIN_FILENO, &stat_buf) != 0) return OFF_T_MAX;
  ...
  if (input_offset < 0) return OFF_T_MAX;
...
__attribute__((always_inline)) inline static size_t read_input_reservoir_sampling(FILE *in, char eolbyte, size_t k,
  ...
  if (line != NULL) {
    ...
    if (!n_lines) die(EXIT_FAILURE, EOVERFLOW, _("too many input lines"));
...
__attribute__((always_inline)) inline static int write_permuted_output_reservoir(size_t n_lines,
...
__attribute__((always_inline)) inline static size_t read_input(FILE *in, char eolbyte, char ***pline) {
  char *p;
  char *buf = NULL;
  size_t used;
  char *lim;
  ...
  if (!(buf = fread_file(in, &used))) die(EXIT_FAILURE, errno, _("read error"));

  if (used && buf[used - 1] != eolbyte) buf[used++] = eolbyte;

  lim = buf + used;

  n_lines = 0;
  for (p = buf; p < lim; p = next_line(p, eolbyte, lim - p)) n_lines++;
...
__attribute__((always_inline)) inline static int write_permuted_lines(size_t n_lines, char *const *line,
                                size_t const *permutation) {
  for (size_t i = 0; i < n_lines; i++) {
    char *const *p = line + permutation[i];
    size_t len = p[1] - p[0];
    if (fwrite(p[0], sizeof *p[0], len, stdout) != len) return -1;
  }
...
__attribute__((always_inline)) inline static int write_permuted_numbers(size_t n_lines, size_t lo_input,
                                  size_t const *permutation, char eolbyte) {
  for (size_t i = 0; i < n_lines; i++) {
    unsigned long int n = lo_input + permutation[i];
    if (printf("%lu%c", n, eolbyte) < 0) return -1;
  }
...
__attribute__((always_inline)) inline static int write_random_numbers(struct randint_source *s, size_t count,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (printf("%lu%c", j, eolbyte) < 0) return -1;
  }
...
__attribute__((always_inline)) inline static int write_random_lines(struct randint_source *s, size_t count,
...
int main(int argc, char **argv) {
    ...
    switch (optc) {
      ...
      case 'n': {
        ...
        if (e == LONGINT_OK)
          ...
          die(EXIT_FAILURE, 0, _("invalid line count: %s"), quote(optarg));
      } break;
  ...
  if (head_lines == 0) {
    ...
    if (n_operands == 1 &&
        !(STREQ(operand[0], "-") || freopen(operand[0], "r", stdin)))
  ...
  if (!repeat) permutation = randperm_new(randint_source, ahead_lines, n_lines);
  ...
  if (repeat) {
    if (head_lines == 0)
      ...
      if (n_lines == 0) die(EXIT_FAILURE, 0, _("no lines to repeat"));
      if (input_range)
  ...
  if (i != 0) die(EXIT_FAILURE, errno, _("write error"));
  ...
  if (reservoir) {
    size_t j;
    for (j = 0; j < n_lines; ++j) freebuffer(&reservoir[j]);
    free(reservoir);
  }
#endif
