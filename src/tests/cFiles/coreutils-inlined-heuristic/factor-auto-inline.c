...
#define count_leading_zeros(count, x)                                          \
  do {                                                                         \
    ...
    for (__clz_c = 0; (__clz_x & ((uintmax_t)0xff << (W_TYPE_SIZE - 8))) == 0; \
         __clz_c += 8)                                                         \
      __clz_x <<= 8;                                                           \
    for (; (intmax_t)__clz_x >= 0; __clz_c++) __clz_x <<= 1;                   \
    (count) = __clz_c;                                                         \
  } while (0)
#endif
...
#define submod2(r1, r0, a1, a0, b1, b0, n1, n0)                             \
  do {                                                                      \
    ...
    if ((intmax_t)(r1) < 0) add_ssaaaa((r1), (r0), (r1), (r0), (n1), (n0)); \
  } while (0)
...
/* Compute r = a mod d, where r = <*t1,retval>, a = <a1,a0>, d = <d1,d0>.
   Requires that d1 != 0.  */
__attribute__((always_inline)) inline static uintmax_t mod2(uintmax_t *r1, uintmax_t a1, uintmax_t a0, uintmax_t d1,
                      uintmax_t d0) {
  int cntd, cnta;
  ...
  for (int i = 0; i < cnt; i++) {
    if (ge2(a1, a0, d1, d0)) sub_ddmmss(a1, a0, a1, a0, d1, d0);
    rsh2(d1, d0, d1, d0, 1);
  }
...
__attribute__((always_inline)) inline static uintmax_t _GL_ATTRIBUTE_CONST gcd_odd(uintmax_t a, uintmax_t b) {
  if ((b & 1) == 0) {
    uintmax_t t = b;
    b = a;
    a = t;
...
__attribute__((always_inline)) inline static uintmax_t gcd2_odd(uintmax_t *r1, uintmax_t a1, uintmax_t a0,
  ...
  while ((a0 & 1) == 0) rsh2(a1, a0, a1, a0, 1);
...
__attribute__((always_inline)) inline static void factor_insert_multiplicity(struct factors *factors, uintmax_t prime,
                                       unsigned int m) {
...
__attribute__((always_inline)) inline static void factor_insert_large(struct factors *factors, uintmax_t p1,
                                uintmax_t p0) {
  if (p1 > 0) {
...
__attribute__((always_inline)) inline static void mpz_va_init(void (*mpz_single_init)(mpz_t), ...) {
  va_list ap;

  va_start(ap, mpz_single_init);

  mpz_t *mpz;
  while ((mpz = va_arg(ap, mpz_t *))) mpz_single_init(*mpz);
...
__attribute__((always_inline)) inline static void mp_factor_clear(struct mp_factors *factors) {
  for (unsigned int i = 0; i < factors->nfactors; i++) mpz_clear(factors->p[i]);

  free(factors->p);
  free(factors->e);
}

__attribute__((always_inline)) inline static void mp_factor_insert(struct mp_factors *factors, mpz_t prime) {
...
__attribute__((always_inline)) inline static void mp_factor_insert_ui(struct mp_factors *factors,
...
__attribute__((always_inline)) inline static void factor_insert_refind(struct factors *factors, uintmax_t p,
                                 unsigned int i, unsigned int off) {
  for (unsigned int j = 0; j < off; j++) p += primes_diff[i + j];
...
__attribute__((always_inline)) inline static uintmax_t factor_using_division(uintmax_t *t1p, uintmax_t t1,
  ...
  for (i = 0; t1 > 0 && i < PRIMES_PTAB_ENTRIES; i++) {
    for (;;) {
      ...
      if (LIKELY(q1 > primes_dtab[i].lim)) break;
      t1 = q1;
      t0 = q0;
      factor_insert(factors, p);
    }
...
#if HAVE_GMP
__attribute__((always_inline)) inline static void mp_factor_using_division(mpz_t t, struct mp_factors *factors) {
  mpz_t q;
  unsigned long int p;
...
__attribute__((always_inline)) static inline uintmax_t mulredc(uintmax_t a, uintmax_t b, uintmax_t m,
                                uintmax_t mi) {
...
__attribute__((always_inline)) inline static uintmax_t mulredc2(uintmax_t *r1p, uintmax_t a1, uintmax_t a0,
  ...
  */
  ...
  if (ge2(r1, r0, m1, m0)) sub_ddmmss(r1, r0, r1, r0, m1, m0);

  *r1p = r1;
  return r0;
}

__attribute__((always_inline)) inline static uintmax_t _GL_ATTRIBUTE_CONST powm(uintmax_t b, uintmax_t e, uintmax_t n,
  ...
  while (e != 0) {
    ...
    if (e & 1) y = mulredc(y, b, n, ni);
  }

  return y;
}

__attribute__((always_inline)) inline static uintmax_t powm2(uintmax_t *r1m, const uintmax_t *bp, const uintmax_t *ep,
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_CONST millerrabin(uintmax_t n, uintmax_t ni,
  ...
  if (y == one || y == nm1) return true;

  for (unsigned int i = 1; i < k; i++) {
    y = mulredc(y, y, n, ni);

    if (y == nm1) return true;
    if (y == one) return false;
  }
  return false;
}

__attribute__((always_inline)) inline static bool millerrabin2(const uintmax_t *np, uintmax_t ni, const uintmax_t *bp,
  ...
  if (y0 == one[0] && y1 == one[1]) return true;

  sub_ddmmss(nm1_1, nm1_0, np[1], np[0], one[1], one[0]);

  if (y0 == nm1_0 && y1 == nm1_1) return true;
  ...
  for (unsigned int i = 1; i < k; i++) {
    ...
    if (y0 == nm1_0 && y1 == nm1_1) return true;
    if (y0 == one[0] && y1 == one[1]) return false;
  }
  return false;
}

#if HAVE_GMP
__attribute__((always_inline)) inline static bool mp_millerrabin(mpz_srcptr n, mpz_srcptr nm1, mpz_ptr x, mpz_ptr y,
                           mpz_srcptr q, unsigned long int k) {
  mpz_powm(y, x, q, n);

  if (mpz_cmp_ui(y, 1) == 0 || mpz_cmp(y, nm1) == 0) return true;

  for (unsigned long int i = 1; i < k; i++) {
    mpz_powm_ui(y, y, 2, n);
    if (mpz_cmp(y, nm1) == 0) return true;
...
__attribute__((always_inline)) inline static bool prime_p(uintmax_t n) {
  int k;
  bool is_prime;
  ...
  if (n < (uintmax_t)FIRST_OMITTED_PRIME * FIRST_OMITTED_PRIME) return true;
  ...
  if (!millerrabin(n, ni, a_prim, q, k, one)) return false;
...
__attribute__((always_inline)) inline static bool prime2_p(uintmax_t n1, uintmax_t n0) {
  uintmax_t q[2], nm1[2];
  uintmax_t a_prim[2];
  uintmax_t one[2];
  uintmax_t na[2];
  ...
  if (!millerrabin2(na, ni, a_prim, q, k, one)) return false;
...
#if HAVE_GMP
__attribute__((always_inline)) inline static bool mp_prime_p(mpz_t n) {
  bool is_prime;
  mpz_t q, a, nm1, tmp;
...
ret1:
  if (flag_prove_primality) mp_factor_clear(&factors);
ret2:
...
__attribute__((always_inline)) inline static void factor_using_pollard_rho(uintmax_t n, unsigned long int a,
                                     struct factors *factors) {
  ...
  while (n != 1) {
    ...
    for (;;) {
      do {
        ...
        if (k % 32 == 1) {
          if (gcd_odd(P, n) != 1) goto factor_found;
          y = x;
        }
      } while (--k != 0);
...
__attribute__((always_inline)) inline static void factor_using_pollard_rho2(uintmax_t n1, uintmax_t n0,
                                      unsigned long int a,
                                      struct factors *factors) {
    ...
    for (;;) {
      do {
        ...
        if (k % 32 == 1) {
          ...
          if (g1 != 0 || g0 != 1) goto factor_found;
          y1 = x1;
          y0 = x0;
        }
      } while (--k != 0);
...
#if HAVE_GMP
__attribute__((always_inline)) inline static void mp_factor_using_pollard_rho(mpz_t n, unsigned long int a,
                                        struct mp_factors *factors) {
  mpz_t x, z, y, P;
  mpz_t t, t2;
...
__attribute__((always_inline)) inline static uintmax_t _GL_ATTRIBUTE_CONST isqrt(uintmax_t n) {
  uintmax_t x;
  unsigned c;
  if (n == 0) return 0;
...
__attribute__((always_inline)) inline static uintmax_t _GL_ATTRIBUTE_CONST isqrt2(uintmax_t nh, uintmax_t nl) {
  unsigned int shift;
  uintmax_t x;
...
__attribute__((always_inline)) inline static uintmax_t _GL_ATTRIBUTE_CONST is_square(uintmax_t x) {
...
__attribute__((always_inline)) inline static bool factor_using_squfof(uintmax_t n1, uintmax_t n0,
                                struct factors *factors) {
  ...
  if (n1 >= ((uintmax_t)1 << (W_TYPE_SIZE - 2))) return false;
  ...
  for (m = (n0 % 4 == 1) ? multipliers_3 : multipliers_1; *m; m++) {
    ...
    if (n1 == 0) {
      if ((uintmax_t)mu * mu * mu >= n0 / 64) continue;
    } else {
      if (n1 > ((uintmax_t)1 << (W_TYPE_SIZE - 2)) / mu) continue;
    }
...
/* Compute the prime factors of the 128-bit number (T1,T0), and put the
   results in FACTORS.  */
__attribute__((always_inline)) inline static void factor(uintmax_t t1, uintmax_t t0, struct factors *factors) {
  factors->nfactors = 0;
  factors->plarge[1] = 0;
...
#if USE_SQUFOF
    if (factor_using_squfof(t1, t0, factors)) return;
#endif
...
__attribute__((always_inline)) inline static void mp_factor(mpz_t t, struct mp_factors *factors) {
...
__attribute__((always_inline)) inline static strtol_error strto2uintmax(uintmax_t *hip, uintmax_t *lop,
                                  const char *s) {
...
__attribute__((always_inline)) inline static void lbuf_alloc(void) {
  if (lbuf.buf) return;
...
__attribute__((always_inline)) inline static void lbuf_flush(void) {
...
__attribute__((always_inline)) inline static void lbuf_putc(char c) {
  *lbuf.end++ = c;
...
__attribute__((always_inline)) inline static void lbuf_putint(uintmax_t i, size_t min_width) {
  char buf[INT_BUFSIZE_BOUND(uintmax_t)];
  char const *umaxstr = umaxtostr(i, buf);
  size_t width = sizeof(buf) - (umaxstr - buf) - 1;
  size_t z = width;

  for (; z < min_width; z++) *lbuf.end++ = '0';

  memcpy(lbuf.end, umaxstr, width);
  lbuf.end += width;
}

__attribute__((always_inline)) inline static void print_uintmaxes(uintmax_t t1, uintmax_t t0) {
  uintmax_t q, r;
...
__attribute__((always_inline)) inline static void print_factors_single(uintmax_t t1, uintmax_t t0) {
  struct factors factors;
...
__attribute__((always_inline)) inline static bool print_factors(const char *input) {
  uintmax_t t1, t0;
...
__attribute__((always_inline)) inline static bool do_stdin(void) {
  bool ok = true;
  ...
  while (true) {
    ...
    if (token_length == (size_t)-1) break;
...
int main(int argc, char **argv) {
  ...
  if (argc <= optind)
    ...
    for (int i = optind; i < argc; i++)
      if (!print_factors(argv[i])) ok = false;
  }
