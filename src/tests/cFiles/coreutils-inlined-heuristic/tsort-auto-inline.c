...
__attribute__((always_inline)) inline static struct item *new_item(const char *str) {
...
__attribute__((always_inline)) inline static struct item *search_item(struct item *root, const char *str) {
...
__attribute__((always_inline)) inline static void record_relation(struct item *j, struct item *k) {
  struct successor *p;
...
__attribute__((always_inline)) inline static bool count_items(struct item *unused _GL_UNUSED) {
  n_strings++;
  return false;
}

__attribute__((always_inline)) inline static bool scan_zeros(struct item *k) {
...
__attribute__((always_inline)) inline static bool detect_loop(struct item *k) {
  if (k->count > 0) {
...
__attribute__((always_inline)) inline static bool recurse_tree(struct item *root, bool (*action)(struct item *)) {
...
__attribute__((always_inline)) inline static void walk_tree(struct item *root, bool (*action)(struct item *)) {
  if (root->right) recurse_tree(root->right, action);
}

/* Do a topological sort on FILE.   Return true if successful.  */

__attribute__((always_inline)) inline static bool tsort(const char *file) {
  bool ok = true;
  struct item *root;
  struct item *j = NULL;
  struct item *k = NULL;
