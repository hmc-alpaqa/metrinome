...
#ifndef HAVE_MKFIFO
__attribute__((always_inline)) inline static int rpl_mkfifo(char const *file, mode_t mode) {
  errno = ENOTSUP;
  return -1;
}
...
__attribute__((always_inline)) inline static bool copy_internal(char const *src_name, char const *dst_name,
...
__attribute__((always_inline)) inline static int follow_fstatat(int dirfd, char const *filename, struct stat *st,
                          int flags) {
...
__attribute__((always_inline)) static inline int utimens_symlink(char const *file,
...
__attribute__((always_inline)) inline static int punch_hole(int fd, off_t offset, off_t length) {
  int ret = 0;
/* +0 is to work around older <linux/fs.h> defining HAVE_FALLOCATE to empty.  */
  ...
  if (ret < 0 && (is_ENOTSUP(errno) || errno == ENOSYS)) ret = 0;
#endif
#endif
  return ret;
}
...
__attribute__((always_inline)) inline static bool create_hole(int fd, char const *name, bool punch_holes,
                        off_t size) {
...
/* Copy the regular file open on SRC_FD/SRC_NAME to DST_FD/DST_NAME,
   ...
   bytes read.  */
__attribute__((always_inline)) inline static bool sparse_copy(int src_fd, int dest_fd, char *buf, size_t buf_size,
    ...
    while (n_read) {
      ...
      if (transition || last_chunk) {
        ...
        if (!prev_hole) {
          ...
          if (!create_hole(dest_fd, dst_name, punch_holes, psize)) return false;
        }
        ...
        if (last_chunk) {
          if (!csize) n_read = 0; /* Finished processing buffer.  */
...
__attribute__((always_inline)) static inline int clone_file(int dest_fd, int src_fd) {
#ifdef FICLONE
...
__attribute__((always_inline)) inline static bool write_zeros(int fd, off_t n_bytes) {
  static char *zeros;
  ...
  while (n_bytes) {
    ...
    if ((full_write(fd, zeros, n)) != n) return false;
    n_bytes -= n;
  }
...
/* Perform an efficient extent copy, if possible.  This avoids
   ...
   return false.  */
__attribute__((always_inline)) inline static bool extent_copy(int src_fd, int dest_fd, char *buf, size_t buf_size,
  ...
  *require_normal_copy = false;
  ...
  do {
    ...
    if (!ok) {
      if (scan.hit_final_extent) break;
    ...
    for (unsigned int i = 0; i < scan.ei_count || empty_extent; i++) {
      ...
      if (src_total_size < ext_start + ext_len) {
        if (src_total_size < ext_start) ext_start = src_total_size;
      ...
      if (0 && (scan.ext_info[i].ext_flags & FIEMAP_EXTENT_UNWRITTEN)) {
        ...
        if (n_read) wrote_hole_at_eof = read_hole;
      }
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE is_ancestor(const struct stat *sb,
...
#if USE_XATTR
__attribute__((always_inline)) inline static void copy_attr_error(struct error_context *ctx _GL_UNUSED,
                            char const *fmt, ...) {
...
__attribute__((always_inline)) inline static void copy_attr_allerror(struct error_context *ctx _GL_UNUSED,
                               char const *fmt, ...) {
  int err = errno;
  va_list ap;
...
__attribute__((always_inline)) inline static char const *copy_attr_quote(struct error_context *ctx _GL_UNUSED,
                                   char const *str) {
  return quoteaf(str);
}

__attribute__((always_inline)) inline static void copy_attr_free(struct error_context *ctx _GL_UNUSED,
...
/* Exclude SELinux extended attributes that are otherwise handled,
   ...
   Return zero to skip.  */
__attribute__((always_inline)) inline static int check_selinux_attr(const char *name, struct error_context *ctx) {
...
__attribute__((always_inline)) inline static bool copy_attr(char const *src_path, int src_fd, char const *dst_path,
...
__attribute__((always_inline)) inline static bool copy_attr(char const *src_path _GL_UNUSED, int src_fd _GL_UNUSED,
...
__attribute__((always_inline)) inline static bool copy_dir(char const *src_name_in, char const *dst_name_in,
...
__attribute__((always_inline)) inline static int set_owner(const struct cp_options *x, char const *dst_name,
  ...
  if (HAVE_FCHOWN && dest_desc != -1) {
    if (fchown(dest_desc, uid, gid) == 0) return 1;
    ...
    if (lchown(dst_name, uid, gid) == 0) return 1;
    if (errno == EPERM || errno == EINVAL) {
      /* We've failed to set *both*.  Now, try to set just the group
         ID, but ignore any failure here, and don't change errno.  */
      int saved_errno = errno;
  ...
  if (!chown_failure_ok(x)) {
    error(0, errno, _("failed to preserve ownership for %s"),
          quoteaf(dst_name));
    if (x->require_preserve) return -1;
  }
...
__attribute__((always_inline)) inline static void set_author(const char *dst_name, int dest_desc,
...
__attribute__((always_inline)) inline bool set_process_security_ctx(char const *src_name, char const *dst_name,
  ...
  if (x->preserve_security_context) {
    ...
    if (0 <= lgetfilecon(src_name, &con)) {
      ...
      if (x->require_preserve_context) return false;
    }
...
__attribute__((always_inline)) inline bool set_file_security_ctx(char const *dst_name, bool process_local,
...
__attribute__((always_inline)) inline static int fchmod_or_lchmod(int desc, char const *name, mode_t mode) {
#if HAVE_FCHMOD
  if (0 <= desc) return fchmod(desc, mode);
#endif
...
__attribute__((always_inline)) inline static bool is_probably_sparse(struct stat const *sb) {
...
__attribute__((always_inline)) inline static bool copy_reg(char const *src_name, char const *dst_name,
  ...
  if (!*new_dst) {
    ...
    if (dest_desc < 0 && x->unlink_dest_after_failed_open) {
      ...
      if (x->verbose) printf(_("removed %s\n"), quoteaf(dst_name));
  ...
  if (data_copy_required) {
    ...
    if (S_ISREG(sb.st_mode)) {
      ...
      if (x->sparse_mode == SPARSE_ALWAYS) make_holes = true;

      /* Use a heuristic to determine whether SRC_NAME contains any sparse
         blocks.  If the file has fewer blocks than would normally be
         needed for a file of its size, then at least one of the blocks in
         the file is a hole.  */
      if (x->sparse_mode == SPARSE_AUTO && sparse_src) make_holes = true;
    }
    ...
    if (!make_holes) {
      ...
      if (buf_size == 0 || blcm_max < buf_size) buf_size = blcm;
    }
...
preserve_metadata:
  ...
  if (x->preserve_xattr) {
    ...
    if (!(sb.st_mode & S_IWUSR) && geteuid() != ROOT_UID) {
      access_changed =
          fchmod_or_lchmod(dest_desc, dst_name, S_IRUSR | S_IWUSR) == 0;
    }
  ...
  if (x->preserve_mode || x->move_mode) {
    ...
    if (set_acl(dst_name, dest_desc, x->mode) != 0) return_val = false;
    ...
    if (omitted_permissions &&
      ...
      if (x->require_preserve) return_val = false;
    }
  }
...
__attribute__((always_inline)) inline static bool same_file_ok(char const *src_name, struct stat const *src_sb,
  ...
  *return_now = false;
  ...
  if (x->dereference == DEREF_NEVER) {
    ...
    if (S_ISLNK(src_sb->st_mode) && S_ISLNK(dst_sb->st_mode)) {
      ...
      if (!sn) {
        ...
        if (x->backup_type != no_backups) return true;
  ...
  if (x->move_mode || x->unlink_dest_before_opening) {
    ...
    if (S_ISLNK(dst_sb_link->st_mode)) return true;
  ...
  if (!S_ISLNK(src_sb_link->st_mode) && !S_ISLNK(dst_sb_link->st_mode)) {
    if (!SAME_INODE(*src_sb_link, *dst_sb_link)) return true;
  ...
  if (x->symbolic_link && S_ISLNK(dst_sb_link->st_mode)) return true;
  ...
  if (x->dereference == DEREF_NEVER) {
    ...
    if (!SAME_INODE(tmp_src_sb, tmp_dst_sb)) return true;
...
__attribute__((always_inline)) inline static bool writable_destination(char const *file, mode_t mode) {
  return (S_ISLNK(mode) || can_write_any_file() || euidaccess(file, W_OK) == 0);
}

__attribute__((always_inline)) inline static bool overwrite_ok(struct cp_options const *x, char const *dst_name,
                         struct stat const *dst_sb) {
...
__attribute__((always_inline)) inline extern void dest_info_init(struct cp_options *x) {
...
__attribute__((always_inline)) inline extern void src_info_init(struct cp_options *x) {
...
__attribute__((always_inline)) inline static bool abandon_move(const struct cp_options *x, char const *dst_name,
                         struct stat const *dst_sb) {
  assert(x->move_mode);
...
__attribute__((always_inline)) inline static void emit_verbose(char const *src, char const *dst,
                         char const *backup_dst_name) {
  printf("%s -> %s", quoteaf_n(0, src), quoteaf_n(1, dst));
  if (backup_dst_name) printf(_(" (backup: %s)"), quoteaf(backup_dst_name));
  putchar('\n');
}

/* A wrapper around "setfscreatecon (NULL)" that exits upon failure.  */
__attribute__((always_inline)) inline static void restore_default_fscreatecon_or_die(void) {
...
__attribute__((always_inline)) inline static bool create_hard_link(char const *src_name, char const *dst_name,
                             bool replace, bool verbose, bool dereference) {
  int err = force_linkat(AT_FDCWD, src_name, AT_FDCWD, dst_name,
                         dereference ? AT_SYMLINK_FOLLOW : 0, replace, -1);
  if (0 < err) {
    error(0, err, _("cannot create hard link %s to %s"), quoteaf_n(0, dst_name),
          quoteaf_n(1, src_name));
    return false;
  }
  if (err < 0 && verbose) printf(_("removed %s\n"), quoteaf(dst_name));
  return true;
}
...
__attribute__((always_inline)) static inline bool _GL_ATTRIBUTE_PURE
...
__attribute__((always_inline)) inline static bool source_is_dst_backup(char const *srcbase, struct stat const *src_st,
                                 char const *dst_name) {
...
__attribute__((always_inline)) inline static bool copy_internal(char const *src_name, char const *dst_name,
  ...
  *copy_into_self = false;
  ...
  if (x->move_mode) {
    if (rename_errno < 0)
      rename_errno =
          (renameatu(AT_FDCWD, src_name, AT_FDCWD, dst_name, RENAME_NOREPLACE)
               ? errno
               : 0);
    new_dst = rename_errno == 0;
    if (rename_succeeded) *rename_succeeded = new_dst;
  }
  ...
  if (command_line_arg && x->src_info) {
    if (!S_ISDIR(src_mode) && x->backup_type == no_backups &&
    ...
    if (rename_errno == EEXIST) {
      ...
      if (x->update && !S_ISDIR(src_mode)) {
        ...
        if (0 <= utimecmp(dst_name, &dst_sb, &src_sb, options)) {
          ...
          if (rename_succeeded) *rename_succeeded = true;
      ...
      if (x->backup_type != no_backups
        ...
        char *tmp_backup =
            backup_file_rename(AT_FDCWD, dst_name, x->backup_type);
    ...
    if (S_ISDIR(src_mode)) {
      ...
      if (same_name(src_name, earlier_file)) {
        ...
        *copy_into_self = true;
        ...
        if (x->move_mode && rename_succeeded) *rename_succeeded = true;
  ...
  if (x->move_mode) {
    ...
    if (rename_errno != EXDEV) {
      /* There are many ways this can happen due to a race condition.
         ...
         fail.  Etc.  */
      error(0, rename_errno, _("cannot move %s to %s"), quoteaf_n(0, src_name),
            quoteaf_n(1, dst_name));
    ...
    if (new_dst || !S_ISDIR(dst_sb.st_mode)) {
      ...
      if (x->set_security_context || x->preserve_security_context)
        if (!set_file_security_ctx(dst_name, x->preserve_security_context,
                                   false, x)) {
          if (x->require_preserve_context) goto un_backup;
        }
    }
  ...
  /* POSIX 2008 states that it is implementation-defined whether
  ...
  else if (x->hard_link && !(!CAN_HARDLINK_SYMLINKS && S_ISLNK(src_mode) &&
                             x->dereference == DEREF_NEVER)) {
    bool replace =
    ...
    if (0 < symlink_err && x->update && !new_dst && S_ISLNK(dst_sb.st_mode) &&
      ...
      if (dest_link_val) {
        if (STREQ(dest_link_val, src_link_val)) symlink_err = 0;
        free(dest_link_val);
      }
    }
    free(src_link_val);
    ...
    if (x->preserve_security_context) restore_default_fscreatecon_or_die();
    ...
    if (x->preserve_ownership) {
      ...
      if (HAVE_LCHOWN && lchown(dst_name, src_sb.st_uid, src_sb.st_gid) != 0 &&
        ...
        if (x->require_preserve) goto un_backup;
      } else {
  ...
  if (command_line_arg && x->dest_info) {
    ...
    struct stat sb;
    if (lstat(dst_name, &sb) == 0) record_file(x->dest_info, dst_name, &sb);
  }
  ...
  if (x->preserve_timestamps) {
    ...
    if ((dest_is_symlink ? utimens_symlink(dst_name, timespec)
      ...
      error(0, errno, _("preserving times for %s"), quoteaf(dst_name));
      if (x->require_preserve) return false;
    }
  }
  ...
  if (x->preserve_mode || x->move_mode) {
    ...
    if (set_acl(dst_name, -1, x->mode) != 0) return false;
  } else if (x->explicit_no_preserve_mode && new_dst) {
    int default_permissions =
        S_ISDIR(src_mode) || S_ISSOCK(src_mode) ? S_IRWXUGO : MODE_RW_UGO;
    ...
    if (omitted_permissions) {
      ...
      if (omitted_permissions && !restore_dst_mode) {
        ...
        dst_mode = dst_sb.st_mode;
        if (omitted_permissions & ~dst_mode) restore_dst_mode = true;
      }
    }
...
un_backup:
  ...
  if (earlier_file == NULL) forget_created(src_sb.st_ino, src_sb.st_dev);
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE valid_options(const struct cp_options *co) {
  assert(co != NULL);
...
__attribute__((always_inline)) inline extern bool copy(char const *src_name, char const *dst_name,
...
__attribute__((always_inline)) inline extern void cp_options_default(struct cp_options *x) {
  memset(x, 0, sizeof *x);
...
__attribute__((always_inline)) inline extern bool chown_failure_ok(struct cp_options const *x) {
...
__attribute__((always_inline)) inline extern mode_t cached_umask(void) {
