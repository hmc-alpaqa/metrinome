...
#if BASE_TYPE == 42
#define AUTHORS proper_name("Simon Josefsson"), proper_name("Assaf Gordon")
#else
...
__attribute__((always_inline)) inline static int base64_length_wrapper(int len) { return BASE64_LENGTH(len); }

__attribute__((always_inline)) inline static void base64_decode_ctx_init_wrapper(struct base_decode_context *ctx) {
  base64_decode_ctx_init(&ctx->ctx.base64);
}

__attribute__((always_inline)) inline static bool base64_decode_ctx_wrapper(struct base_decode_context *ctx,
                                      const char *restrict in, size_t inlen,
                                      char *restrict out, size_t *outlen) {
...
__attribute__((always_inline)) inline static void init_inbuf(struct base_decode_context *ctx) {
  ctx->bufsize = DEC_BLOCKSIZE;
  ctx->inbuf = xcharalloc(ctx->bufsize);
}

__attribute__((always_inline)) inline static void prepare_inbuf(struct base_decode_context *ctx, size_t inlen) {
...
__attribute__((always_inline)) inline static void base64url_encode(const char *restrict in, size_t inlen,
                             char *restrict out, size_t outlen) {
...
__attribute__((always_inline)) inline static bool isbase64url(char ch) {
  return (ch == '-' || ch == '_' || (ch != '+' && ch != '/' && isbase64(ch)));
}

__attribute__((always_inline)) inline static void base64url_decode_ctx_init_wrapper(struct base_decode_context *ctx) {
  base64_decode_ctx_init(&ctx->ctx.base64);
  init_inbuf(ctx);
}

__attribute__((always_inline)) inline static bool base64url_decode_ctx_wrapper(struct base_decode_context *ctx,
                                         const char *restrict in, size_t inlen,
                                         char *restrict out, size_t *outlen) {
...
__attribute__((always_inline)) inline static int base32_length_wrapper(int len) { return BASE32_LENGTH(len); }

__attribute__((always_inline)) inline static void base32_decode_ctx_init_wrapper(struct base_decode_context *ctx) {
  base32_decode_ctx_init(&ctx->ctx.base32);
}

__attribute__((always_inline)) inline static bool base32_decode_ctx_wrapper(struct base_decode_context *ctx,
                                      const char *restrict in, size_t inlen,
                                      char *restrict out, size_t *outlen) {
...
__attribute__((always_inline)) inline static bool isbase32hex(char ch) {
  return ('0' <= ch && ch <= '9') || ('A' <= ch && ch <= 'V');
}

__attribute__((always_inline)) inline static void base32hex_encode(const char *restrict in, size_t inlen,
                             char *restrict out, size_t outlen) {
...
__attribute__((always_inline)) inline static void base32hex_decode_ctx_init_wrapper(struct base_decode_context *ctx) {
  base32_decode_ctx_init(&ctx->ctx.base32);
  init_inbuf(ctx);
}

__attribute__((always_inline)) inline static bool base32hex_decode_ctx_wrapper(struct base_decode_context *ctx,
                                         const char *restrict in, size_t inlen,
                                         char *restrict out, size_t *outlen) {
  prepare_inbuf(ctx, inlen);
...
__attribute__((always_inline)) inline static bool isbase16(char ch) {
  return ('0' <= ch && ch <= '9') || ('A' <= ch && ch <= 'F');
}

__attribute__((always_inline)) inline static int base16_length(int len) { return len * 2; }

static const char base16[16] = "0123456789ABCDEF";

__attribute__((always_inline)) inline static void base16_encode(const char *restrict in, size_t inlen,
                          char *restrict out, size_t outlen) {
  while (inlen--) {
    unsigned char c = *in;
    *out++ = base16[c >> 4];
...
__attribute__((always_inline)) inline static void base16_decode_ctx_init(struct base_decode_context *ctx) {
  init_inbuf(ctx);
  ctx->ctx.base16.have_nibble = false;
  ctx->i = 1;
}

__attribute__((always_inline)) inline static bool base16_decode_ctx(struct base_decode_context *ctx,
                              const char *restrict in, size_t inlen,
                              char *restrict out, size_t *outlen) {
  ...
  *outlen = 0;
  ...
  if (inlen == 0) return !ctx->ctx.base16.have_nibble;
...
__attribute__((always_inline)) inline static int z85_length(int len) {
  /* Z85 does not allow padding, so no need to round to highest integer.  */
  int outlen = (len * 5) / 4;
  return outlen;
}

__attribute__((always_inline)) inline static bool isz85(char ch) {
...
__attribute__((always_inline)) inline static void z85_encode(const char *restrict in, size_t inlen,
                       char *restrict out, size_t outlen) {
  int i = 0;
  ...
  while (1) {
    ...
    if (i == 4) {
      val = ((uint32_t)quad[0] << 24) + ((uint32_t)quad[1] << 16) +
            ((uint32_t)quad[2] << 8) + quad[3];
      ...
      for (int j = 4; j >= 0; --j) {
        ...
        /* NOTE: if there is padding (which is trimmed by z85
           ...
           so don't store them. */
        if (outidx + j < outlen) out[j] = z85_encoding[c];
      }
      out += 5;
      outidx += 5;
      i = 0;
...
__attribute__((always_inline)) inline static void z85_decode_ctx_init(struct base_decode_context *ctx) {
  init_inbuf(ctx);
  ctx->ctx.z85.i = 0;
  ctx->i = 1;
}
...
#define Z85_LO_CTX_TO_32BIT_VAL(ctx)                                        \
  (((ctx)->ctx.z85.octets[1] * 85 * 85 * 85) +                              \
   ((ctx)->ctx.z85.octets[2] * 85 * 85) + ((ctx)->ctx.z85.octets[3] * 85) + \
...
__attribute__((always_inline)) inline static bool z85_decode_ctx(struct base_decode_context *ctx,
                           const char *restrict in, size_t inlen,
                           char *restrict out, size_t *outlen) {
  bool ignore_lines = true; /* for now, always ignore them */
  ...
  *outlen = 0;
  ...
  while (inlen--) {
    ...
    if (c >= 33 && c <= 125) {
      c = z85_decoding[c - 33];
      if (c == 255) return false; /* garbage - return false */
    } else
      return false; /* garbage - return false */
...
__attribute__((always_inline)) inline static bool isbase2(char ch) { return ch == '0' || ch == '1'; }

__attribute__((always_inline)) inline static int base2_length(int len) { return len * 8; }

__attribute__((always_inline)) inline static void base2msbf_encode(const char *restrict in, size_t inlen,
                                    char *restrict out, size_t outlen) {
  while (inlen--) {
    unsigned char c = *in;
    for (int i = 0; i < 8; i++) {
...
__attribute__((always_inline)) inline static void base2lsbf_encode(const char *restrict in, size_t inlen,
                                    char *restrict out, size_t outlen) {
  while (inlen--) {
    unsigned char c = *in;
    for (int i = 0; i < 8; i++) {
...
__attribute__((always_inline)) inline static void base2_decode_ctx_init(struct base_decode_context *ctx) {
  init_inbuf(ctx);
  ctx->ctx.base2.octet = 0;
  ctx->i = 0;
}

__attribute__((always_inline)) inline static bool base2lsbf_decode_ctx(struct base_decode_context *ctx,
                                 const char *restrict in, size_t inlen,
                                 char *restrict out, size_t *outlen) {
  bool ignore_lines = true; /* for now, always ignore them */
...
__attribute__((always_inline)) inline static bool base2msbf_decode_ctx(struct base_decode_context *ctx,
                                 const char *restrict in, size_t inlen,
                                 char *restrict out, size_t *outlen) {
  bool ignore_lines = true; /* for now, always ignore them */
...
__attribute__((always_inline)) inline static void wrap_write(const char *buffer, size_t len, uintmax_t wrap_column,
  ...
  if (wrap_column == 0) {
    ...
    for (written = 0; written < len;) {
      ...
      if (to_write == 0) {
        if (fputc('\n', out) == EOF) die(EXIT_FAILURE, errno, _("write error"));
        *current_column = 0;
      } else {
...
__attribute__((always_inline)) inline static void do_encode(FILE *in, FILE *out, uintmax_t wrap_column) {
  ...
  if (ferror(in)) die(EXIT_FAILURE, errno, _("read error"));

  IF_LINT(free(inbuf));
  IF_LINT(free(outbuf));
}

__attribute__((always_inline)) inline static void do_decode(FILE *in, FILE *out, bool ignore_garbage) {
  char *inbuf, *outbuf;
  size_t sum;
  ...
  do {
    ...
    for (k = 0; k < 1 + !!feof(in); k++) {
      ...
      if (!ok) die(EXIT_FAILURE, 0, _("invalid input"));
    }
  } while (!feof(in));
...
int main(int argc, char **argv) {
  ...
  if (STREQ(infile, "-")) {
    ...
    if (input_fh == NULL) die(EXIT_FAILURE, errno, "%s", quotef(infile));
  }
