...
__attribute__((always_inline)) inline static bool no_leading_hyphen(char const *file) {
  char const *p;
...
__attribute__((always_inline)) inline static bool portable_chars_only(char const *file, size_t filelen) {
...
__attribute__((always_inline)) inline static char *_GL_ATTRIBUTE_PURE component_start(char *f) {
  while (*f == '/') f++;
  return f;
}
...
__attribute__((always_inline)) inline static size_t _GL_ATTRIBUTE_PURE component_len(char const *f) {
  size_t len;
  for (len = 1; f[len] != '/' && f[len]; len++) continue;
  return len;
}
...
__attribute__((always_inline)) inline static bool validate_file_name(char *file, bool check_basic_portability,
  ...
  if (check_extra_portability && !no_leading_hyphen(file)) return false;
  ...
  if (check_basic_portability) {
    if (!portable_chars_only(file, filelen)) return false;
  } else {
