...
__attribute__((always_inline)) inline static void add_range_pair(uintmax_t lo, uintmax_t hi) {
  if (n_frp == n_frp_allocated) frp = X2NREALLOC(frp, &n_frp_allocated);
  frp[n_frp].lo = lo;
  frp[n_frp].hi = hi;
  ++n_frp;
}
...
/* Comparison function for qsort to order the list of
   struct range_pairs.  */
__attribute__((always_inline)) inline static int compare_ranges(const void *a, const void *b) {
...
__attribute__((always_inline)) inline static void complement_rp(void) {
  ...
  if (c[0].lo > 1) add_range_pair(1, c[0].lo - 1);

  for (size_t i = 1; i < n; ++i) {
    if (c[i - 1].hi + 1 == c[i].lo) continue;

    add_range_pair(c[i - 1].hi + 1, c[i].lo - 1);
  }

  if (c[n - 1].hi < UINTMAX_MAX) add_range_pair(c[n - 1].hi + 1, UINTMAX_MAX);
...
/* Given the list of field or byte range specifications FIELDSTR,
   ...
   given '3-',    frp = [ { .lo = 3,           .hi = UINTMAX_MAX },
                          { .lo = UINTMAX_MAX, .hi = UINTMAX_MAX } ];
*/
__attribute__((always_inline)) inline void set_fields(const char *fieldstr, unsigned int options) {
    ...
    if (*fieldstr == '-') {
      ...
      if (dash_found) {
        ...
        if (!rhs_specified) {
          ...
          /* 'm-n' or '-n' (1-n). */
          if (value < initial) FATAL_ERROR(_("invalid decreasing range"));
      ...
      if (!in_digits || !num_start) num_start = fieldstr;
      in_digits = true;
  ...
  if (options & SETFLD_COMPLEMENT) complement_rp();
...
__attribute__((always_inline)) inline void reset_fields(void) {
  n_frp = 0;
  n_frp_allocated = 0;
  free(frp);
  frp = NULL;
