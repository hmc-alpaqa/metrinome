...
enum { RFC_3339_OPTION = CHAR_MAX + 1, DEBUG_DATE_PARSING };
...
__attribute__((always_inline)) inline static bool batch_convert(const char *input_filename, const char *format,
  ...
  while (1) {
    ...
    if (!parse_datetime2(&when, line, NULL, parse_datetime_flags, tz,
                         tzstring)) {
  ...
  while ((optc = getopt_long(argc, argv, short_options, long_options, NULL)) !=
    ...
    switch (optc) {
      ...
      case RFC_3339_OPTION: {
        ...
        enum Time_spec i = XARGMATCH("--rfc-3339", optarg, time_spec_string + 2,
                                     time_spec + 2);
      ...
      case 'I': {
        ...
        enum Time_spec i = (optarg ? XARGMATCH("--iso-8601", optarg,
      ...
      case 'u':
        /* POSIX says that 'date -u' is equivalent to setting the TZ
           ...
           than setting TZ.  */
        if (putenv(bad_cast("TZ=UTC0")) != 0) xalloc_die();
        TZSET;
        break;
    ...
    if (new_format) {
      if (format) die(EXIT_FAILURE, 0, _("multiple output formats specified"));
      format = new_format;
    }
  }
  ...
  if (batch_file != NULL)
    ...
    if (!option_specified_date && !set_date) {
      ...
      if (reference != NULL) {
        ...
        if (set_datestr) datestr = set_datestr;
        valid_date = parse_datetime2(&when, datestr, NULL, parse_datetime_flags,
                                     tz, tzstring);
      }
    }

    if (!valid_date) die(EXIT_FAILURE, 0, _("invalid date %s"), quote(datestr));
...
__attribute__((always_inline)) inline static bool show_date(const char *format, struct timespec when, timezone_t tz) {
  struct tm tm;

  if (localtime_rz(tz, &when.tv_sec, &tm)) {
    if (format == rfc_email_format) setlocale(LC_TIME, "C");
    fprintftime(stdout, format, &tm, tz, when.tv_nsec);
    if (format == rfc_email_format) setlocale(LC_TIME, "");
    fputc('\n', stdout);
    return true;
  } else {
