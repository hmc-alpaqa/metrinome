...
__attribute__((always_inline)) inline static void print_table_row(int num_width, int signum, int name_width,
                            char const *signame) {
...
__attribute__((always_inline)) inline static int list_signals(bool table, char *const *argv) {
  int signum;
  ...
  if (table) {
    ...
    for (signum = 1; signum <= SIGNUM_BOUND / 10; signum *= 10) num_width++;
    ...
    if (argv)
      ...
      for (signum = 1; signum <= SIGNUM_BOUND; signum++)
        if (sig2str(signum, signame) == 0) puts(signame);
  }
...
__attribute__((always_inline)) inline static int send_signals(int signum, char *const *argv) {
  int status = EXIT_SUCCESS;
  char const *arg = *argv;
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, short_options, long_options, NULL)) !=
    ...
    switch (optc) {
      ...
      case 'Y':
      case 'Z':
        if (!optarg) optarg = argv[optind - 1] + strlen(argv[optind - 1]);
