...
__attribute__((always_inline)) inline static void advance(bool f) {
  ++pos;

  if (f && pos >= argc) beyond();
}

__attribute__((always_inline)) inline static void unary_advance(void) {
  advance(true);
  ++pos;
}
 ...
 *	error condition)
 */
__attribute__((always_inline)) inline static void beyond(void) {
...
__attribute__((always_inline)) inline static char const *find_int(char const *string) {
  char const *p;
  char const *number_start;

  for (p = string; isblank(to_uchar(*p)); p++) continue;
  ...
  if (ISDIGIT(*p++)) {
    while (ISDIGIT(*p)) p++;
    while (isblank(to_uchar(*p))) p++;
    if (!*p) return number_start;
  }
...
__attribute__((always_inline)) inline static bool get_mtime(char const *filename, struct timespec *mtime) {
  struct stat finfo;
  ...
  *mtime = zero;
#endif
  if (ok) *mtime = get_stat_mtime(&finfo);
  return ok;
}

/* Return true if S is one of the test command's binary operators.  */
__attribute__((always_inline)) inline static bool binop(char const *s) {
...
__attribute__((always_inline)) inline static bool binary_operator(bool l_is_l) {
  int op;
  struct stat stat_buf, stat_spare;
  /* Is the right integer expression of the form '-l string'? */
  bool r_is_l;

  if (l_is_l) advance(false);
  op = pos + 1;
...
__attribute__((always_inline)) inline static bool unary_operator(void) {
  ...
  switch (argv[pos][1]) {
    ...
    {
      unary_advance();
      if (stat(argv[pos - 1], &stat_buf) != 0) return false;
...
__attribute__((always_inline)) inline static bool and (void) {
  ...
  while (true) {
    value &= term();
    if (!(pos < argc && STREQ(argv[pos], "-a"))) return value;
    advance(false);
  }
}
...
__attribute__((always_inline)) inline static bool or (void) {
  ...
  while (true) {
    value |= and();
    if (!(pos < argc && STREQ(argv[pos], "-o"))) return value;
    advance(false);
  }
}
...
__attribute__((always_inline)) inline static bool posixtest(int nargs) {
  bool value;
...
int main(int margc, char **margv) {
  ...
  if (LBRACKET) {
    ...
    if (margc == 2) {
      if (STREQ(margv[1], "--help")) usage(EXIT_SUCCESS);
  ...
  if (pos >= argc) test_main_return(TEST_FALSE);

  value = posixtest(argc - 1);

  if (pos != argc) test_syntax_error(_("extra argument %s"), quote(argv[pos]));
