...
__attribute__((always_inline)) inline static wide_uint _GL_ATTRIBUTE_CONST binvert(wide_uint a) {
...
__attribute__((always_inline)) inline static void process_prime(struct prime *info, unsigned p) {
  wide_uint max = -1;
  info->p = p;
  info->pinv = binvert(p);
  info->lim = max / p;
...
__attribute__((always_inline)) inline static void print_wide_uint(wide_uint n, int nesting, unsigned wide_uint_bits) {
...
__attribute__((always_inline)) inline static void output_primes(const struct prime *primes, unsigned nprimes) {
  unsigned i;
  unsigned p;
  int is_prime;
  ...
  wide_uint mask = -1;
  for (wide_uint_bits = 0; mask; wide_uint_bits++) mask >>= 1;
  ...
  do {
    ...
    for (i = 0, is_prime = 1; is_prime; i++) {
      if (primes[i].p * primes[i].p > p) break;
...
__attribute__((always_inline)) inline static void *_GL_ATTRIBUTE_MALLOC xalloc(size_t s) {
  void *p = malloc(s);
  if (p) return p;
...
int main(int argc, char **argv) {
  ...
  limit = atoi(argv[1]);
  if (limit < 3) return EXIT_SUCCESS;
  ...
  for (i = 0; i < size;) {
    ...
    for (j = (p * p - 3) / 2; j < size; j += p) sieve[j] = 0;
