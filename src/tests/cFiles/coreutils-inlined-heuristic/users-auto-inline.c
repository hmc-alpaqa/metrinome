...
__attribute__((always_inline)) inline static int userid_compare(const void *v_a, const void *v_b) {
  char **a = (char **)v_a;
  char **b = (char **)v_b;
  return strcmp(*a, *b);
}

__attribute__((always_inline)) inline static void list_entries_users(size_t n, const STRUCT_UTMP *this) {
  ...
  for (i = 0; i < n_entries; i++) free(u[i]);
  free(u);
}
...
__attribute__((always_inline)) inline static void users(const char *filename, int options) {
  size_t n_users;
