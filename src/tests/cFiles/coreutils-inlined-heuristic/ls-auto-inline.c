...
__attribute__((always_inline)) inline static size_t quote_name(char const *name,
                         struct quoting_options const *options,
                         int needs_general_quoting, const struct bin_str *color,
                         bool allow_pad, struct obstack *stack,
                         char const *absolute_name);
__attribute__((always_inline)) inline static size_t quote_name_buf(char **inbuf, size_t bufsize, char *name,
                             struct quoting_options const *options,
...
__attribute__((always_inline)) inline static void dev_ino_push(dev_t dev, ino_t ino) {
  void *vdi;
  struct dev_ino *di;
...
__attribute__((always_inline)) inline static struct dev_ino dev_ino_pop(void) {
  void *vdi;
  struct dev_ino *di;
  int dev_ino_size = sizeof *di;
...
__attribute__((always_inline)) inline static void dired_dump_obstack(const char *prefix, struct obstack *os) {
  size_t n_pos;
...
__attribute__((always_inline)) inline static struct timespec get_stat_btime(struct stat const *st) {
...
#if HAVE_STATX && defined STATX_INO
__attribute__((always_inline)) inline static unsigned int _GL_ATTRIBUTE_PURE time_type_to_statx(void) {
  switch (time_type) {
    case time_ctime:
      return STATX_CTIME;
    case time_mtime:
...
__attribute__((always_inline)) inline static unsigned int _GL_ATTRIBUTE_PURE calc_req_mask(void) {
  unsigned int mask = STATX_MODE;

  if (print_inode) mask |= STATX_INO;

  if (print_block_size) mask |= STATX_BLOCKS;

  if (format == long_format) {
    mask |= STATX_NLINK | STATX_SIZE | time_type_to_statx();
    if (print_owner || print_author) mask |= STATX_UID;
    if (print_group) mask |= STATX_GID;
  }
...
__attribute__((always_inline)) inline static int do_statx(int fd, const char *name, struct stat *st, int flags,
                    unsigned int mask) {
  struct statx stx;
...
__attribute__((always_inline)) static inline int do_stat(const char *name, struct stat *st) {
  return do_statx(AT_FDCWD, name, st, 0, calc_req_mask());
}

__attribute__((always_inline)) static inline int do_lstat(const char *name, struct stat *st) {
  return do_statx(AT_FDCWD, name, st, AT_SYMLINK_NOFOLLOW, calc_req_mask());
}

__attribute__((always_inline)) static inline int stat_for_mode(const char *name, struct stat *st) {
  return do_statx(AT_FDCWD, name, st, 0, STATX_MODE);
}

/* dev+ino should be static, so no need to sync with backing store */
__attribute__((always_inline)) static inline int stat_for_ino(const char *name, struct stat *st) {
  return do_statx(AT_FDCWD, name, st, 0, STATX_INO);
}

__attribute__((always_inline)) static inline int fstat_for_ino(int fd, struct stat *st) {
...
__attribute__((always_inline)) inline static char const *_GL_ATTRIBUTE_PURE first_percent_b(char const *fmt) {
  for (; *fmt; fmt++)
...
__attribute__((always_inline)) inline static void file_escape_init(void) {
  for (int i = 0; i < 256; i++)
...
__attribute__((always_inline)) inline static bool abmon_init(char abmon[12][ABFORMAT_SIZE]) {
  ...
  do {
    ...
    for (int i = 0; i < 12; i++) {
      ...
      if (strchr(abbr, '%')) return false;
      mbs_align_t alignment =
          isdigit(to_uchar(*abbr)) ? MBS_ALIGN_RIGHT : MBS_ALIGN_LEFT;
      size_t req =
          mbsalign(abbr, abmon[i], ABFORMAT_SIZE, &width, alignment, 0);
      if (!(req < ABFORMAT_SIZE)) return false;
...
__attribute__((always_inline)) inline static void abformat_init(void) {
  char const *pb[2];
  ...
  for (int recent = 0; recent < 2; recent++) {
    ...
    for (int i = 0; i < 12; i++) {
      ...
      if (!pb[recent])
        ...
        if (!(pb[recent] - fmt <= MIN(ABFORMAT_SIZE, INT_MAX))) return;
        int prefix_len = pb[recent] - fmt;
        nbytes = snprintf(nfmt, ABFORMAT_SIZE, "%.*s%s%s", prefix_len, fmt,
                          abmon[i], pb[recent] + 2);
      }

      if (!(0 <= nbytes && nbytes < ABFORMAT_SIZE)) return;
    }
  }
...
__attribute__((always_inline)) inline static size_t dev_ino_hash(void const *x, size_t table_size) {
  struct dev_ino const *p = x;
  return (uintmax_t)p->st_ino % table_size;
}

__attribute__((always_inline)) inline static bool dev_ino_compare(void const *x, void const *y) {
  struct dev_ino const *a = x;
  struct dev_ino const *b = y;
  return SAME_INODE(*a, *b) ? true : false;
}

__attribute__((always_inline)) inline static void dev_ino_free(void *x) { free(x); }

/* Add the device/inode pair (P->st_dev/P->st_ino) to the set of
   active directories.  Return true if there is already a matching
   entry in the table.  */

__attribute__((always_inline)) inline static bool visit_dir(dev_t dev, ino_t ino) {
  struct dev_ino *ent;
...
__attribute__((always_inline)) inline static void free_pending_ent(struct pending *p) {
  free(p->name);
  free(p->realname);
  free(p);
}

__attribute__((always_inline)) inline static bool is_colored(enum indicator_no type) {
...
__attribute__((always_inline)) inline static void restore_default_color(void) {
  put_indicator(&color_indicator[C_LEFT]);
  put_indicator(&color_indicator[C_RIGHT]);
}

__attribute__((always_inline)) inline static void set_normal_color(void) {
...
__attribute__((always_inline)) inline static void sighandler(int sig) {
  ...
  if (!interrupt_signal) interrupt_signal = sig;
}

/* A SIGTSTP was received; arrange for the program to suspend itself.  */

__attribute__((always_inline)) inline static void stophandler(int sig) {
  if (!SA_NOCLDSTOP) signal(sig, stophandler);
  if (!interrupt_signal) stop_signal_count++;
}
...
__attribute__((always_inline)) inline static void process_signals(void) {
  while (interrupt_signal || stop_signal_count) {
    int sig;
    int stops;
    sigset_t oldset;

    if (used_color) restore_default_color();
    fflush(stdout);
...
__attribute__((always_inline)) inline static void signal_setup(bool init) {
  ...
  if (init) {
    ...
    for (j = 0; j < nsigs; j++) {
      ...
      if (act.sa_handler != SIG_IGN) sigaddset(&caught_signals, sig[j]);
    }
...
#if SA_NOCLDSTOP
    for (j = 0; j < nsigs; j++)
      if (sigismember(&caught_signals, sig[j])) signal(sig[j], SIG_DFL);
#else
    for (j = 0; j < nsigs; j++)
      if (caught_sig[j]) signal(sig[j], SIG_DFL);
#endif
  }
}

__attribute__((always_inline)) static inline void signal_init(void) { signal_setup(true); }

__attribute__((always_inline)) static inline void signal_restore(void) { signal_setup(false); }
...
int main(int argc, char **argv) {
  ...
  if (recursive) {
    ...
    if (active_dir_set == NULL) xalloc_die();
  ...
  if (cwd_n_used) {
    sort_files();
    if (!immediate_dirs) extract_dirs_from_files(NULL, true);
  ...
  if (print_with_color && used_color) {
    ...
    /* Act on any signals that arrived before the default was restored.
       ...
       important anyway.  */
    for (j = stop_signal_count; j; j--) raise(SIGSTOP);
    j = interrupt_signal;
    if (j) raise(j);
  }
...
__attribute__((always_inline)) inline static bool set_line_length(char const *spec) {
  uintmax_t val;
...
__attribute__((always_inline)) inline static int decode_switches(int argc, char **argv) {
    ...
    switch (c) {
      ...
      case 'w':
        if (!set_line_length(optarg))
          die(LS_FAILURE, 0, "%s: %s", _("invalid line width"), quote(optarg));
        break;
      ...
      case '1':
        ...
        if (format != long_format) format = one_per_line;
        break;
      ...
      case COLOR_OPTION: {
        ...
        print_with_color =
            (i == when_always || (i == when_if_tty && isatty(STDOUT_FILENO)));
      ...
      case INDICATOR_STYLE_OPTION:
        indicator_style =
            XARGMATCH("--indicator-style", optarg, indicator_style_args,
                      indicator_style_types);
        break;

      case QUOTING_STYLE_OPTION:
        set_quoting_style(
            NULL, XARGMATCH("--quoting-style", optarg, quoting_style_args,
                            quoting_style_vals));
        break;
  ...
  align_variable_outer_quotes =
      format != with_commas && format != one_per_line &&
  ...
  if (dired && (format != long_format || print_hyperlink)) dired = false;
  ...
  if (format == long_format) {
    ...
    if (!style)
      if (!(style = getenv("TIME_STYLE"))) style = bad_cast("locale");
    ...
    if (*style == '+') {
      ...
      if (!p1)
        ...
        if (strchr(p1 + 1, '\n'))
          die(LS_FAILURE, 0, _("invalid time style format %s"), quote(p0));
        *p1++ = '\0';
      }
      ...
      if (res < 0) {
        ...
        while (*p) fprintf(stderr, "  - [posix-]%s\n", *p++);
...
__attribute__((always_inline)) inline static bool get_funky_string(char **dest, const char **src, bool equals_end,
                             size_t *output_count) {
...
__attribute__((always_inline)) inline static bool known_term_type(void) {
  ...
  while (line - G_line < sizeof(G_line)) {
    if (STRNCMP_LIT(line, "TERM ") == 0) {
      if (fnmatch(line + 5, term, 0) == 0) return true;
    }
    line += strlen(line) + 1;
  }
      ...
      case PS_START: /* First label character */
        switch (*p) {
          ...
          case '*':
            ...
            state = (get_funky_string(&buf, &p, true, &ext->ext.len) ? PS_4
                                                                     : PS_FAIL);
            break;
...
__attribute__((always_inline)) inline static void set_exit_status(bool serious) {
  if (serious)
...
__attribute__((always_inline)) inline static void file_failure(bool serious, char const *message, char const *file) {
...
__attribute__((always_inline)) inline static void print_dir(char const *name, char const *realname,
  ...
  if (recursive || print_dir_name) {
    ...
    quote_name(realname ? realname : name, dirname_quoting_options, -1, NULL,
               true, &subdired_obstack, absolute_name);
  ...
  if (recursive) extract_dirs_from_files(name, false);
...
__attribute__((always_inline)) inline static bool patterns_match(struct ignore_pattern const *patterns,
                           char const *file) {
  struct ignore_pattern const *p;
  for (p = patterns; p; p = p->next)
    if (fnmatch(p->pattern, file, FNM_PERIOD) == 0) return true;
  return false;
}
...
__attribute__((always_inline)) inline static uintmax_t unsigned_file_size(off_t size) {
  return size + (size < 0) * ((uintmax_t)OFF_T_MAX - OFF_T_MIN + 1);
}

#ifdef HAVE_CAP
/* Return true if NAME has a capability (see linux/capability.h) */
__attribute__((always_inline)) inline static bool has_capability(char const *name) {
  char *result;
  bool has_cap;

  cap_t cap_d = cap_get_file(name);
  if (cap_d == NULL) return false;
...
#else
__attribute__((always_inline)) inline static bool has_capability(char const *name _GL_UNUSED) {
  errno = ENOTSUP;
  return false;
}
#endif
...
__attribute__((always_inline)) inline static void free_ent(struct fileinfo *f) {
  free(f->name);
  free(f->linkname);
...
__attribute__((always_inline)) inline static bool errno_unsupported(int err) {
...
__attribute__((always_inline)) inline static int getfilecon_cache(char const *file, struct fileinfo *f, bool deref) {
  ...
  if (r < 0 && errno_unsupported(errno)) unsupported_device = f->stat.st_dev;
  return r;
}
...
__attribute__((always_inline)) inline static int file_has_acl_cache(char const *file, struct fileinfo *f) {
  /* st_dev of the most recently processed device for which we've
  ...
  if (n <= 0 && errno_unsupported(errno)) unsupported_device = f->stat.st_dev;
  return n;
}
...
__attribute__((always_inline)) inline static bool has_capability_cache(char const *file, struct fileinfo *f) {
  /* st_dev of the most recently processed device for which we've
  ...
  if (!b && errno_unsupported(errno)) unsupported_device = f->stat.st_dev;
  return b;
}

__attribute__((always_inline)) inline static bool needs_quoting(char const *name) {
  char test[2];
  size_t len =
...
__attribute__((always_inline)) inline static uintmax_t gobble_file(char const *name, enum filetype type, ino_t inode,
  ...
  if ((!cwd_some_quoted) && align_variable_outer_quotes) {
    ...
    if (f->quoted) cwd_some_quoted = 1;
  }

  if (command_line_arg || print_hyperlink ||
      format_needs_stat
...

  {
    ...
    if (format == long_format || print_scontext) {
      ...
      if (err == 0) {
        ...
        if (is_ENOTSUP(errno) || errno == ENODATA) err = 0;
      }
      ...
      if (err) error(0, errno, "%s", quotef(full_name));
    }
    ...
    if (format == long_format || print_block_size) {
      ...
      int len = mbswidth(human_readable(blocks, buf, human_output_opts,
                         ...
                         0);
      if (block_size_width < len) block_size_width = len;
    }

    if (format == long_format) {
      if (print_owner) {
        int len = format_user_width(f->stat.st_uid);
        if (owner_width < len) owner_width = len;
      }

      if (print_group) {
        int len = format_group_width(f->stat.st_gid);
        if (group_width < len) group_width = len;
      }

      if (print_author) {
        int len = format_user_width(f->stat.st_author);
        if (author_width < len) author_width = len;
      }
    }

    if (print_scontext) {
      int len = strlen(f->scontext);
      if (scontext_width < len) scontext_width = len;
    }

    if (format == long_format) {
      char b[INT_BUFSIZE_BOUND(uintmax_t)];
      int b_len = strlen(umaxtostr(f->stat.st_nlink, b));
      if (nlink_width < b_len) nlink_width = b_len;

      if (S_ISCHR(f->stat.st_mode) || S_ISBLK(f->stat.st_mode)) {
        char buf[INT_BUFSIZE_BOUND(uintmax_t)];
        int len = strlen(umaxtostr(major(f->stat.st_rdev), buf));
        if (major_device_number_width < len) major_device_number_width = len;
        len = strlen(umaxtostr(minor(f->stat.st_rdev), buf));
        if (minor_device_number_width < len) minor_device_number_width = len;
        len = major_device_number_width + 2 + minor_device_number_width;
        if (file_size_width < len) file_size_width = len;
      } else {
        char buf[LONGEST_HUMAN_READABLE + 1];
  ...
  if (print_inode) {
    ...
    if (inode_number_width < len) inode_number_width = len;
  }
...
__attribute__((always_inline)) inline static bool is_directory(const struct fileinfo *f) {
  return f->filetype == directory || f->filetype == arg_directory;
}

/* Return true if F refers to a (symlinked) directory.  */
__attribute__((always_inline)) inline static bool is_linked_directory(const struct fileinfo *f) {
  return f->filetype == directory || f->filetype == arg_directory ||
         S_ISDIR(f->linkmode);
}
...
__attribute__((always_inline)) inline static bool basename_is_dot_or_dotdot(const char *name) {
...
__attribute__((always_inline)) inline static void extract_dirs_from_files(char const *dirname,
                                    bool command_line_arg) {
  size_t i;
  size_t j;
  ...
  for (i = cwd_n_used; i-- != 0;) {
    ...
    if (is_directory(f) &&
      ...
      if (f->filetype == arg_directory) free_ent(f);
    }
  }
...
__attribute__((always_inline)) inline static int xstrcoll(char const *a, char const *b) {
  int diff;
  errno = 0;
  diff = strcoll(a, b);
  if (errno) {
...
#define DIRFIRST_CHECK(a, b)                                         \
  do {                                                               \
    ...
    if (a_is_dir && !b_is_dir) return -1; /* a goes before b */      \
    if (!a_is_dir && b_is_dir) return 1;  /* b goes before a */      \
  } while (0)
...
__attribute__((always_inline)) static inline int cmp_ctime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_ctime(&b->stat), get_stat_ctime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

__attribute__((always_inline)) static inline int cmp_mtime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_mtime(&b->stat), get_stat_mtime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

__attribute__((always_inline)) static inline int cmp_atime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_atime(&b->stat), get_stat_atime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

__attribute__((always_inline)) static inline int cmp_btime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_btime(&b->stat), get_stat_btime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

__attribute__((always_inline)) static inline int cmp_size(struct fileinfo const *a, struct fileinfo const *b,
                           int (*cmp)(char const *, char const *)) {
  int diff = longdiff(b->stat.st_size, a->stat.st_size);
  return diff ? diff : cmp(a->name, b->name);
}

__attribute__((always_inline)) static inline int cmp_name(struct fileinfo const *a, struct fileinfo const *b,
                           int (*cmp)(char const *, char const *)) {
...
__attribute__((always_inline)) static inline int cmp_extension(struct fileinfo const *a,
...
__attribute__((always_inline)) static inline int cmp_version(struct fileinfo const *a,
                              struct fileinfo const *b) {
  return filevercmp(a->name, b->name);
}

__attribute__((always_inline)) inline static int xstrcoll_version(V a, V b) { return cmp_version(a, b); }
__attribute__((always_inline)) inline static int rev_xstrcoll_version(V a, V b) { return cmp_version(b, a); }
__attribute__((always_inline)) inline static int xstrcoll_df_version(V a, V b) {
  DIRFIRST_CHECK(a, b);
  return cmp_version(a, b);
}
__attribute__((always_inline)) inline static int rev_xstrcoll_df_version(V a, V b) {
  DIRFIRST_CHECK(a, b);
...
__attribute__((always_inline)) inline static void initialize_ordering_vector(void) {
  for (size_t i = 0; i < cwd_n_used; i++) sorted_file[i] = &cwd_file[i];
}
...
__attribute__((always_inline)) inline static size_t align_nstrftime(char *buf, size_t size, bool recent,
...
__attribute__((always_inline)) inline static int long_time_expected_width(void) {
  static int width = -1;
  ...
  if (width < 0) {
    ...
    if (localtime_rz(localtz, &epoch, &tm)) {
      ...
      if (len != 0) width = mbsnwidth(buf, len, 0);
    }
...
__attribute__((always_inline)) inline static void format_user_or_group(char const *name, unsigned long int id,
                                 int width) {
  size_t len;
...
__attribute__((always_inline)) inline static void format_user(uid_t u, int width, bool stat_ok) {
...
__attribute__((always_inline)) inline static void format_group(gid_t g, int width, bool stat_ok) {
...
__attribute__((always_inline)) inline static int format_user_or_group_width(char const *name, unsigned long int id) {
  if (name) {
...
__attribute__((always_inline)) inline static char *format_inode(char *buf, size_t buflen, const struct fileinfo *f) {
...
__attribute__((always_inline)) inline static void print_long_format(const struct fileinfo *f) {
    ...
    char const *size =
        (!f->stat_ok ? "?"
                     : human_readable(unsigned_file_size(f->stat.st_size), hbuf,
                                      ...
                                      file_output_block_size));
    int pad;
    for (pad = file_size_width - mbswidth(size, 0); 0 < pad; pad--) *p++ = ' ';
    while ((*p++ = *size++)) continue;
    p[-1] = ' ';
  }
  ...
  *p = '\1';
  ...
  if (f->stat_ok && btime_ok &&
    ...
    if (timespec_cmp(current_time, when_timespec) < 0) gettime(&current_time);
...
__attribute__((always_inline)) inline static size_t quote_name_buf(char **inbuf, size_t bufsize, char *name,
  ...
  if (needs_general_quoting != 0) {
    ...
    len = strlen(name);
    if (bufsize <= len) buf = xmalloc(len + 1);
          ...
          default:
               ...
               character with a single question mark.  */
            {
              ...
              do {
                ...
                if (bytes == 0) /* A null wide character was encountered.  */
                  bytes = 1;
  ...
  *pad = (align_variable_outer_quotes && cwd_some_quoted && !quoted);
  ...
  if (width != NULL) *width = displayed_width;
...
__attribute__((always_inline)) inline static size_t quote_name_width(const char *name,
                               struct quoting_options const *options,
  ...
  if (buf != smallbuf && buf != name) free(buf);
...
__attribute__((always_inline)) inline static char *file_escape(const char *str, bool path) {
...
__attribute__((always_inline)) inline static size_t quote_name(char const *name,
  ...
  if (pad && allow_pad) DIRED_PUTCHAR(' ');

  if (color) print_color_indicator(color);
  ...
  if (stack) PUSH_CURRENT_DIRED_POS(stack);
  ...
  if (absolute_name) {
    ...
    if (skip_quotes) putchar(*(buf + len - 1));
  }
...
/* Given these arguments describing a file, return the single-byte
   type indicator, or 0.  */
__attribute__((always_inline)) inline static char get_type_indicator(bool stat_ok, mode_t mode, enum filetype type) {
  char c;
...
__attribute__((always_inline)) inline static bool print_type_indicator(bool stat_ok, mode_t mode,
                                 enum filetype type) {
  char c = get_type_indicator(stat_ok, mode, type);
  if (c) DIRED_PUTCHAR(c);
  return !!c;
}
...
get_color_indicator(const struct fileinfo *f, bool symlink_target) {
  ...
  if (type == C_LINK && !linkok) {
    if (color_symlink_as_referent || is_colored(C_ORPHAN)) type = C_ORPHAN;
  }
...
__attribute__((always_inline)) inline static size_t length_of_file_name_and_frills(const struct fileinfo *f) {
  size_t len = 0;
  char buf[MAX(LONGEST_HUMAN_READABLE + 1, INT_BUFSIZE_BOUND(uintmax_t))];
...
__attribute__((always_inline)) inline static void init_column_info(void) {
  size_t i;
...
__attribute__((always_inline)) inline static size_t calculate_columns(bool by_columns) {
  ...
  for (cols = max_cols; 1 < cols; --cols) {
    if (column_info[cols - 1].valid_len) break;
  }
