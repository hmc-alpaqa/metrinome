...
__attribute__((always_inline)) inline static char *realpath_canon(const char *fname, int can_mode) {
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE path_prefix(const char *prefix,
                                           const char *path) {
  ...
  if (*prefix == '/' && !prefix[1]) return *path == '/';
...
__attribute__((always_inline)) inline static bool isdir(const char *path) {
  struct stat sb;
  if (stat(path, &sb) != 0)
...
__attribute__((always_inline)) inline static bool process_path(const char *fname, int can_mode) {
  char *can_fname = realpath_canon(fname, can_mode);
  if (!can_fname) {
    if (verbose) error(0, errno, "%s", quotef(fname));
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  if (relative_base && !relative_to) relative_to = relative_base;

  bool need_dir = (can_mode & CAN_MODE_MASK) == CAN_EXISTING;
  if (relative_to) {
    can_relative_to = realpath_canon(relative_to, can_mode);
    if (!can_relative_to) die(EXIT_FAILURE, errno, "%s", quotef(relative_to));
  ...
  if (relative_base == relative_to)
    ...
    if (!base) die(EXIT_FAILURE, errno, "%s", quotef(relative_base));
  ...
  for (; optind < argc; ++optind) ok &= process_path(argv[optind], can_mode);
