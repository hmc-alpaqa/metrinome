...
__attribute__((always_inline)) inline static int ATTRIBUTE_WARN_UNUSED_RESULT statfs(char const *filename,
                                               struct fs_info *buf) {
...
static char const fmt_terse_regular[] =
    "%n %s %b %f %u %g %D %i %h %t %T"
    " %X %Y %Z %W %o\n";
static char const fmt_terse_selinux[] =
    "%n %s %b %f %u %g %D %i %h %t %T"
    " %X %Y %Z %W %o %C\n";
...
static char const *const cached_args[] = {"default", "never", "always", NULL};
...
__attribute__((always_inline)) inline static bool print_stat(char *pformat, size_t prefix_len, unsigned int m, int fd,
...
__attribute__((always_inline)) inline static char *ATTRIBUTE_WARN_UNUSED_RESULT human_time(struct timespec t) {
  ...
  static char str[INT_BUFSIZE_BOUND(intmax_t) + INT_STRLEN_BOUND(int) /* YYYY */
                  + 1 /* because YYYY might equal INT_MAX + 1900 */
                  + sizeof "-MM-DD HH:MM:SS.NNNNNNNNN +"];
  static timezone_t tz;
  if (!tz) tz = tzalloc(getenv("TZ"));
  struct tm tm;
  int ns = t.tv_nsec;
...
/* PFORMAT points to a '%' followed by a prefix of a format, all of
   ...
   append SUFFIX.  */
__attribute__((always_inline)) inline static void make_format(char *pformat, size_t prefix_len,
  ...
  for (src = dst; src < srclim && strchr(printf_flags, *src); src++)
    if (strchr(allowed_flags, *src)) *dst++ = *src;
  while (src < srclim) *dst++ = *src++;
  strcpy(dst, suffix);
}

__attribute__((always_inline)) inline static void out_string(char *pformat, size_t prefix_len, char const *arg) {
  make_format(pformat, prefix_len, "-", "s");
  printf(pformat, arg);
}
__attribute__((always_inline)) inline static int out_int(char *pformat, size_t prefix_len, intmax_t arg) {
  make_format(pformat, prefix_len, "'-+ 0", PRIdMAX);
  return printf(pformat, arg);
}
__attribute__((always_inline)) inline static int out_uint(char *pformat, size_t prefix_len, uintmax_t arg) {
  make_format(pformat, prefix_len, "'-0", PRIuMAX);
  return printf(pformat, arg);
}
__attribute__((always_inline)) inline static void out_uint_o(char *pformat, size_t prefix_len, uintmax_t arg) {
  make_format(pformat, prefix_len, "-#0", PRIoMAX);
  printf(pformat, arg);
}
__attribute__((always_inline)) inline static void out_uint_x(char *pformat, size_t prefix_len, uintmax_t arg) {
  make_format(pformat, prefix_len, "-#0", PRIxMAX);
  printf(pformat, arg);
}
__attribute__((always_inline)) inline static int out_minus_zero(char *pformat, size_t prefix_len) {
...
__attribute__((always_inline)) inline static void out_epoch_sec(char *pformat, size_t prefix_len,
                          struct timespec arg) {
...
out_file_context(char *pformat, size_t prefix_len, char const *filename) {
  ...
  if (scontext) freecon(scontext);
  return fail;
}

/* Print statfs info.  Return zero upon success, nonzero upon failure.  */
__attribute__((always_inline)) inline static bool ATTRIBUTE_WARN_UNUSED_RESULT print_statfs(char *pformat,
                                                      size_t prefix_len,
  ...
  switch (m) {
    ...
    case 'S': {
      ...
      if (!frsize) frsize = statfsbuf->f_bsize;
...
find_bind_mount(char const *name) {
  ...
  if (stat(name, &name_stats) != 0) return NULL;
...
__attribute__((always_inline)) static inline struct timespec neg_to_zero(struct timespec ts) {
  if (0 <= ts.tv_nsec) return ts;
...
__attribute__((always_inline)) inline static void getenv_quoting_style(void) {
  ...
  if (q_style) {
    ...
    if (0 <= i)
      ...
      error(0, 0,
            _("ignoring invalid value of environment "
...
__attribute__((always_inline)) inline static void print_esc_char(char c) {
  switch (c) {
    case 'a': /* Alert. */
      c = '\a';
      break;
...
__attribute__((always_inline)) inline static size_t _GL_ATTRIBUTE_PURE format_code_offset(char const *directive) {
  size_t len = strspn(directive + 1, printf_flags);
  char const *fmt_char = directive + len + 1;
  fmt_char += strspn(fmt_char, digits);
  if (*fmt_char == '.') fmt_char += 1 + strspn(fmt_char + 1, digits);
        ...
        switch (*fmt_char) {
          ...
          case '%':
            if (1 < len) {
              dest[len] = *fmt_char;
              dest[len + 1] = '\0';
              die(EXIT_FAILURE, 0, _("%s: invalid directive"), quote(dest));
            }
            putchar('%');
            break;
          default:
            fail |=
                print_func(dest, len, to_uchar(*fmt_char), fd, filename, data);
            break;
        }
        break;
      }
...
__attribute__((always_inline)) inline static bool ATTRIBUTE_WARN_UNUSED_RESULT do_statfs(char const *filename,
                                                   char const *format) {
  STRUCT_STATVFS statfsbuf;

  if (STREQ(filename, "-")) {
    error(0, 0,
          _("using %s to denote standard input does not work"
            " in file system mode"),
          quoteaf(filename));
    return false;
  }
...
#if USE_STATX
__attribute__((always_inline)) inline static unsigned int fmt_to_mask(char fmt) {
  switch (fmt) {
    case 'N':
      return STATX_MODE;
    case 'd':
...
__attribute__((always_inline)) inline static unsigned int _GL_ATTRIBUTE_PURE format_to_mask(char const *format) {
  unsigned int mask = 0;
  char const *b;
...
__attribute__((always_inline)) inline static bool ATTRIBUTE_WARN_UNUSED_RESULT do_stat(char const *filename,
                                                 char const *format,
                                                 char const *format2) {
  ...
  if (S_ISBLK(stx.stx_mode) || S_ISCHR(stx.stx_mode)) format = format2;
...
__attribute__((always_inline)) inline static struct timespec get_birthtime(int fd, char const *filename,
...
__attribute__((always_inline)) inline static bool ATTRIBUTE_WARN_UNUSED_RESULT do_stat(char const *filename,
  ...
  if (S_ISBLK(statbuf.st_mode) || S_ISCHR(statbuf.st_mode)) format = format2;
...
__attribute__((always_inline)) inline static char *default_format(bool fs, bool terse, bool device) {
  char *format;
  if (fs) {
    if (terse)
...
int main(int argc, char *argv[]) {
  ...
  if (format) {
    if (strstr(format, "%N")) getenv_quoting_style();
    format2 = format;
  } else {
