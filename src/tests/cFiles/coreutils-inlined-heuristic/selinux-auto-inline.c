...
*/
__attribute__((always_inline)) inline static security_class_t mode_to_security_class(mode_t m) {
  if (S_ISREG(m)) return string_to_security_class("file");
  if (S_ISDIR(m)) return string_to_security_class("dir");
  if (S_ISCHR(m)) return string_to_security_class("chr_file");
  if (S_ISBLK(m)) return string_to_security_class("blk_file");
  if (S_ISFIFO(m)) return string_to_security_class("fifo_file");
  if (S_ISLNK(m)) return string_to_security_class("lnk_file");
  if (S_ISSOCK(m)) return string_to_security_class("sock_file");
...
__attribute__((always_inline)) inline static int computecon(char const *path, mode_t mode, char **con) {
  char *scon = NULL;
  char *tcon = NULL;
  ...
  if (!dir) goto quit;
  if (getcon(&scon) < 0) goto quit;
  if (getfilecon(dir, &tcon) < 0) goto quit;
...
/*
  ...
  Returns -1 on failure.  errno will be set appropriately.
*/
__attribute__((always_inline)) inline int defaultcon(char const *path, mode_t mode) {
  int rc = -1;
  char *scon = NULL;
  char *tcon = NULL;
  ...
  if (!IS_ABSOLUTE_FILE_NAME(path)) {
    ...
    if (!newpath)
      die(EXIT_FAILURE, errno, _("error canonicalizing %s"), quoteaf(path));
    path = newpath;
  }
  ...
  if (computecon(path, mode, &tcon) < 0) goto quit;
  if (!(scontext = context_new(scon))) goto quit;
  if (!(tcontext = context_new(tcon))) goto quit;

  if (!(contype = context_type_get(scontext))) goto quit;
  if (context_type_set(tcontext, contype)) goto quit;
  if (!(constr = context_str(tcontext))) goto quit;
...
/*
  ...
  Returns -1 on failure.  errno will be set appropriately.
*/
__attribute__((always_inline)) inline static int restorecon_private(char const *path, bool local) {
  int rc = -1;
  struct stat sb;
  char *scon = NULL;
  char *tcon = NULL;
  ...
  if (local) {
    if (getfscreatecon(&tcon) < 0) return rc;
    if (!tcon) {
      errno = ENODATA;
      return rc;
    }
  ...
  if (fd == -1 && (errno != ELOOP)) goto quit;

  if (fd != -1) {
    if (fstat(fd, &sb) < 0) goto quit;
  } else {
    if (lstat(path, &sb) < 0) goto quit;
  }
  ...
  if (fd != -1) {
    if (fgetfilecon(fd, &tcon) < 0) goto quit;
  } else {
    if (lgetfilecon(path, &tcon) < 0) goto quit;
  }
...
*/
__attribute__((always_inline)) inline bool restorecon(char const *path, bool recurse, bool local) {
  char *newpath = NULL;
  FTS *fts;
  bool ok = true;
