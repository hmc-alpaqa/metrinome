...
__attribute__((always_inline)) inline extern void chopt_init(struct Chown_option *chopt) {
...
__attribute__((always_inline)) inline extern void chopt_free(struct Chown_option *chopt) {
...
__attribute__((always_inline)) inline extern char *gid_to_name(gid_t gid) {
...
__attribute__((always_inline)) inline extern char *uid_to_name(uid_t uid) {
  char buf[INT_BUFSIZE_BOUND(intmax_t)];
...
__attribute__((always_inline)) inline static char *user_group_str(char const *user, char const *group) {
  char *spec = NULL;
...
__attribute__((always_inline)) inline static void describe_change(const char *file, enum Change_status changed,
...
__attribute__((always_inline)) inline static enum RCH_status restricted_chown(int cwd_fd, char const *file,
...
__attribute__((always_inline)) inline static bool change_file_owner(FTS *fts, FTSENT *ent, uid_t uid, gid_t gid,
  ...
  if (do_chown) {
    ...
    if (do_chown && !ok && !chopt->force_silent)
      error(0, errno,
            (uid != (uid_t)-1 ? _("changing ownership of %s")
  ...
  if (!chopt->recurse) fts_set(fts, ent, FTS_SKIP);
...
/* Change the owner and/or group of the specified FILES.
   ...
   Return true if successful.  */
__attribute__((always_inline)) inline extern bool chown_files(char **files, int bit_flags, uid_t uid, gid_t gid,
                        uid_t required_uid, gid_t required_gid,
                        struct Chown_option const *chopt) {
  bool ok = true;
  ...
  while (1) {
    ...
    if (ent == NULL) {
      if (errno != 0) {
        ...
        if (!chopt->force_silent) error(0, errno, _("fts_read failed"));
        ok = false;
      }
      break;
    }
