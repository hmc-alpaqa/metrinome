...
#define SKIP_SOMETHING(cursor, limit)                                       \
  if (word_regex.string) {                                                  \
    ...
    if (count == -2) matcher_error();                                       \
    cursor += count == -1 ? 1 : count;                                      \
  } else if (word_fastmap[to_uchar(*cursor)])                               \
    while (cursor < limit && word_fastmap[to_uchar(*cursor)]) cursor++;     \
  else                                                                      \
    cursor++
...
__attribute__((always_inline)) inline static void ATTRIBUTE_NORETURN matcher_error(void) {
...
__attribute__((always_inline)) inline static char *copy_unescaped_string(const char *string) {
...
__attribute__((always_inline)) inline static void compile_regex(struct regex_data *regex) {
...
__attribute__((always_inline)) inline static void initialize_regex(void) {
  ...
  if (context_regex.string) {
    if (!*context_regex.string) context_regex.string = NULL;
  } else if (gnu_extensions && !input_reference)
    context_regex.string = "[.?!][]\"')}]*\\($\\|\t\\|  \\)[ \t\n]*";
  else
    context_regex.string = "\n";

  if (context_regex.string) compile_regex(&context_regex);
...
__attribute__((always_inline)) inline static void swallow_file_in_memory(const char *file_name, BLOCK *block) {
...
__attribute__((always_inline)) inline static int compare_words(const void *void_first, const void *void_second) {
...
__attribute__((always_inline)) inline static int compare_occurs(const void *void_first, const void *void_second) {
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE search_table(WORD *word, WORD_TABLE *table) {
...
__attribute__((always_inline)) inline static void sort_found_occurs(void) {
...
__attribute__((always_inline)) inline static void digest_break_file(const char *file_name) {
...
__attribute__((always_inline)) inline static void digest_word_file(const char *file_name, WORD_TABLE *table) {
  BLOCK file_contents; /* to receive a copy of the file */
  char *cursor;        /* cursor in file copy */
  ...
  while (cursor < file_contents.end) {
    ...
    if (cursor < file_contents.end) cursor++;
  }
...
__attribute__((always_inline)) inline static void find_occurs_in_text(int file_index) {
  ...
  for (cursor = text_buffer->start; cursor < text_buffer->end;
    ...
    while (1) {
      ...
      {
        scan = cursor;
        while (scan < context_end && !word_fastmap[to_uchar(*scan)]) scan++;
      ...
      if (input_reference) {
        while (line_scan < possible_key.start)
          if (*line_scan == '\n') {
            ...
            line_scan++;
        if (line_scan > possible_key.start) continue;
      }
      ...
      if (ignore_file && search_table(&possible_key, &ignore_table)) continue;
      if (only_file && !search_table(&possible_key, &only_table)) continue;
...
__attribute__((always_inline)) inline static void print_spaces(ptrdiff_t number) {
  for (ptrdiff_t counter = number; counter > 0; counter--) putchar(' ');
}
...
__attribute__((always_inline)) inline static void print_field(BLOCK field) {
...
__attribute__((always_inline)) inline static void fix_output_parameters(void) {
  ...
  if (auto_reference) {
    ...
    for (file_index = 0; file_index < number_input_files; file_index++) {
      ...
      if (file_index > 0) line_ordinal -= file_line_count[file_index - 1];
  ...
  if (gnu_extensions) {
    ...
    if (before_max_width < 0) before_max_width = 0;
...
__attribute__((always_inline)) inline static void define_all_fields(OCCURS *occurs) {
  ...
  if (cursor <= keyafter.start + keyafter_max_width) keyafter.end = cursor;
  ...
  if (tail_max_width > 0) {
    ...
    if (cursor < tail.start + tail_max_width) tail.end = cursor;
...
__attribute__((always_inline)) inline static void output_one_roff_line(void) {
  /* Output the 'tail' field.  */

  printf(".%s \"", macro_name);
  print_field(tail);
  if (tail_truncation) fputs(truncation_string, stdout);
  putchar('"');

  /* Output the 'before' field.  */

  fputs(" \"", stdout);
  if (before_truncation) fputs(truncation_string, stdout);
  print_field(before);
  putchar('"');
  ...
  if (keyafter_truncation) fputs(truncation_string, stdout);
  putchar('"');

  /* Output the 'head' field.  */

  fputs(" \"", stdout);
  if (head_truncation) fputs(truncation_string, stdout);
  print_field(head);
  putchar('"');
...
__attribute__((always_inline)) inline static void output_one_tex_line(void) {
...
__attribute__((always_inline)) inline static void output_one_dumb_line(void) {
  if (!right_reference) {
    if (auto_reference) {
...
__attribute__((always_inline)) inline static void generate_all_output(void) {
...
int main(int argc, char **argv) {
  ...
  while (optchar = getopt_long(
             argc, argv, "AF:GM:ORS:TW:b:i:fg:o:trw:", long_options, NULL),
         optchar != EOF) {
    switch (optchar) {
      default:
        usage(EXIT_FAILURE);
      ...
      case 'g': {
        intmax_t tmp;
        if (!(xstrtoimax(optarg, NULL, 0, &tmp, "") == LONGINT_OK && 0 < tmp &&
              tmp <= PTRDIFF_MAX))
          die(EXIT_FAILURE, 0, _("invalid gap width: %s"), quote(optarg));
        gap_size = tmp;
        break;
      }
      ...
      case 'w': {
        ...
        if (!(xstrtoimax(optarg, NULL, 0, &tmp, "") == LONGINT_OK && 0 < tmp &&
              tmp <= PTRDIFF_MAX))
          die(EXIT_FAILURE, 0, _("invalid line width: %s"), quote(optarg));
        line_width = tmp;
        break;
      }
      ...
      case 'W':
        ...
        if (!*word_regex.string) word_regex.string = NULL;
        break;
  ...
  if (break_file) digest_break_file(break_file);
  ...
  if (ignore_file) {
    ...
    if (ignore_table.length == 0) ignore_file = NULL;
  }

  if (only_file) {
    digest_word_file(only_file, &only_table);
    if (only_table.length == 0) only_file = NULL;
  }
