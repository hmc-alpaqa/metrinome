...
__attribute__((always_inline)) inline static void launch_program(const char *prog_name, int prog_argc,
                           char **prog_argv) {
  int (*prog_main)(int, char **) = NULL;

  /* Ensure that at least one parameter was passed.  */
  if (!prog_argc || !prog_argv || !prog_argv[0] || !prog_name) return;
...
int main(int argc, char **argv) {
  ...
  if (STREQ(prog_name, "install")) prog_name = ginstall;
  ...
  if (argc >= 2) {
    ...
    if (nskip) {
      ...
      die(EXIT_FAILURE, 0, _("unknown program %s"), quote(prog_name));
    }
  }
