...
__attribute__((always_inline)) inline static bool strict_posix2(void) {
...
__attribute__((always_inline)) inline static size_t size_opt(char const *opt, char const *msgid) {
  uintmax_t size;
...
__attribute__((always_inline)) inline static char *_GL_ATTRIBUTE_PURE find_field(struct linebuffer const *line) {
  size_t count;
...
__attribute__((always_inline)) inline static bool different(char *old, char *new, size_t oldlen, size_t newlen) {
  if (check_chars < oldlen) oldlen = check_chars;
  if (check_chars < newlen) newlen = check_chars;
...
__attribute__((always_inline)) inline static void writeline(struct linebuffer const *line, bool match,
                      uintmax_t linecount) {
  if (!(linecount == 0
            ? output_unique
  ...
  if (countmode == count_occurrences) printf("%7" PRIuMAX " ", linecount + 1);
...
__attribute__((always_inline)) inline static void check_file(const char *infile, const char *outfile,
                       char delimiter) {
  ...
  */
  if (output_unique && output_first_repeated && countmode == count_none) {
    ...
    while (!feof(stdin)) {
      ...
      if (readlinebuffer_delim(thisline, stdin, delimiter) == 0) break;
    ...
    if (readlinebuffer_delim(prevline, stdin, delimiter) == 0) goto closefiles;
    ...
    while (!feof(stdin)) {
      ...
      if (readlinebuffer_delim(thisline, stdin, delimiter) == 0) {
        if (ferror(stdin)) goto closefiles;
        break;
      }
      thisfield = find_field(thisline);
      thislen = thisline->length - 1 - (thisfield - thisline->buffer);
  ...
  while (true) {
    ...
    if (optc == -1 || (posixly_correct && nfiles != 0) ||
      ...
      switch (optc) {
        ...
        case '8':
        case '9': {
          if (skip_field_option_type == SFO_NEW) skip_fields = 0;
        ...
        case 'D':
          ...
          if (optarg == NULL)
            ...
            delimit_groups =
                XARGMATCH("--all-repeated", optarg, delimit_method_string,
                          delimit_method_map);
          output_option_used = true;
          break;
        ...
        case 'f':
          ...
          skip_fields =
              size_opt(optarg, N_("invalid number of fields to skip"));
          break;
