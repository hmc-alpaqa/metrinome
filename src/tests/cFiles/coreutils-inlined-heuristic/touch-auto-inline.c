...
__attribute__((always_inline)) inline static void get_reldate(struct timespec *result, char const *flex_date,
...
__attribute__((always_inline)) inline static bool touch(const char *file) {
  bool ok;
  int fd = -1;
  int open_errno = 0;
  ...
  if (fd == STDIN_FILENO) {
    ...
    if (!ok && errno == EBADF && no_create) return true;
  }
  ...
  if (!ok) {
    if (open_errno) {
      ...
      if (no_create && errno == ENOENT) return true;
...
int main(int argc, char **argv) {
    ...
    switch (c) {
      ...
      case 't':
        if (!posixtime(&newtime[0].tv_sec, optarg,
          ...
          die(EXIT_FAILURE, 0, _("invalid date format %s"), quote(optarg));
        newtime[0].tv_nsec = 0;
        newtime[1] = newtime[0];
        date_set = true;
        break;
  ...
  if (change_times == 0) change_times = CH_ATIME | CH_MTIME;
  ...
  for (; optind < argc; ++optind) ok &= touch(argv[optind]);
