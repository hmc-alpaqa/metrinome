...
__attribute__((always_inline)) inline static int parse_size(char const *str, size_t *size) {
  uintmax_t tmp_size;
  enum strtol_error e = xstrtoumax(str, NULL, 10, &tmp_size, "EGkKMPTYZ0");
  if (e == LONGINT_OK && SIZE_MAX < tmp_size) e = LONGINT_OVERFLOW;
...
__attribute__((always_inline)) inline static void set_program_path(const char *arg) {
...
__attribute__((always_inline)) inline static int optc_to_fileno(int c) {
  int ret = -1;
...
__attribute__((always_inline)) inline static void set_LD_PRELOAD(void) {
  int ret;
#ifdef __APPLE__
  ...
  while (true) {
    ...
    ++path;
    if (!*path) die(EXIT_CANCELED, 0, _("failed to find %s"), quote(LIB_NAME));
  }
...
#ifdef __APPLE__
  if (ret == 0) ret = setenv("DYLD_FORCE_FLAT_NAMESPACE", "y", 1);
#endif

  if (ret != 0) {
    die(EXIT_CANCELED, errno, _("failed to update the environment with %s"),
        quote(LD_PRELOAD));
  }
}
...
__attribute__((always_inline)) inline static bool set_libstdbuf_options(void) {
  bool env_set = false;
