...
#define output_char(c)                          \
  do {                                          \
    obuf[oc++] = (c);                           \
    if (oc >= output_blocksize) write_output(); \
  } while (0)
...
__attribute__((always_inline)) inline static void maybe_close_stdout(void) {
...
"),
    ...
    if (O_DIRECTORY) fputs(_("  directory  fail unless a directory\n"), stdout);
    if (O_DSYNC)
...
__attribute__((always_inline)) inline static void alloc_ibuf(void) {
  if (ibuf) return;
  ...
  if (!real_buf) {
    ...
    die(EXIT_FAILURE, 0,
        ...
        ibs,
        human_readable(input_blocksize, hbuf, human_opts | human_base_1024, 1,
                       1));
  }
...
__attribute__((always_inline)) inline static void alloc_obuf(void) {
  if (obuf) return;
    ...
    if (!real_obuf) {
      ...
      die(EXIT_FAILURE, 0,
          _("memory exhausted by output buffer of size %" PRIuMAX
            " bytes (%s)"),
          obs,
          human_readable(output_blocksize, hbuf, human_opts | human_base_1024,
                         1, 1));
    }
...
__attribute__((always_inline)) inline static void translate_charset(char const *new_trans) {
  for (int i = 0; i < 256; i++) trans_table[i] = new_trans[trans_table[i]];
  translation_needed = true;
}

/* Return true if I has more than one bit set.  I must be nonnegative.  */

__attribute__((always_inline)) static inline bool multiple_bits_set(int i) { return MULTIPLE_BITS_SET(i); }

__attribute__((always_inline)) inline static bool abbreviation_lacks_prefix(char const *message) {
  return message[strlen(message) - 2] == ' ';
}

/* Print transfer statistics.  */

__attribute__((always_inline)) inline static void print_xfer_stats(xtime_t progress_time) {
  ...
  if (progress_time) fputc('\r', stderr);
  ...
  int stats_len =
      (abbreviation_lacks_prefix(si)
           ...
           : abbreviation_lacks_prefix(iec)
                 ? fprintf(stderr, _("%" PRIuMAX " bytes (%s) copied, %s, %s"),
...
__attribute__((always_inline)) inline static void print_stats(void) {
...
__attribute__((always_inline)) inline static void interrupt_handler(int sig) {
  if (!SA_RESETHAND) signal(sig, SIG_DFL);
  interrupt_signal = sig;
}

/* An info signal was received; arrange for the program to print status.  */

__attribute__((always_inline)) inline static void siginfo_handler(int sig) {
  if (!SA_NOCLDSTOP) signal(sig, siginfo_handler);
  info_signal_count++;
}

/* Install the signal handlers.  */

__attribute__((always_inline)) inline static void install_signal_handlers(void) {
  ...
  if (catch_siginfo) sigaddset(&caught_signals, SIGINFO);
  sigaction(SIGINT, NULL, &act);
  if (act.sa_handler != SIG_IGN) sigaddset(&caught_signals, SIGINT);
...
__attribute__((always_inline)) inline static int iclose(int fd) {
  if (close(fd) != 0) do
...
__attribute__((always_inline)) inline static void cleanup(void) {
  ...
  if (iclose(STDOUT_FILENO) != 0)
    die(EXIT_FAILURE, errno, _("closing output file %s"), quoteaf(output_file));
}
...
__attribute__((always_inline)) inline static void process_signals(void) {
  while (interrupt_signal || info_signal_count) {
    ...
    if (infos) info_signal_count = infos - 1;
...
__attribute__((always_inline)) inline static void finish_up(void) {
...
__attribute__((always_inline)) inline static void ATTRIBUTE_NORETURN quit(int code) {
  finish_up();
  exit(code);
}
...
__attribute__((always_inline)) inline static off_t cache_round(int fd, off_t len) {
...
__attribute__((always_inline)) inline static bool invalidate_cache(int fd, off_t len) {
  int adv_ret = -1;
  off_t offset;
  ...
  if (0 <= offset) {
    ...
    if (clen == 0) offset -= offset % page_size;
...
__attribute__((always_inline)) inline static ssize_t iread(int fd, char *buf, size_t size) {
  ssize_t nread;
  ...
  do {
    ...
    if (nread == -1 && errno == EINVAL && 0 < prev_nread && prev_nread < size &&
  ...
  if (0 < nread && nread < size) process_signals();
  ...
  if (0 < nread && warn_partial_read) {
    if (0 < prev_nread && prev_nread < size) {
      ...
      if (status_level != STATUS_NONE)
        error(0, 0,
              ngettext(("warning: partial read (%" PRIuMAX " byte); "
...
__attribute__((always_inline)) inline static ssize_t iread_fullblock(int fd, char *buf, size_t size) {
  ssize_t nread = 0;

  while (0 < size) {
    ssize_t ncurr = iread(fd, buf, size);
    if (ncurr < 0) return ncurr;
    if (ncurr == 0) break;
    nread += ncurr;
    buf += ncurr;
    size -= ncurr;
...
__attribute__((always_inline)) inline static size_t iwrite(int fd, char const *buf, size_t size) {
  ...
  if (o_nocache && total_written) invalidate_cache(fd, total_written);
...
__attribute__((always_inline)) inline static void write_output(void) {
...
__attribute__((always_inline)) inline static int ifdatasync(int fd) {
  int ret;
...
__attribute__((always_inline)) inline static int ifd_reopen(int desired_fd, char const *file, int flag, mode_t mode) {
  int ret;
...
__attribute__((always_inline)) inline static int ifstat(int fd, struct stat *st) {
  int ret;
...
__attribute__((always_inline)) inline static int ifsync(int fd) {
  int ret;
...
__attribute__((always_inline)) inline static int iftruncate(int fd, off_t length) {
  int ret;
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE operand_matches(char const *str,
                                               char const *pattern,
                                               char delim) {
  while (*pattern)
...
__attribute__((always_inline)) inline static int parse_symbols(char const *str, struct symbol_value const *table,
...
__attribute__((always_inline)) inline static uintmax_t parse_integer(const char *str, strtol_error *invalid) {
  uintmax_t n;
  char *suffix;
...
__attribute__((always_inline)) inline static bool _GL_ATTRIBUTE_PURE operand_is(char const *operand,
                                          char const *name) {
  return operand_matches(operand, name, '=');
}

__attribute__((always_inline)) inline static void scanargs(int argc, char *const *argv) {
  size_t blocksize = 0;
  ...
  for (int i = optind; i < argc; i++) {
    ...
    if (operand_is(name, "if"))
      ...
      if (invalid != LONGINT_OK)
        die(EXIT_FAILURE, invalid == LONGINT_OVERFLOW ? EOVERFLOW : 0, "%s: %s",
            _("invalid number"), quote(val));
    }
  }
  ...
  if (input_blocksize == 0) input_blocksize = DEFAULT_BLOCKSIZE;
  if (output_blocksize == 0) output_blocksize = DEFAULT_BLOCKSIZE;
  if (conversion_blocksize == 0) conversions_mask &= ~(C_BLOCK | C_UNBLOCK);
...
__attribute__((always_inline)) inline static void apply_translations(void) {
  int i;

  if (conversions_mask & C_ASCII) translate_charset(ebcdic_to_ascii);

  if (conversions_mask & C_UCASE) {
    for (i = 0; i < 256; i++) trans_table[i] = toupper(trans_table[i]);
    translation_needed = true;
  } else if (conversions_mask & C_LCASE) {
    for (i = 0; i < 256; i++) trans_table[i] = tolower(trans_table[i]);
    translation_needed = true;
  }
...
__attribute__((always_inline)) inline static void translate_buffer(char *buf, size_t nread) {
  size_t i;
  char *cp;
  for (i = nread, cp = buf; i; i--, cp++) *cp = trans_table[to_uchar(*cp)];
}
...
__attribute__((always_inline)) inline static char *swab_buffer(char *buf, size_t *nread) {
  char *bufstart = buf;
  ...
  for (size_t i = *nread / 2; i; i--, cp -= 2) *cp = *(cp - 2);
...
__attribute__((always_inline)) inline static void advance_input_offset(uintmax_t offset) {
  input_offset += offset;
  if (input_offset < offset) input_offset_overflow = true;
}
...
__attribute__((always_inline)) inline static off_t skip_via_lseek(char const *filename, int fdesc, off_t offset,
                            int whence) {
  struct mtget s1;
  struct mtget s2;
  ...
  if (0 <= new_position && got_original_tape_position &&
    ...
    if (status_level != STATUS_NONE)
      error(0, 0,
            _("warning: working around lseek kernel bug for file "
              "(%s)\n  of mt_type=0x%0lx -- "
...
__attribute__((always_inline)) inline static uintmax_t skip(int fdesc, char const *file, uintmax_t records,
...
__attribute__((always_inline)) inline static bool advance_input_after_read_error(size_t nbytes) {
  if (!input_seekable) {
    if (input_seek_errno == ESPIPE) return true;
    ...
    if (0 <= offset) {
      off_t diff;
      if (offset == input_offset) return true;
      ...
      if (0 <= skip_via_lseek(input_file, STDIN_FILENO, diff, SEEK_CUR))
        return true;
      if (errno == 0) error(0, 0, _("cannot work around kernel bug after all"));
    }
  }
...
__attribute__((always_inline)) inline static void copy_simple(char const *buf, size_t nread) {
...
__attribute__((always_inline)) inline static void copy_with_block(char const *buf, size_t nread) {
...
__attribute__((always_inline)) inline static void copy_with_unblock(char const *buf, size_t nread) {
...
__attribute__((always_inline)) inline static void set_fd_flags(int fd, int add_flags, char const *name) {
  ...
  if (add_flags) {
    ...
    if (!ok) die(EXIT_FAILURE, errno, _("setting flags for %s"), quoteaf(name));
  }
}

/* The main loop.  */

__attribute__((always_inline)) inline static int dd_copy(void) {
  ...
  if (max_records == 0 && max_bytes == 0) return exit_status;
  ...
  while (1) {
    ...
    if (r_partial + r_full >= max_records + !!max_bytes) break;
    ...
    if (nread > 0) {
      advance_input_offset(nread);
      if (i_nocache) invalidate_cache(STDIN_FILENO, nread);
    } else if (nread == 0) {
    ...
    if (translation_needed) translate_buffer(ibuf, n_bytes_read);
...
int main(int argc, char **argv) {
  ...
  if (output_file == NULL) {
    ...
    if (seek_records != 0 && !(conversions_mask & C_NOTRUNC)) {
      ...
      if (iftruncate(STDOUT_FILENO, size) != 0) {
        ...
        struct stat stdout_stat;
        if (ifstat(STDOUT_FILENO, &stdout_stat) != 0)
          die(EXIT_FAILURE, errno, _("cannot fstat %s"), quoteaf(output_file));
  ...
  if (max_records == 0 && max_bytes == 0) {
    ...
    if (i_nocache || i_nocache_eof) invalidate_cache(STDIN_FILENO, 0);
    if (o_nocache || o_nocache_eof) invalidate_cache(STDOUT_FILENO, 0);
  }
