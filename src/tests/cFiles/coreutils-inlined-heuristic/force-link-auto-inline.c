...
__attribute__((always_inline)) inline static char *samedir_template(char const *dstname, char buf[smallsize]) {
...
__attribute__((always_inline)) inline static int try_link(char *dest, void *arg) {
...
__attribute__((always_inline)) inline extern int force_linkat(int srcdir, char const *srcname, int dstdir,
                        char const *dstname, int flags, bool force,
                        int linkat_errno) {
  if (linkat_errno < 0)
    linkat_errno =
        (linkat(srcdir, srcname, dstdir, dstname, flags) == 0 ? 0 : errno);
  if (!force || linkat_errno != EEXIST) return linkat_errno;

  char buf[smallsize];
  char *dsttmp = samedir_template(dstname, buf);
  if (!dsttmp) return errno;
...
__attribute__((always_inline)) inline static int try_symlink(char *dest, void *arg) {
...
/* Create a symlink containing SRCNAME in directory DSTDIR's file DSTNAME.
   ...
   0 if successful and DSTNAME did not already exist, and
   a positive errno value on failure.  */
__attribute__((always_inline)) inline extern int force_symlinkat(char const *srcname, int dstdir, char const *dstname,
                           bool force, int symlinkat_errno) {
  if (symlinkat_errno < 0)
    symlinkat_errno = symlinkat(srcname, dstdir, dstname) == 0 ? 0 : errno;
  if (!force || symlinkat_errno != EEXIST) return symlinkat_errno;
