...
#if HAVE_STATX && defined STATX_INO
static unsigned int _GL_ATTRIBUTE_PURE time_type_to_statx(void) {
  switch (time_type) {
    case time_ctime:
      return STATX_CTIME;
    case time_mtime:
...
static unsigned int _GL_ATTRIBUTE_PURE calc_req_mask(void) {
...
static int do_statx(int fd, const char *name, struct stat *st, int flags,
                    unsigned int mask) {
  struct statx stx;
...
static inline int do_stat(const char *name, struct stat *st) {
...
static inline int stat_for_mode(const char *name, struct stat *st) {
  return do_statx(AT_FDCWD, name, st, 0, STATX_MODE);
}

/* dev+ino should be static, so no need to sync with backing store */
static inline int stat_for_ino(const char *name, struct stat *st) {
...
static char const *_GL_ATTRIBUTE_PURE first_percent_b(char const *fmt) {
  for (; *fmt; fmt++)
...
static inline void signal_init(void) { signal_setup(true); }

static inline void signal_restore(void) { signal_setup(false); }
...
static bool patterns_match(struct ignore_pattern const *patterns,
                           char const *file) {
...
static inline int cmp_btime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
...
static size_t align_nstrftime(char *buf, size_t size, bool recent,
