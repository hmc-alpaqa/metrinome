...
static void append_unset_var(const char *var) {
...
static bool _GL_ATTRIBUTE_PURE valid_escape_sequence(const char c) {
  return (c == 'c' || c == 'f' || c == 'n' || c == 'r' || c == 't' ||
          c == 'v' || c == '#' || c == '$' || c == '_' || c == '"' ||
          c == '\'' || c == '\\');
}

static char _GL_ATTRIBUTE_PURE escape_char(const char c) {
  switch (c) {
...
static const char *_GL_ATTRIBUTE_PURE scan_varname(const char *str) {
...
static char *extract_varname(const char *str) {
  ptrdiff_t i;
  const char *p;
...
static void validate_split_str(const char *str, size_t * /*out*/ bufsize,
                               int * /*out*/ maxargc) {
  bool dq, sq, sp;
  const char *pch;
  size_t buflen;
...
/* Return a newly-allocated *arg[]-like array,
   ...
   To free allocated memory:
     free (argv[0]);
     free (argv); */
static char **build_argv(const char *str, int extra_argc) {
...
static void parse_split_string(const char *str, int /*out*/ *orig_optind,
                               int /*out*/ *orig_argc,
                               char *** /*out*/ orig_argv) {
  int i, newargc;
  char **newargv, **nextargv;
...
static void parse_signal_action_params(const char *optarg, bool set_default) {
...
static void parse_block_signal_params(const char *optarg, bool block) {
  char signame[SIG2STR_MAX];
  char *opt_sig;
  char *optarg_writable;
