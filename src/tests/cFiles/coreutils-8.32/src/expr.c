...
static char *_GL_ATTRIBUTE_MALLOC mpz_get_str(char const *str, int base,
                                              mpz_t z) {
  (void)str;
  (void)base;
...
static size_t mbs_logical_cspn(const char *s, const char *accept) {
  size_t idx = 0;
...
static char *mbs_logical_substr(const char *s, size_t pos, size_t len) {
  char *v, *vlim;
...
static size_t mbs_offset_to_chars(const char *s, size_t ofs) {
  mbui_iterator_t iter;
  size_t c = 0;
  for (mbui_init(iter, s); mbui_avail(iter); mbui_advance(iter)) {
...
static void integer_overflow(char op) { die(EXPR_FAILURE, ERANGE, "%c", op); }
#endif
