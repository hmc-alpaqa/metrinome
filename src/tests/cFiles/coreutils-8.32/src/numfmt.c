...
/* Read a 'human' INPUT_STR represented as "NNNN[.NNNNN] + suffix",
   ...
   Returns:
      ...
      SSE_INVALID_SUFFIX
      SSE_MISSING_I_SUFFIX  */
static enum simple_strtod_error simple_strtod_human(
...
static enum simple_strtod_error parse_human_number(
...
static bool _GL_ATTRIBUTE_PURE include_field(uintmax_t field) {
...
static bool process_field(char *text, uintmax_t field) {
  long double val = 0;
  size_t precision = 0;
