"""
The same as fc_path_complexity in metrics, but with time breakdown
Uses old eliminate function, but with some small fixes for correctness
Uses new numNodes, counts fcn_call multiple times, does not count recursive calls
Uses old solve -> nsolve after 10 seconds
Has the time breakdown
"""
...
class FunctionCallPathComplexity(ABC):
    ...
    def fcn_call_apc(self, edgelist, call_list):
        ...
        apc_and_time = {"apc":apc, "pc":pc, "graphSystemsTime":graphSystemsTime, "gammaTime": gammaTime, "discrimTime":discrimTime, 
                "realnrootsTime":realnrootsTime, "coeffsTime": coeffsTime, "exprsTime": exprsTime, "genFuncTime":genFuncTime,
