...
int power_of_two_rec(int n) {
    if(n==0) {
        return 1;
    }
    return power_of_two_rec(n-1) + power_of_two_rec(n-1);
}

int power_of_two_rec_split_calls(int n) {
    if(n==0) {
        return 1;
    }
    int a = power_of_two_rec(n-1);
    int b = power_of_two_rec(n-1);
    return a + b;
}
...
// recursive and iterative// int fib_rec_split_calls(int n)
{
    if (n <= 1) {
      return n;
    } else {
...
int max_value_iter_no_int_min(int A[], int n)
{
    int best = 999999999;
    for (int i = 0; i < n; i++) {
        if (best < A[i]) {
...
int last_value_iter(int A[], int n) {
  for (int i = 1; i <= n; i++) {
    if (i == n) {
      return A[i-1];
    }
...
int last_value_rec(int A[], int n) {
  if (n == 1) {
    return A[0];
  } else {
    return last_value_iter(A + 1, n - 1);
  }
}

void count_from_n_iter(int n) {
  for (int i = n; i > 0; i--) {
    int a = 5;
  }
}

void count_to_n_rec(int n) {
  int a = 5;
  if (n > 1) {
    count_to_n_rec(n - 1);
  }
...
int ackermann_rec_split_calls(int m, int n)
{
    if (m == 0) {
        return n + 1;
    } else if (n == 0) {
        ...
        int a = ackermann_rec(m, n - 1);
        return ackermann_rec(m - 1, a);
    }
}
